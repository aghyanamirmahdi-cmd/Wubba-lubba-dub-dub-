...
class ColumnRef:
    ...
    def notin_(self, values):
        return Predicate(self, "notin", list(values))

    def isnot(self, other):
        return Predicate(self, "isnot", other)

    def is_(self, other):
        return Predicate(self, "is", other)
...
class Select:
    def __init__(self, *entities):
        ...
        self._order_bys: List[Tuple[ColumnRef, bool]] = []
    ...
    def order_by(self, *cols_or_desc):
        for col_or_desc in cols_or_desc:
            if isinstance(col_or_desc, DescMarker):
                self._order_bys.append((col_or_desc.colref, True))
            else:
                self._order_bys.append((col_or_desc, False))
        return self
...
class JsonSession:
    ...
    def _exec_select(self, query: Select) -> Result:
        ...
        # ordering -- apply each column in turn, least-significant first,
        # relying on Python's sort stability so the end result respects
        # every column (e.g. .order_by(A.desc(), B.asc()) sorts by A desc
        # and breaks ties by B asc), instead of only the single column
        # that used to survive here.
        for ob, is_desc in reversed(query._order_bys):
            contexts.sort(
