...
class Advisor:
    async def analyze_user(self, user_id: int, session: AsyncSession) -> Dict[str, Any]:
        ...
        if profile.attendance_rate < 0.4:
            insights.append(
                "📵 در بیشتر جلسات اخیر شرکت نکردی. اگه دلیلش زمان‌بندیه، از تنظیمات می‌تونی ساعت مناسب‌ترت رو مشخص کنی."
            )
        elif profile.attendance_rate > 0.85:
            insights.append("✅ حضورت در جلسات عالیه. همین ثبات، بزرگ‌ترین امتیازته.")
        ...
        if profile.momentum_score < 0.3 and profile.recovery_score < 0.3:
            insights.append(
                "🌧️ این روزها هم شتاب مطالعه‌ات کم شده هم برگشتنت بعد از وقفه‌ها کند بوده. "
        ...
        if profile.long_session_tolerance > 0.7 and profile.short_session_response < 0.4:
            insights.append("⏳ در پارت‌های بلندتر بهتر ظاهر می‌شی؛ پارت‌های خیلی کوتاه ریتمت رو نمی‌گیره.")
        elif profile.short_session_response > 0.7 and profile.long_session_tolerance < 0.4:
            insights.append("⏱ پارت‌های کوتاه‌تر برات مؤثرترن؛ لازم نیست خودتو مجبور به پارت‌های طولانی کنی.")
        ...
        if (profile.maximum_observed_capacity - profile.current_capacity >= 15
            ...
            insights.append(
                f"🏅 یه بار تا {int(profile.maximum_observed_capacity)} دقیقه پیش رفتی. "
                f"با این روند، رسیدن به همون سطح دوباره دور از دسترس نیست."
            )
        ...
        if profile.average_session > 0:
            ...
            if delta_ratio >= 0.2:
                insights.append("📈 پارت‌های اخیرت نسبت به میانگین کلی‌ت بلندتر شده -- ظرفیتت داره واقعاً رشد می‌کنه.")
            elif delta_ratio <= -0.2:
                insights.append("📉 پارت‌های اخیرت کوتاه‌تر از میانگین همیشگی‌ته. طبیعیه، فقط بدون این یه علامت برای کوتاه‌تر گرفتن پارت بعدیه.")
    ...
    async def get_recovery_suggestion(self, user_id: int, session: AsyncSession) -> str:
        ...
        return (
            ...
            "🔬 چرا این‌طوری: در روان‌شناسی رفتاری، کوچک شروع‌کردن بعد از یک وقفه "
            "(نه جبران یک‌جای چیزی که از دست رفته) شانس واقعی برگشت به عادت رو بیشتر می‌کنه.\n"
            "آماده‌ای؟"
        )
    ...
    async def get_growth_suggestion(self, user_id: int, session: AsyncSession) -> str:
        ...
        return (
            ...
            "🔬 چرا این‌طوری: افزایش پله‌ای و کوچک (نه پرش ناگهانی) همون اصل «چالش مطلوب» "
            "است -- به‌اندازه‌ای سخت که پیشرفت بده، نه اونقدر که باعث افت انگیزه بشه.\n"
            "آماده‌ای؟"
        )
    ...
    async def get_calm_suggestion(self, user_id: int, session: AsyncSession) -> str:
        ...
        profile = await profile_repo.get_by_user_id(user_id)
        if not profile:
            return "🌙 پیشنهاد می‌کنم امشب رو با یک پارت کوتاه و آروم جمع کنی."
        comfort = profile.comfort_capacity
        ...
        rationale = (
            "🔬 چرا این‌طوری: تثبیت حافظه بیشتر در خواب اتفاق می‌افته، نه با فشار آوردن نزدیک زمان خواب -- "
            "یه مرور سبک بهتر از یه پارت سنگین دیر وقته."
            if is_night else
            "🔬 چرا این‌طوری: وقتی بار امروزت زیاده، یه پارت سبک‌تر همون نتیجه‌ی حفظ ریتم رو می‌ده، "
            "بدون ریسک افت کیفیت به‌خاطر خستگی."
        )
        return (
            "🌙 حالت آرامش (Calm Mode)\n\n"
            "بار امروزت زیاده یا وقتشه یه پارت سبک بگیری.\n"
            f"پیشنهاد می‌کنم یک پارت آروم {int(suggested)} دقیقه‌ای باشه -- هدف تمومش کردنه، نه فشار آوردن.\n\n"
            "✨ هدف: حفظ ریتم بدون فشار اضافه.\n"
            f"{rationale}\n"
            "آماده‌ای؟"
        )
