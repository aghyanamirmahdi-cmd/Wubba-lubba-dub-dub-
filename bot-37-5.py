...
from typing import Optional, List, Dict, Any, Union, Tuple, Callable, Awaitable, Set
...
class Config:
    ...
    # Safety ceiling on how many parts a single /plan day can be split
    # into. The real number of parts comes from daily_goal /
    # interval_minutes (see Advisor.generate_personalized_plan) -- i.e.
    # from the numbers the user themself set -- so this is NOT meant to
    # trim ordinary goals any more. It used to default to 8, which forced
    # any big goal to be crammed into 8 (very long) parts. The default is
    # now 120 = the largest count the inputs can even produce (daily goal
    # is capped at 600 min, and the shortest allowed part is 5 min), so in
    # practice it never truncates anything.
    # NOTE: if MAX_DAILY_PLAN_PARTS is still set to 8 in your server's
    # environment variables, that value wins over this default -- remove
    # it or raise it there too.
    MAX_DAILY_PLAN_PARTS: int = int(os.getenv("MAX_DAILY_PLAN_PARTS", "120"))
    # How many part-checkbox buttons ("✅ / ⬜ پارت N") one page of the plan
    # keyboard shows. Up to this many parts -> exactly the old single
    # keyboard; more than this -> the keyboard is paginated with
    # ◀️ قبلی / بعدی ▶️ (see plan_keyboard / cb_plan_page). Telegram's hard
    # limit is 100 buttons per message, so it's clamped to 90 (leaving room
    # for the nav row + the two bottom buttons).
    PLAN_KEYBOARD_PAGE_SIZE: int = max(1, min(90, int(os.getenv("PLAN_KEYBOARD_PAGE_SIZE", "10"))))
    # The plan TEXT is split into several messages once it would get near
    # Telegram's 4096-character limit (a 20-part plan easily does). Kept a
    # bit under 4096 for headroom (emoji count double, prefixes get added).
    PLAN_MESSAGE_CHUNK_LIMIT: int = 3700
    ...
    # Safety valve for recover_from_crash(). A job/session that's overdue by
    # more than this is treated as "belongs to a different point in time"
    # (typically: an old JSON backup was just restored) rather than "the
    # process was briefly offline". In that case we quietly close it out
    # (MISSED/CANCELLED, no group announcement, no re-opened report button)
    # instead of running the normal end-of-part flow, which would otherwise
    # resurrect a long-dead part in the group and invite people to tap
    # "ثبت دستی" on something that isn't really happening anymore.
    RECOVERY_STALE_THRESHOLD_MINUTES: int = int(
        os.getenv("RECOVERY_STALE_THRESHOLD_MINUTES", str(6 * 60))
    )
    ...
    # ------------------------------------------------------------------
    # "برخورد جدی" (tough-love confrontation) -- opt-in accountability
    # messages for users whose behavior profile shows real, sustained
    # laziness, not a single missed part. Every threshold below is a
    # separate *signal*; LazinessAssessor only escalates once enough of
    # them fire together (see CONFRONTATION_MIN_SIGNALS), so nobody gets
    # scolded off one bad day. Deliberately env-overridable rather than
    # hardcoded, the same way MISS_STREAK_PAUSE_THRESHOLD is, so an admin
    # can tune "how lazy is lazy" without a code change.
    LAZINESS_CRITERIA: Dict[str, float] = {
        # N *explicit* consecutive "انجام ندادم" reports in a row.
        "MISS_STREAK_THRESHOLD": float(os.getenv("LAZINESS_MISS_STREAK", "3")),
        # UserBehaviorProfile.completion_rate at/under this = signal.
        "MIN_COMPLETION_RATE": float(os.getenv("LAZINESS_MIN_COMPLETION_RATE", "0.35")),
        # UserBehaviorProfile.consistency_score at/under this = signal.
        "MIN_CONSISTENCY_SCORE": float(os.getenv("LAZINESS_MIN_CONSISTENCY", "0.3")),
        # No study activity at all reported in this many days = signal
        # (this is what catches silent no-shows, not just honest "MISSED"
        # taps).
        "INACTIVITY_DAYS_THRESHOLD": float(os.getenv("LAZINESS_INACTIVITY_DAYS", "2")),
    }
    # How many of the four signals above have to fire together before the
    # bot escalates to "firm" vs "severe" tone. Below CONFRONTATION_MIN_SIGNALS["firm"]
    # signals, nothing is sent at all -- an off day stays an off day.
    CONFRONTATION_MIN_SIGNALS: Dict[str, int] = {"firm": 2, "severe": 3}
    # Minimum hours between two confrontation messages to the same person,
    # so this can never turn into nagging/spam regardless of how many
    # parts they miss in that window.
    CONFRONTATION_COOLDOWN_HOURS: int = int(os.getenv("CONFRONTATION_COOLDOWN_HOURS", "20"))
    # Fixed daily time the inactivity sweep runs (catches people who never
    # tapped anything at all, so the miss-streak-based check in cb_result
    # would never fire for them) -- see _daily_laziness_confrontation_check.
    CONFRONTATION_CHECK_HOUR: int = int(os.getenv("CONFRONTATION_CHECK_HOUR", "21"))
    CONFRONTATION_CHECK_MINUTE: int = int(os.getenv("CONFRONTATION_CHECK_MINUTE", "0"))
    # ---- evidence-based inactivity detection (see gather_inactivity_evidence
    # / LazinessAssessor.evaluate). The profile numbers above are moving
    # averages that drift and can even *hide* inactivity (an explicit
    # "انجام ندادم" tap used to reset "days inactive"), so the detector now
    # looks at hard evidence: real study events, the weekly goal, and the
    # plan parts left unticked.
    # Days after signup during which nobody is confronted at all.
    CONFRONTATION_SIGNUP_GRACE_DAYS: float = float(os.getenv("CONFRONTATION_SIGNUP_GRACE_DAYS", "3"))
    # Window (days) for weekly-goal and plan-adherence evidence.
    CONFRONTATION_LOOKBACK_DAYS: int = int(os.getenv("CONFRONTATION_LOOKBACK_DAYS", "7"))
    # Signal: reached at most this fraction of the goal over the window.
    CONFRONTATION_GOAL_SHORTFALL_RATIO: float = float(os.getenv("CONFRONTATION_GOAL_SHORTFALL_RATIO", "0.25"))
    # Signal: at most this fraction of the plan's parts ticked done, judged
    # only once at least CONFRONTATION_PLAN_MIN_PARTS parts were scheduled.
    CONFRONTATION_PLAN_IGNORED_RATIO: float = float(os.getenv("CONFRONTATION_PLAN_IGNORED_RATIO", "0.25"))
    CONFRONTATION_PLAN_MIN_PARTS: int = int(os.getenv("CONFRONTATION_PLAN_MIN_PARTS", "4"))
    # After this many confrontations in a row with no study in between,
    # scolding is clearly not working: switch to a gentle weekly check-in.
    CONFRONTATION_MAX_UNANSWERED: int = int(os.getenv("CONFRONTATION_MAX_UNANSWERED", "3"))
    CONFRONTATION_CHECKIN_DAYS: int = int(os.getenv("CONFRONTATION_CHECKIN_DAYS", "7"))
    # "⏸ چند روز نپرس" button on a confrontation.
    CONFRONTATION_SNOOZE_DAYS: int = int(os.getenv("CONFRONTATION_SNOOZE_DAYS", "3"))

    # ------------------------------------------------------------------
    # Per-activity neglect warnings -- separate from the "برخورد جدی"
    # block above. Tough-love looks at the *person's* overall behavior
    # profile across every group study part; this looks at one specific
    # subject/task (UserActivity) the user told the bot about via
    # /activities, and whether the parts the daily plan scheduled *for
    # that subject specifically* are actually getting done. A subject
    # rated 5 (حیاتی/فوری) tolerates almost no slack; a subject rated 1
    # (کم‌اهمیت) gets a much longer leash before the bot says anything.
    # See ActivityNeglectAssessor / maybe_send_activity_neglect_warning.
    ACTIVITY_NEGLECT_CHECK_HOUR: int = int(os.getenv("ACTIVITY_NEGLECT_CHECK_HOUR", "21"))
    ACTIVITY_NEGLECT_CHECK_MINUTE: int = int(os.getenv("ACTIVITY_NEGLECT_CHECK_MINUTE", "30"))
    # How far back (in days) each activity's scheduled-parts history is
    # looked at when computing its completion ratio.
    ACTIVITY_NEGLECT_LOOKBACK_DAYS: int = int(os.getenv("ACTIVITY_NEGLECT_LOOKBACK_DAYS", "14"))
    # Minimum hours between two neglect warnings for the *same activity*
    # (UserActivity.last_neglect_warning_at) -- keeps this from firing
    # every single evening once a subject starts slipping.
    ACTIVITY_NEGLECT_COOLDOWN_HOURS: int = int(os.getenv("ACTIVITY_NEGLECT_COOLDOWN_HOURS", "20"))
    # Per-importance-level thresholds. Three independent signals, same
    # "2+ fire together = severe, 1 = firm" escalation shape as
    # LAZINESS_CRITERIA / CONFRONTATION_MIN_SIGNALS above:
    #   max_miss_streak       -- this many *most-recent* scheduled parts
    #                            for the activity in a row left unchecked
    #   max_days_no_progress  -- this many days since the activity last
    #                            had a part actually ticked done (or,
    #                            if never, since the activity was added)
    #   min_completion_ratio  -- done/scheduled ratio over the lookback
    #                            window falling at/under this (only
    #                            evaluated once at least 2 parts have
    #                            been scheduled, so one lonely part
    #                            can't trip it)
    ACTIVITY_NEGLECT_RULES: Dict[int, Dict[str, float]] = {
        1: {"max_miss_streak": 5, "max_days_no_progress": 7, "min_completion_ratio": 0.20},
        2: {"max_miss_streak": 4, "max_days_no_progress": 5, "min_completion_ratio": 0.30},
        3: {"max_miss_streak": 3, "max_days_no_progress": 3, "min_completion_ratio": 0.40},
        4: {"max_miss_streak": 2, "max_days_no_progress": 2, "min_completion_ratio": 0.55},
        5: {"max_miss_streak": 1, "max_days_no_progress": 1, "min_completion_ratio": 0.70},
    }
    # Same two-signals-for-severe shape as CONFRONTATION_MIN_SIGNALS.
    ACTIVITY_NEGLECT_MIN_SIGNALS: Dict[str, int] = {"firm": 1, "severe": 2}
...
class UserPreference(Base):
    ...
    # Opt-in "برخورد جدی" (tough-love confrontation) -- off by default.
    # This is a deliberate style choice, not everyone wants a bot that
    # scolds them, so it only ever fires for users who explicitly turned
    # it on from settings (see NOTIFICATION_PREF_FIELDS / TOUGH_LOVE
    # category / LazinessAssessor).
    tough_love_enabled = Column(Boolean, default=False, nullable=False)
    # Opt-in "هشدار غفلت از درس" -- per-activity warnings when the parts
    # scheduled for a specific subject/task (see UserActivity) go undone,
    # or get done too rarely, weighted by that activity's own importance
    # and how many days it's been neglected. On by default (unlike
    # tough_love_enabled) since it's scoped to a concrete subject rather
    # than a blanket character judgment -- see ActivityNeglectAssessor /
    # maybe_send_activity_neglect_warning.
    activity_neglect_warnings_enabled = Column(Boolean, default=True, nullable=False)
...
class UserPlanPart(Base):
    ...
    # ------------------------------------------------------------------
    # "📤 ارسال پارت شخصی" -- automatic delivery of this plan part.
    # All four are nullable, so plans/backups created before this feature
    # existed load unchanged (json_orm fills missing columns with None).
    #   send_at     exact moment this part is due (the plan's own
    #               HH:MM label resolved to a real date+time, Tehran tz).
    #   send_status None      never queued (or plan not "armed" yet)
    #               PENDING   queued in APScheduler, will fire at send_at
    #               SENT      fired: a personal Session was created and
    #                         the part message was delivered
    #               SKIPPED   its time had already passed when the plan
    #                         was armed / the bot was down / the person
    #                         was busy in another part when it fired
    #               CANCELLED person stopped the delivery, or asked for a
    #                         new plan while it was still pending
    #   session_id  the personal Session this part turned into once SENT
    #               (plain Integer, not a ForeignKey: this table is
    #               declared before Session, and json_orm's restore order
    #               follows declaration order).
    #   message_id  Telegram message_id of the delivered part message, so
    #               it can be closed out if the part gets cancelled.
    # ------------------------------------------------------------------
    send_at = Column(DateTime, nullable=True)
    send_status = Column(String(12), nullable=True)
...
class UserActivity(Base):
    ...
    # Self-reported, optional estimate of how many hours of study this
    # activity would take to reach mastery -- asked once right after
    # deadline during /activities creation (see add_activity_deadline /
    # add_activity_mastery_hours). Purely informational for now (shown on
    # the activity detail view); if the user doesn't answer with a valid
    # number, this stays None -- the bot deliberately never guesses a
    # number on their behalf.
    estimated_mastery_hours = Column(Float, nullable=True)
    active = Column(Boolean, default=True, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    # Cooldown bookkeeping for ActivityNeglectAssessor -- the last time a
    # neglect warning actually went out *for this specific activity*, so
    # the daily sweep (_daily_activity_neglect_check) can never nag about
    # the same lesson more than once per
    # config.ACTIVITY_NEGLECT_COOLDOWN_HOURS, no matter how bad the streak
    # gets in the meantime.
    last_neglect_warning_at = Column(DateTime, nullable=True)
    __table_args__ = (
...
class UserBehaviorProfile(Base):
    ...
    # Cooldown bookkeeping for the "برخورد جدی" tough-love feature (see
    # LazinessAssessor / CONFRONTATION_COOLDOWN_HOURS) -- separate from
    # last_updated, which changes on every ordinary report and would
    # otherwise reset the cooldown on ANY activity, including a COMPLETED
    # one right after a confrontation was just sent.
    last_confrontation_at = Column(DateTime, nullable=True)
    # Confrontations sent in a row with no real study in between (reset to
    # 0 once they study, or answer with the snooze button). Drives both the
    # escalation firm -> severe and the back-off to a gentle check-in --
    # see LazinessAssessor.evaluate / maybe_send_confrontation.
    confrontation_streak = Column(Integer, default=0, nullable=False)
    last_confrontation_tier = Column(String(10), nullable=True)
    # "⏸ چند روز نپرس": nobody is confronted before this (naive UTC).
    confrontation_snooze_until = Column(DateTime, nullable=True)


class Session(Base):
    __tablename__ = "sessions"
    id = Column(Integer, primary_key=True, index=True)
    # Nullable now: a "personal" part (started with /session from a
    # private chat) has no group at all -- see owner_user_id below and
    # _start_personal_session. Every existing group-part code path that
    # reads group_id already either requires it explicitly (group
    # commands) or degrades gracefully on None (see _session_end,
    # _refresh_roster_message, _schedule_next_poll), so this widening is
    # safe for the group flow.
    group_id = Column(Integer, ForeignKey("groups.id"), nullable=True)
    # Set only for a personal part (group_id is None): the internal
    # User.id of the one person it belongs to. Used purely for the
    # personal <-> group mutual-exclusion lock (see
    # _has_active_group_lock / _has_active_personal_lock) -- the actual
    # participation is still tracked the normal way via
    # SessionParticipant, so report_result/stats need no special-casing.
    ...
    __table_args__ = (
        ...
        Index("idx_sessions_owner_user_id", "owner_user_id"),
...
class GroupRepository:
    ...
    async def get_or_create_group(self, telegram_chat_id: int, title: Optional[str] = None) -> Group:
        ...
        if not group:
            ...
            changed = False
            # BUGFIX: a group that was ever marked inactive (bot removed
            # from the group, see on_bot_membership_changed's "left"/
            # "kicked" branch) stayed inactive forever after being
            # re-added -- this only ever updated the title, never
            # reactivated it. GroupSyncMiddleware calls get_or_create_group
            # on every message and add_member() DOES reactivate a stale
            # GroupMember row, so commands scoped to the group itself
            # (/autopoll, /cancel, /report -- all read via get_by_chat_id,
            # which never filters on active) kept working, silently
            # masking the problem. But get_user_groups() -- used by
            # /session and /plan -- filters on Group.active == True, so
            # those saw an empty list and told a real, active member
            # "شما عضو هیچ گروه مطالعاتی نیستید" even though every other
            # group command worked fine for them.
            if not group.active:
                group.active = True
                changed = True
...
class ActivityRepository:
    ...
    async def create(self, user_id: int, name: str, importance: int,
                      ...
                      estimated_mastery_hours: Optional[float] = None) -> "UserActivity":
        activity = UserActivity(
            user_id=user_id, name=name.strip()[:100], importance=importance, deadline=deadline,
            estimated_mastery_hours=estimated_mastery_hours,
        )
        self.session.add(activity)
        await self.session.flush()
        return activity

    async def set_mastery_hours(
        self, activity_id: int, user_id: int, estimated_mastery_hours: Optional[float]
    ) -> Optional["UserActivity"]:
        activity = await self.get_by_id(activity_id, user_id)
        if activity:
            activity.estimated_mastery_hours = estimated_mastery_hours
            await self.session.flush()
        return activity
    ...
    async def get_all_active(self) -> List["UserActivity"]:
        """Every active activity across every user -- used once a day by
        the neglect sweep (_daily_activity_neglect_check), the same way
        RoutineRepository.get_all_active feeds the routine rescheduler."""
        result = await self.session.execute(
            select(UserActivity).where(UserActivity.active == True)
        )
        return result.scalars().all()

    async def deactivate_all_for_user(self, user_id: int) -> int:
        """🗑 ریست همه فعالیت‌ها: soft-deletes every active activity this
        user has, same as tapping 🗑 حذف on each one individually. Returns
        how many were cleared."""
        activities = await self.get_active_by_user(user_id)
...
class RoutineRepository:
    ...
    async def deactivate_all_for_user(self, user_id: int) -> List[int]:
        """🗑 ریست همه روتین‌ها: soft-deletes every active routine this user
        has. Returns their ids so the caller can also unschedule each
        APScheduler job (mirrors cb_routine_delete's unschedule_routine call
        for a single routine -- see cb_routine_resetall_confirm)."""
        routines = await self.get_active_by_user(user_id)
...
class PlanRepository:
    ...
    async def replace_today_plan(
        ...
        for old_plan in result.scalars().all():
            # Safety net for the "📤 ارسال پارت شخصی" feature: a part that's
            # still queued in APScheduler must not outlive its own row
            # (it would fire against a part that no longer exists). The
            # normal 🆕 برنامه جدید flow cancels these explicitly *before*
            # getting here (see _cancel_personal_run) -- this just makes
            # sure no other caller can ever leave a dangling job behind.
            old_parts = await self.session.execute(
                select(UserPlanPart).where(UserPlanPart.plan_id == old_plan.id)
            )
            for old_part in old_parts.scalars().all():
                if old_part.send_status == "PENDING":
                    _remove_plan_part_job(old_part.id)
    ...
    async def get_today_plan_with_parts(self, user_id: int) -> Tuple[Optional["UserPlan"], List["UserPlanPart"]]:
        ...
        # json_orm's scalars() result only has .all() (no .first()), so
        # take the newest row (the query is ordered newest-first) by hand.
        # The old `.scalars().first()` raised AttributeError here, which
        # broke the ✅/⬜ part toggle right after it had saved the change.
    ...
    async def get_unfinished_carryover(self, user_id: int, days_back: int = 14) -> Dict[str, int]:
        """Minutes from *past* (before today) plan parts that never got
        ticked ✅, grouped by activity name. A part left unchecked isn't
        assumed done and doesn't just vanish once the day rolls over --
        Advisor.generate_personalized_plan folds this back into today's
        weighting so the same activity gets pulled forward until it
        actually gets ticked."""
        today_start = time_service.start_of_day()
        earliest = today_start - timedelta(days=days_back)
        result = await self.session.execute(
            select(UserPlanPart.activity_name, func.sum(UserPlanPart.duration_minutes))
            .join(UserPlan, UserPlanPart.plan_id == UserPlan.id)
            .where(
                UserPlanPart.user_id == user_id,
                UserPlan.plan_day < today_start,
                UserPlan.plan_day >= earliest,
                UserPlanPart.is_done == False,
            )
            .group_by(UserPlanPart.activity_name)
        )
        carryover: Dict[str, int] = {}
        for name, minutes in result.all():
            if name:
                carryover[name] = int(minutes or 0)
        return carryover

    async def get_activity_part_history(
        self, user_id: int, activity_name: str, days_back: int
    ) -> List[Tuple[datetime, bool]]:
        """(plan_day, is_done) for every part scheduled under this exact
        activity_name over the last `days_back` days (today inclusive),
        newest first. Feeds ActivityNeglectAssessor -- unlike
        get_unfinished_carryover (which only cares about *minutes still
        owed*), this keeps every part so the assessor can see the miss
        streak and completion ratio, not just the leftover total."""
        today_start = time_service.start_of_day()
        earliest = today_start - timedelta(days=days_back)
        result = await self.session.execute(
            select(UserPlan.plan_day, UserPlanPart.is_done)
            .join(UserPlan, UserPlanPart.plan_id == UserPlan.id)
            .where(
                UserPlanPart.user_id == user_id,
                UserPlanPart.activity_name == activity_name,
                UserPlan.plan_day >= earliest,
                UserPlan.plan_day <= today_start,
            )
            .order_by(desc(UserPlan.plan_day), desc(UserPlanPart.part_index))
        )
        return result.all()

    @staticmethod
    def compute_part_times(plan: "UserPlan", parts: List["UserPlanPart"]) -> List[datetime]:
        """Resolves each part's plain "HH:MM" label into the real,
        timezone-aware moment it's due -- returned in the same order as
        `parts`. This is what lets "📤 ارسال پارت شخصی" send every part
        at *exactly* the time written in the person's plan.

        A label alone is ambiguous (UserPlanPart doesn't store a day), so:
          * the plan's own calendar day (plan_day) is the starting point;
          * parts are strictly increasing in real time, so whenever a
            label's clock is *earlier* than the previous part's, the
            plan has run past midnight -- that part (and every one after
            it) belongs to the next day. This mirrors how
            Advisor.generate_personalized_plan wraps its clock, which is
            also where the 🌙 tag in the plan text comes from;
          * a plan built with '🆕 برنامه جدید' always starts in the
            *future* (now + offset). If that lands past midnight the
            generator stores a bare "00:05" with no day marker, so for
            manual plans a first part that would otherwise resolve to
            *before the plan was even created* is moved to the next day.
        """
        base_date = plan.plan_day.date()
        created_local = time_service.to_tehran(plan.created_at) if plan.created_at else None
        day_offset = 0
        prev_clock: Optional[int] = None
        by_id: Dict[int, datetime] = {}
        for idx, part in enumerate(sorted(parts, key=lambda p: p.part_index)):
            hh, mm = (int(x) for x in part.scheduled_label.split(":"))
            clock = hh * 60 + mm
            ...
            naive = (
                ...
                + timedelta(days=day_offset, hours=hh, minutes=mm)
            )
            when = time_service.tz.localize(naive)
            if (idx == 0 and plan.source == "manual" and created_local is not None
                    and when < created_local - timedelta(minutes=5)):
                day_offset += 1
                when = time_service.tz.localize(naive + timedelta(days=1))
            by_id[part.id] = when
        return [by_id[p.id] for p in parts]

    async def mark_part_done_for_session(self, session_id: int, user_id: int) -> None:
        """When a plan-driven personal part is reported "🟢 انجام دادم",
        tick its ✅ checkbox in the plan too -- otherwise a part the person
        genuinely did would still count as *unfinished* (carried over into
        tomorrow's plan by get_unfinished_carryover, and counted against
        them by ActivityNeglectAssessor). No-op for any session that isn't
        linked to a plan part (group parts, /session personal parts)."""
        result = await self.session.execute(
            select(UserPlanPart).where(
                UserPlanPart.session_id == session_id,
                UserPlanPart.user_id == user_id,
            )
        )
        part = result.scalar_one_or_none()
        if part is not None and not part.is_done:
            part.is_done = True
            part.done_at = datetime.utcnow()
            await self.session.flush()
...
class SessionRepository:
    ...
    async def get_group_part_number(self, group_id: int, session_id: int) -> int:
        ...
        answered) and CANCELLED ones (an admin called it off via /cancel)
        -- neither ever actually became a part anyone finished attending,
        so counting them inflated the number a real part got labeled
        with (e.g. a group's 4th real part showing as "پارت #6" because
        two earlier polls went unanswered or got cancelled)."""
        result = await self.session.execute(
            select(func.count()).select_from(Session).where(
                Session.group_id == group_id,
                Session.id <= session_id,
                Session.status.notin_(["MISSED", "CANCELLED"]),
            )
        )
        count = result.scalar()
        return count or 1
...
class MessageRepository:
    ...
    async def delete_many(self, message_ids: List[int]) -> int:
        """Deletes a specific set of messages (the ☑️ selection from the
        bulk-delete screen). Missing ids are silently skipped so a stale
        selection can't blow up the request."""
        deleted = 0
    ...
    async def delete_by_category(self, category: str) -> int:
        """Wipes every message (active or not) in one category -- the
        per-category ♻️ ریست این دسته button."""
        result = await self.session.execute(
            select(MessageTemplate).where(MessageTemplate.category == category)
        )
        msgs = result.scalars().all()
        for msg in msgs:
            await self.session.delete(msg)
    ...
    async def delete_all(self) -> int:
        """Wipes every message across every category -- the 🗑 ریست همه
        پیام‌ها button on the categories overview screen."""
    ...
    async def count_all(self) -> int:
        result = await self.session.execute(select(func.count(MessageTemplate.id)))
...
class SessionManager:
    ...
    async def register_explicit_late_join(self, session_id: int, user_id: int) -> Optional[SessionParticipant]:
        """Explicit "⏳ با تاخیر پیوستن" tap: someone who missed the "هستم"
        attendance window but wants to officially join a part that's
        already running. Unlike `register_late_participant` (which only
        ever gets called *after the fact*, backfilling a participant row
        at the exact moment someone reports a result with none on file),
        this captures the real moment they joined on `joined_at` -- so
        `report_result` can later credit them for the time between this
        tap and their "🟢 انجام دادم", instead of the time since the whole
        part started. Only valid while the part is actually STARTED:
        before that the normal "🟢 هستم" button is the right one (and
        still gives full credit from the real start), and once the part
        has ENDED there's no live part left to join into -- a report at
        that point still falls back to the old backfill behavior."""
        existing = await self.session.execute(
            select(SessionParticipant).where(
                SessionParticipant.session_id == session_id,
                SessionParticipant.user_id == user_id,
        ...
        try:
            ...
            # Same idempotency guard as register_participant: a race
            # between two late-join taps must not crash or double-insert.
            await self.session.rollback()
            return None
        return participant

    async def register_late_participant(self, session_id: int, user_id: int) -> Optional[SessionParticipant]:
        """Silently enrolls someone in a session's participant table who
        never tapped 'هستم' (so never went through register_participant)
        but is now reporting a result anyway. Marked attendance_status
        "LATE" -- distinct from "READY" -- so the roster can show them as
        a "تازه‌پیوستگان" (newly-joined) entry instead of pretending they
        were part of the original headcount, and so they're excluded from
        ready_ids in the early-finish check (that check already filters on
        "READY"; a bonus reporter must not itself close the part out)."""
        existing = await self.session.execute(
            select(SessionParticipant).where(
                SessionParticipant.session_id == session_id,
                SessionParticipant.user_id == user_id,
        ...
        try:
            ...
            # Same idempotency guard as register_participant: a race
            # between two report attempts must not crash or double-insert.
            await self.session.rollback()
            return None
        return participant
    ...
    async def report_result(self, session_id: int, user_id: int,
        ...
        if sess.status == "CANCELLED":
            # The part was called off (/cancel) -- a stale button tap that
            # lands after that (race with the edit, or a client that
            # hadn't refreshed) must not silently create a result (and a
            # late-joined participant row) for a part that officially
            # never happened.
            return False
        ...
        # ...unless this person explicitly tapped "⏳ با تاخیر پیوستن"
        # earlier in this same part (see register_explicit_late_join): in
        # that case a SessionParticipant row already exists with
        # attendance_status "LATE" and a real joined_at timestamp from
        # that tap, and it's that moment -- not the whole part's start --
        # that should be measured against, since that's genuinely when
        # this person started studying. A participant row created purely
        # as the after-the-fact backfill below (register_late_participant,
        # called once this very report lands) doesn't exist yet at this
        # point, so it can never be mistaken for an explicit late join.
        ...
        participant_row = existing_participant.scalar_one_or_none()
        if participant_row and participant_row.attendance_status == "LATE" and participant_row.joined_at:
            start_reference = participant_row.joined_at
        ...
        # Someone can report a result without ever having tapped "هستم"
        # (per design -- see the result buttons' caption). Without this,
        # they'd have a SessionResult row but no SessionParticipant row at
        # all, so the pinned roster would never show their name or dot,
        # and they'd silently vanish from "N نفر" counts and the
        # completed/partial/missed/avg-minutes breakdown even though they
        # actually studied. register_late_participant is a no-op if a
        # participant row already exists (e.g. they tapped "هستم" then
        # reported normally), so this is safe to call unconditionally.
        await self.register_late_participant(session_id, user_id)
        ...
        # A personal part that came from the plan ("📤 ارسال پارت شخصی")
        # reported as done also ticks its ✅ in the plan -- see
        # PlanRepository.mark_part_done_for_session.
        if completion_status == "COMPLETED":
            await PlanRepository(self.session).mark_part_done_for_session(session_id, user_id)
    ...
    async def cancel_session(self, session_id: int) -> bool:
        """Cancels a part that's still in flight -- attendance still open,
        or already started -- regardless of how many people have tapped
        "هستم" or reported a result already (all of that stays on record,
        it just no longer counts toward closing this part out normally).
        A no-op (returns False) once the part has already ended/missed/
        been cancelled, so a stray double-tap of /cancel can't do
        anything unexpected."""
        sess = await self.get_session(session_id)
        if not sess or sess.status not in ("SCHEDULED", "ATTENDANCE", "READY", "STARTED"):
            return False
        sess.actual_end = time_service.now() if sess.status == "STARTED" else sess.actual_end
...
# ------------------------------------------------------------
# Personal <-> group mutual-exclusion lock
# ------------------------------------------------------------
# Rule (per product decision): a person can freely choose to study solo
# (via /session in a private chat, see _start_personal_session) instead
# of waiting to be counted in a group's part, *unless* they're already
# committed to one side -- then the other is blocked until that specific
# part finishes (COMPLETED/MISSED/CANCELLED/timed-out -- anything that
# takes it out of the "still going" status set below). This intentionally
# checks only "هستم"/READY commitment, not a bare LATE report (someone
# who only reported a result after the fact was never actively "in" the
# part the way a lock is meant to protect against double-booking).
_ACTIVE_SESSION_STATUSES = ("ATTENDANCE", "READY", "STARTED")


async def _has_active_group_lock(session: AsyncSession, user_id: int) -> bool:
    """True if this user has tapped 'هستم' (or otherwise become a READY
    participant) in a *group* part that hasn't finished yet."""
    result = await session.execute(
        select(Session)
        .join(SessionParticipant, Session.id == SessionParticipant.session_id)
        .where(
            ...
            Session.group_id.isnot(None),
            Session.status.in_(list(_ACTIVE_SESSION_STATUSES)),
        )
    )
    return len(result.scalars().all()) > 0


async def _has_active_personal_lock(session: AsyncSession, user_id: int) -> bool:
    """True if this user currently has a personal (non-group) part
    running -- see _start_personal_session. Personal parts skip the
    attendance phase entirely (there's nobody else to poll), so the only
    "still going" status that matters here is STARTED."""
    result = await session.execute(
        select(Session).where(
            Session.owner_user_id == user_id,
            Session.group_id.is_(None),
...
def _format_countdown(remaining_seconds: float) -> str:
    """Bold mm:ss countdown text, e.g. '<b>04:37</b>'. Never shows a
    negative value -- once the target instant has passed (the tick job
    firing a beat before the real transition, or just clock skew) it
    clamps to 00:00 instead of a confusing negative number."""
    remaining_seconds = max(0, int(remaining_seconds))
    minutes, seconds = divmod(remaining_seconds, 60)
    return f"<b>{minutes:02d}:{seconds:02d}</b>"


def _countdown_job_id(kind: str, session_id: int) -> str:
    return f"countdown_{kind}_{session_id}"


def _stop_countdown_job(kind: str, session_id: int) -> None:
    """Removes a live 5-second countdown-tick job, if one is scheduled.
    Safe to call even when no such job exists (already fired its last
    tick, was never started, or this session never reaches that phase)."""
    try:
        scheduler.remove_job(_countdown_job_id(kind, session_id))
    except JobLookupError:
        pass


def _start_countdown_job(kind: str, session_id: int) -> None:
    """(Re)starts a 5-second countdown-tick job for this session. Not
    persisted to the `scheduler_jobs` table on purpose -- unlike
    attendance_timeout/session_start/session_end, losing a few ticks (or
    all of them) across a restart isn't a correctness problem, just a
    frozen countdown until the next tick resumes it; the underlying
    session lifecycle jobs are what actually start/end the part."""
    func = _tick_attendance_countdown if kind == "attend" else _tick_start_countdown
    scheduler.add_job(
        func,
        IntervalTrigger(seconds=5),
        id=_countdown_job_id(kind, session_id),
        args=[session_id],
        replace_existing=True,
    )


async def _tick_attendance_countdown(session_id: int) -> None:
    """One 5-second beat of the attendance-poll countdown. Stops itself
    the moment the session has left ATTENDANCE (closed, cancelled, or the
    poll message is gone) instead of relying solely on whoever ends that
    phase to remember to call _stop_countdown_job."""
    async with get_session() as db_session:
        manager = SessionManager(db_session)
        sess = await manager.get_session(session_id)
        if not sess or sess.status != "ATTENDANCE" or not sess.attendance_message_id:
            _stop_countdown_job("attend", session_id)
            return
    await _refresh_attendance_message(session_id)


async def _tick_start_countdown(session_id: int) -> None:
    """One 5-second beat of the running-part countdown. Stops itself once
    the session is no longer STARTED (ended, finished early, or
    cancelled)."""
    async with get_session() as db_session:
        manager = SessionManager(db_session)
        sess = await manager.get_session(session_id)
        if not sess or sess.status != "STARTED" or not sess.start_message_id:
            _stop_countdown_job("start", session_id)
            return
    await _refresh_roster_message(session_id)


async def _refresh_attendance_message(session_id: int) -> None:
    """Re-renders the attendance-poll ('کیا هستن؟' / 'آماده پارت هستید؟')
    message in place with an up-to-date, bold '⏳ تا شروع پارت' countdown.
    Same best-effort error handling as `_refresh_roster_message`: a
    missing bot/group/message, or a Telegram "not modified" no-op, is
    routine and must never bubble up -- this is purely a cosmetic tick."""
    if not bot:
        return
    try:
        async with get_session() as db_session:
            ...
            if not group:
                return
            remaining = (sess.scheduled_start - time_service.now()).total_seconds()
            countdown_line = f"⏳ <b>تا شروع پارت:</b> {_format_countdown(remaining)}"
            if sess.creation_source == "auto_poll":
                text = (
                ...
                text = (
                    ...
                    f"🕐 زمان: {time_service.format_datetime(sess.scheduled_start)}\n"
                    f"🎯 هدف: {sess.target_duration} دقیقه\n"
                    f"🌱 حداقل: {sess.minimum_duration} دقیقه\n"
                    f"🔥 حداکثر: {sess.extension_duration} دقیقه\n\n"
                    f"چه کسانی هستن؟ 🟢\n\n"
                    f"{countdown_line}"
                )
            await bot.edit_message_text(
                chat_id=group.telegram_chat_id,
                message_id=sess.attendance_message_id,
                text=text,
                reply_markup=attendance_keyboard(sess.id),
            )
    except TelegramBadRequest as e:
        if "not modified" not in str(e).lower():
            logger.debug(f"Could not refresh attendance countdown for session {session_id}: {e}")
    except Exception as e:
        logger.debug(f"Could not refresh attendance countdown for session {session_id}: {e}")
...
def _format_roster(
    users: List["User"],
    results: Optional[Dict[int, Tuple[str, Optional[int]]]] = None,
    late_ids: Optional[Set[int]] = None,
) -> str:
    """Renders a roster for a "کیا هستن؟"/part-announcement style message.
    Each name is a real Telegram mention (tg://user?id=...) rather than
    plain text, so appearing in the roster also pings that person -- the
    same reminder effect as being @-tagged. `results`, if given, maps
    ...
    having to run /report. `late_ids`, if given, is the set of user ids who
    joined this part after it was already running -- either by explicitly
    tapping "⏳ با تاخیر پیوستن" (see register_explicit_late_join) or, for
    an older/bonus report, by never tapping "هستم" at all and only showing
    up via a direct report (see register_late_participant). Rather than
    interleaving them with everyone who was ready on time, they're rendered
    as their own numbered "⏳ با تاخیر پیوستگان" block underneath the on-time
    roster, so the two groups stay visually distinct. Returns a friendly
    placeholder line if there's nobody at all yet."""
    late_ids = late_ids or set()
    on_time_users = [u for u in users if u.id not in late_ids]
    late_users = [u for u in users if u.id in late_ids]
    if not on_time_users and not late_users:
        return "— هنوز کسی اعلام آمادگی نکرده —"

    def _render_block(block_users: List["User"]) -> str:
        lines = []
        for i, u in enumerate(block_users, start=1):
    ...
    blocks = []
    blocks.append(_render_block(on_time_users) if on_time_users else "— کسی به‌موقع نپیوسته —")
    if late_users:
        blocks.append(f"⏳ <b>با تاخیر پیوستگان:</b>\n{_render_block(late_users)}")
    return "\n\n".join(blocks)
...
async def _get_next_readiness_poll_run_at(
    db_session: "AsyncSession", group_id: int
) -> Optional[datetime]:
    """Best-effort lookup of the next scheduled "readiness_poll" job for
    this group. Needed when a late report (see `report_result` /
    `register_late_participant`) arrives *after* a part has already been
    finalized: at that point `_refresh_roster_message` has to rebuild the
    same closed-out "پارت بعدی ساعت ..." line that `_finalize_roster_message`
    showed, but that value was only ever a local variable there, never
    stored on the session row. Reading it back from the durable
    `scheduler_jobs` table (see SchedulerService.schedule_job) is the only
    place it still exists. Returns None if nothing pending is found --
    callers already fall back to a generic "به‌زودی" line in that case."""
    result = await db_session.execute(
        select(SchedulerJob).where(
            SchedulerJob.job_type == "readiness_poll",
            SchedulerJob.status == "pending",
        )
    )
    candidates = [
        row for row in result.scalars().all()
        if (row.job_meta or {}).get("group_id") == group_id
    ]
    if not candidates:
        return None
    return min(candidates, key=lambda r: r.run_at).run_at
...
async def _refresh_roster_message(session_id: int) -> None:
    ...
    Called after every successful report_result -- including a *late* one
    that arrives after the part has already been finalized (see
    `_finalize_roster_message`): tapping a result button never stops
    working just because the part's timer ran out, so this has to keep
    rendering whichever design actually matches the session's current
    status instead of always re-drawing the live "🚀 شروع شد" card over
    the closed-out one. STARTED sessions get the live design; anything
    else that still has a roster message (in practice just ENDED) gets
    the closed-out design from `_build_completed_roster_text`, buttons
    kept so later late reports keep landing. Best-effort: a missing
    ...
    try:
        async with get_session() as db_session:
            ...
            if not group:
                return
            ready_users, late_ids = await _get_ready_and_late_users(db_session, session_id)
            ...
            if sess.status == "STARTED":
                ...
                roster = _format_roster(ready_users, results_map, late_ids)
                late_note = f" + ⏳ {len(late_ids)} با تاخیر" if late_ids else ""
                topic_line = f"📖 موضوع: <b>{html.escape(sess.topic)}</b>\n" if sess.topic else ""
                remaining = (sess.scheduled_end - time_service.now()).total_seconds()
                countdown_line = f"⏳ <b>تا پایان پارت:</b> {_format_countdown(remaining)}\n"
                text = (
                    f"🚀 <b>پارت #{part_number} شروع شد</b>\n"
                    f"{topic_line}"
                    f"➖➖➖➖➖➖➖➖➖➖\n"
                    f"👥 <b>آمادگان ({ready_count} نفر{late_note}):</b>\n"
                    f"{roster}\n"
                    f"➖➖➖➖➖➖➖➖➖➖\n"
                ...
                next_run_at = await _get_next_readiness_poll_run_at(db_session, sess.group_id)
                text = _build_completed_roster_text(
                    part_number, sess.topic, ready_users, results_map, actual_minutes, next_run_at, late_ids,
                )
            else:
                # CANCELLED (or anything else) already has its own final
                # card -- report_result itself rejects a late tap on a
                # cancelled part, so this path shouldn't normally be
                # reached, but skip rather than overwrite it just in case.
                return
...
async def _get_ready_and_late_users(
    db_session: "AsyncSession", session_id: int
) -> Tuple[List["User"], Set[int]]:
    """Like `_get_ready_users`, but also appends anyone who joined this
    session late -- i.e. never tapped "هستم" and only exists because they
    reported a result directly (see SessionManager.register_late_participant).
    Late joiners are listed after the real READY roster, ordered by when
    they joined. Returns (all_users, late_user_ids) so callers can pass
    late_user_ids straight into `_format_roster` / `_build_completed_roster_text`
    for the 🆕 tag, and can still tell real headcount from late arrivals
    when totalling things up."""
    ready_users = await _get_ready_users(db_session, session_id)
    result = await db_session.execute(
        select(SessionParticipant).where(
            SessionParticipant.session_id == session_id,
            SessionParticipant.attendance_status == "LATE",
        )
    )
    late_participants = list(result.scalars().all())
    late_participants.sort(key=lambda p: p.joined_at or datetime.min)
    user_repo = UserRepository(db_session)
...
def _build_completed_roster_text(
    part_number: Optional[int],
    topic: Optional[str],
    ready_users: List["User"],
    results_map: Dict[int, Tuple[str, Optional[int]]],
    actual_minutes: Optional[int],
    next_run_at: Optional[datetime],
    late_ids: Optional[Set[int]] = None,
) -> str:
    """The 'closed-out' version of the live roster card. While a part is
    running, `_refresh_roster_message` keeps patching the same message
    with a live 🟢/🟡/🔴/⚪ roster under a "پارت شروع شد" header. Once the
    part is actually over, re-using that exact in-progress look for the
    final state made a finished part visually indistinguishable from one
    still running -- someone scrolling back couldn't tell "done" from
    "in progress" without checking the clock. This builds a deliberately
    different card for that final edit: a boxed ✅ header instead of the
    🚀 one, the same per-person dots (nobody's status data changes, only
    the frame around it), plus a real completed/partial/missed/no-report
    breakdown and an average-minutes line the live view never showed.
    `late_ids`, if given (see `_get_ready_and_late_users`), marks anyone in
    `ready_users` who wasn't actually part of the original READY headcount
    -- they still count toward the completed/partial/missed/avg-minutes
    breakdown (their reported numbers are real), but the headline "N نفر"
    is split into the real headcount plus a separate late-joiner count so
    the two aren't conflated.
    """
    late_ids = late_ids or set()
    total = len(ready_users)
    ready_total = total - len(late_ids)
    completed = partial = missed = 0
    reported_minutes: List[int] = []
    for u in ready_users:
        status, minutes = results_map.get(u.id, ("PENDING", None))
        if status == "COMPLETED":
            completed += 1
        elif status == "PARTIAL":
            partial += 1
        ...
        if minutes is not None:
            reported_minutes.append(minutes)
    no_report = total - completed - partial - missed
    avg_minutes = round(sum(reported_minutes) / len(reported_minutes)) if reported_minutes else None

    header = (
        f"┏━━ ✅ پارت #{part_number} — تکمیل شد ━━┓"
        if part_number else "┏━━ ✅ پارت — تکمیل شد ━━┓"
    )
    lines = [header]
    if topic:
        lines.append(f"┃ 📖 موضوع: <b>{html.escape(topic)}</b>")
    lines.append("┗━━━━━━━━━━━━━━━━━━━━━━━━━┛")
    late_note = f" + ⏳ {len(late_ids)} با تاخیر" if late_ids else ""
    lines.append(f"👥 <b>نتیجهٔ نهایی ({ready_total} نفر{late_note}):</b>")
    lines.append(_format_roster(ready_users, results_map, late_ids))
    lines.append("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬")
    lines.append(f"🟢 انجام دادن: <b>{completed}</b>   🟡 نصفه‌کاره: <b>{partial}</b>")
    lines.append(f"🔴 انجام ندادن: <b>{missed}</b>   ⚪ بی‌خبر: <b>{no_report}</b>")
    if avg_minutes is not None:
        lines.append(f"📊 میانگین مطالعهٔ گزارش‌شده: <b>{avg_minutes}</b> دقیقه")
    if actual_minutes is not None:
        lines.append(f"⏱ مدت کل پارت: <b>{actual_minutes}</b> دقیقه")
    lines.append("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬")
    lines.append(
        f"🧠 {config.MIN_BREAK_MINUTES} دقیقه استراحت کن -- استراحت کوتاه بین پارت‌ها به یادسپاری کمک می‌کنه."
    )
    if next_run_at:
        next_clock = time_service.format_datetime(next_run_at, "%H:%M")
        lines.append(f"⏰ پارت بعدی حدود ساعت <b>{next_clock}</b> پرسیده می‌شه.")
        ...
        lines.append("⏰ پارت بعدی به‌زودی برنامه‌ریزی می‌شه.")
    if no_report > 0:
        lines.append("👇 دیر رسیدی؟ اشکالی نداره، دکمه‌های زیر همچنان فعاله.")
    return "\n".join(lines)


def _build_cancelled_roster_text(
    part_number: Optional[int],
    topic: Optional[str],
    ready_users: List["User"],
    results_map: Dict[int, Tuple[str, Optional[int]]],
    late_ids: Optional[Set[int]],
    cancelled_by: Optional["User"],
) -> str:
    """The 'cancelled' counterpart to `_build_completed_roster_text`. Reuses
    the same per-person 🟢/🟡/🔴/⚪ dots (whatever anyone had already
    reported before the cancel stays visible -- it isn't erased, it just
    no longer determines how the part ended), but with an unmistakably
    different ❌ header so nobody scrolling back mistakes a cancelled part
    for a completed or still-running one. No average-minutes / next-part
    line -- a cancelled part isn't a data point and doesn't imply another
    one is already scheduled."""
    late_ids = late_ids or set()
    total = len(ready_users)
    ready_total = total - len(late_ids)
    header = (
        f"┏━━ ❌ پارت #{part_number} — لغو شد ━━┓"
        if part_number else "┏━━ ❌ پارت — لغو شد ━━┓"
    )
    lines = [header]
    if topic:
        lines.append(f"┃ 📖 موضوع: <b>{html.escape(topic)}</b>")
    lines.append("┗━━━━━━━━━━━━━━━━━━━━━━━━━┛")
    if cancelled_by:
        lines.append(f"🚫 این پارت توسط {_mention_html(cancelled_by)} لغو شد.")
    late_note = f" + ⏳ {len(late_ids)} با تاخیر" if late_ids else ""
    lines.append(f"👥 <b>وضعیت تا لحظهٔ لغو ({ready_total} نفر{late_note}):</b>")
    lines.append(_format_roster(ready_users, results_map, late_ids))
    lines.append("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬")
    lines.append("⏰ پارت بعدی هر وقت لازم بود با /session یا نظرسنجی خودکار شروع می‌شه.")
    return "\n".join(lines)


async def _finalize_roster_message(
    session_id: int,
    part_number: Optional[int],
    topic: Optional[str],
    actual_minutes: Optional[int],
    ...
    """Last edit ever made to the pinned "پارت شروع شد" message
    (`sess.start_message_id`): swaps its live in-progress design for the
    closed-out one from `_build_completed_roster_text`. The result buttons
    are kept (not stripped) -- someone who's a minute late still has a
    live "🟢 انجام دادم / 🔴 انجام ندادم" to tap, and `report_result` already
    accepts a late report and folds them into the roster as a
    "تازه‌پیوست" (see `register_late_participant`); only a CANCELLED part
    rejects a late tap. Same message_id `_refresh_roster_message` was
    patching throughout the part -- so the one card people had pinned the
    whole time visibly changes to the closed-out design while staying
    tappable. Best-effort and silent on failure, same reasoning as
    `_refresh_roster_message`: a missing bot/group/message or a Telegram
    hiccup here must never break the actual session-ending flow that
    triggered it."""
    if not bot:
        return
    try:
        async with get_session() as db_session:
            ...
            results_map = await _get_session_results_map(db_session, session_id)
            text = _build_completed_roster_text(
                part_number, topic, ready_users, results_map, actual_minutes, next_run_at, late_ids,
            )
            await bot.edit_message_text(
                chat_id=group.telegram_chat_id,
                message_id=sess.start_message_id,
        ...
        if "not modified" not in str(e).lower():
            logger.debug(f"Could not finalize roster message for session {session_id}: {e}")
    except Exception as e:
        logger.debug(f"Could not finalize roster message for session {session_id}: {e}")
...
async def _attendance_timeout(session_id: int):
    ...
    async with get_session() as db_session:
        ...
        # Keep the row itself in sync with the just-recomputed real end
        # time -- it was only ever a placeholder from before we knew who
        # actually showed up. Anything reading sess.scheduled_end (the
        # countdown ticks below, crash-recovery's staleness check) needs
        # the real value, not the pre-attendance guess.
        sess.scheduled_end = new_end_at
        await db_session.flush()

        # The "کیا هستن؟" countdown is done its job now that attendance
        # has closed -- stop it before the poll message itself gets
        # deleted below, so a stray tick never races an edit against a
        # message that's about to vanish.
        _stop_countdown_job("attend", session_id)
        ...
        try:
            ...
            if group and bot:
                ...
                topic_line = f"📖 موضوع: <b>{html.escape(sess.topic)}</b>\n" if sess.topic else ""
                countdown_line = f"⏳ <b>تا پایان پارت:</b> {_format_countdown(rec['extension'] * 60)}\n"
                ...
                await db_session.flush()
                # Every 5s from here until the part ends, keep the
                # countdown above ticking down in place.
                _start_countdown_job("start", session_id)
                try:
...
async def _cleanup_stale_attendance_poll(db_session: "AsyncSession", group_id: int, chat_id: int) -> None:
    """Finds the most recent attendance poll for this group that expired
    with nobody tapping "هستم" (status MISSED) and still has its message
    sitting undeleted in the chat, and deletes it. Meant to be called
    right before sending a brand-new attendance poll -- automatic or via
    /session -- so the group never sees two "آماده‌اید؟" prompts at once:
    the dead one (its 🟢 button no longer does anything useful; tapping
    it just re-triggers a fresh poll via GroupSyncMiddleware) sitting
    right next to the live one, with no way to tell which is which."""
    if not bot:
        return
    result = await db_session.execute(
        select(Session)
        .where(
            ...
            Session.attendance_message_id.isnot(None),
        )
        .order_by(desc(Session.id))
        .limit(1)
    )
    stale_sess = result.scalar_one_or_none()
    if not stale_sess or not stale_sess.attendance_message_id:
        return
    try:
        await bot.delete_message(chat_id, stale_sess.attendance_message_id)
    except Exception as del_err:
        logger.debug(f"Could not delete stale expired poll message for session {stale_sess.id}: {del_err}")
    # Clear it either way -- whether the delete actually succeeded or the
    # message was already gone (manually deleted, chat cleared, etc.) --
    # so a long MISS streak doesn't retry the same doomed delete_message
    # call before every subsequent poll.
    stale_sess.attendance_message_id = None
    await db_session.flush()
...
async def _create_readiness_poll(group_id: int):
    ...
    async with get_session() as session:
        ...
        if bot:
            # Clear out the stale expired poll, right at the moment the
            # new one is about to replace it -- same moment, so there's
            # never a window where both are visible together.
            await _cleanup_stale_attendance_poll(session, group_id, group.telegram_chat_id)
            try:
                sent = await bot.send_message(
                    group.telegram_chat_id,
                    "☀️ آماده پارت مطالعاتی هستید؟\nهرکی هست بزنه 🟢 (حداقل یک نفر کافیه تا پارت برگزار بشه)",
                ...
                await session.flush()
                _start_countdown_job("attend", sess.id)
            except Exception as e:
...
async def _finish_session_early(session_id: int):
    ...
    async with get_session() as db_session:
        ...
        if not sess:
            return
        group_id = sess.group_id
        part_number = await manager.get_group_part_number(sess.group_id, session_id) if group_id else None
        if sess.status == "STARTED":
            _stop_countdown_job("start", session_id)
    ...
    # Turn the pinned roster card people have been watching all part into
    # its finished design -- before the separate announcement below, so
    # the edit (silent, no notification) lands first and the announcement
    # (which does notify) is what people actually see arrive.
    await _finalize_roster_message(session_id, part_number, topic, actual_minutes, next_run_at)
    if group and bot:
        header = (
...
async def _cancel_active_session(session_id: int, cancelled_by: Optional["User"] = None) -> bool:
    """Cancels whatever part is currently in flight for a group -- still
    in the attendance/"هستم؟" phase, or already running -- no matter how
    many people tapped in or already reported a result. Distinct from
    _finish_session_early / _session_end (those represent the part
    actually happening); this represents someone deciding it isn't
    happening after all.

    Cancels every scheduled job tied to this session (attendance_timeout /
    session_start / session_end) so none of them fire later against a
    part that no longer exists, closes out whichever pinned message was
    live (the attendance poll if nobody's tapped past it yet, or the
    roster if the part had already started -- and if a bunch of people
    had already reported, their dots stay visible, just under a ❌
    header instead of pretending nothing happened), and posts one clear
    group-wide announcement -- far cheaper than pinging everyone who was
    in individually, which for a big roster would just be spam stacked
    on top of the cancellation itself. Returns False if there was nothing
    active to cancel."""
    async with get_session() as db_session:
        manager = SessionManager(db_session)
        sess = await manager.get_session(session_id)
        ...
        group_id = sess.group_id
        topic = sess.topic
        attendance_message_id = sess.attendance_message_id
        start_message_id = sess.start_message_id
        part_number = await manager.get_group_part_number(group_id, session_id)
        ready_users, late_ids = await _get_ready_and_late_users(db_session, session_id)
        results_map = await _get_session_results_map(db_session, session_id)

        # All three are safe to call even if the matching job was never
        # scheduled or already fired/was cancelled -- _cancel_scheduled_job
        # is a no-op in that case.
        await _cancel_scheduled_job(_attendance_timeout, session_id, db_session=db_session)
        await _cancel_scheduled_job(_session_start, session_id, db_session=db_session)
        await _cancel_scheduled_job(_session_end, session_id, db_session=db_session)
        _stop_countdown_job("attend", session_id)
        _stop_countdown_job("start", session_id)

        if not await manager.cancel_session(session_id):
            return False
    ...
    try:
        if start_message_id:
            # Part had already started (or attendance had already closed
            # into a roster) -- close out the same pinned card people were
            # watching, same as a normal end, just with a ❌ frame instead
            # of ✅.
            cancelled_text = _build_cancelled_roster_text(
                part_number, topic, ready_users, results_map, late_ids, cancelled_by,
            )
            await bot.edit_message_text(
                chat_id=group.telegram_chat_id,
        ...
        elif attendance_message_id:
            # Still just the "هستید؟" poll -- nobody's dots to preserve yet,
            # just close the poll itself so the 🟢 button stops doing
            # anything.
            poll_header = (
                f"❌ <b>این نظرسنجی لغو شد — پارت #{part_number}</b>"
                if part_number else "❌ <b>این نظرسنجی لغو شد</b>"
            )
            await bot.edit_message_text(
                chat_id=group.telegram_chat_id,
                message_id=attendance_message_id,
                text=f"{poll_header}\nدیگه نیازی به زدن 🟢 نیست.",
                reply_markup=None,
            )
    except TelegramBadRequest as e:
        if "not modified" not in str(e).lower():
            logger.debug(f"Could not close out pinned message while cancelling session {session_id}: {e}")
    except Exception as e:
        logger.debug(f"Could not close out pinned message while cancelling session {session_id}: {e}")

    try:
        by_note = f" — توسط {_mention_html(cancelled_by)}" if cancelled_by else ""
        header = (
            f"❌ <b>پارت #{part_number} لغو شد{by_note}</b>"
            if part_number else f"❌ <b>پارت لغو شد{by_note}</b>"
        )
        await bot.send_message(group.telegram_chat_id, header)
    except Exception as e:
        logger.error(f"Failed to announce cancellation for session {session_id}: {e}")
...
async def _session_end(session_id: int):
    async with get_session() as db_session:
        ...
        if group_id:
            ...
            result = await db_session.execute(select(User).where(User.id == owner_user_id))
    ...
    if group_id:
        ...
        next_run_at = await _schedule_next_poll(session_id)
        # Same finished-design edit as the early-finish path, so a part
        # that simply ran out its timer (nobody tapped "همه" in time)
        # still gets its pinned roster card visibly closed out, not just
        # a fresh announcement while the old card sits there looking
        # exactly like it did mid-part.
        await _finalize_roster_message(session_id, part_number, topic, actual_minutes, next_run_at)
        ...
        # Personal part hit its "حداکثر" window without the owner tapping
        # a result button -- unlike the group flow there's no pinned
        # roster to fall back on as a record, so nudge them in DM to
        # report a result themselves (report_result still accepts it
        # after end_session -- see report_result's status checks, ENDED
        # isn't in its reject list). This also fully releases the
        # personal lock: _has_active_personal_lock only matches STARTED.
        try:
            await bot.send_message(
                owner.telegram_id,
                "⏰ زمان پارت شخصیت تموم شد.\n"
                "اگه چیزی خوندی، همون پیام قبلی رو بزن و «انجام دادم» یا «⏱ ثبت دستی» رو انتخاب کن.",
            )
        except Exception as e:
            logger.error(f"Failed to notify owner for personal session end {session_id}: {e}")
...
def _clear_live_session_jobs():
    """Removes every currently-scheduled APScheduler job tied to session/
    readiness-poll logic (attendance_timeout, session_start, session_end,
    readiness_poll). Used right before a DB restore replaces the tables
    those jobs' ids/args point at -- otherwise a job scheduled against the
    pre-restore data could fire later against an id that means something
    completely different (or nothing at all) post-restore. Recurring
    fixed jobs (morning poll, backup, weekly summary, ...) are untouched;
    recover_from_crash() re-registers whatever the restored
    scheduler_jobs table says should still be pending."""
    stale_funcs = {_attendance_timeout, _session_start, _session_end, _create_readiness_poll}
    for job in scheduler.get_jobs():
        # planpart_* jobs (📤 ارسال پارت شخصی) point at user_plan_parts ids,
        # which a restore replaces just like session ids.
        if (
            job.func in stale_funcs
            or str(job.id).startswith("planpart_")
            or str(job.id).startswith("countdown_attend_")
            or str(job.id).startswith("countdown_start_")
        ):
            try:
...
# ============================================================
# PERSONAL PLAN PARTS ("📤 ارسال پارت شخصی")
# ------------------------------------------------------------
# Turns today's personal plan (UserPlan/UserPlanPart -- the same rows
# the ✅/⬜ checklist under /plan is built from) into real, timed
# personal parts:
#
#   * "arming" a plan registers one APScheduler DateTrigger per part,
#     firing at exactly the HH:MM written in that plan (see
#     PlanRepository.compute_part_times);
#   * when a part fires, a normal personal Session is created for the
#     person (same shape as _start_personal_session: group_id=None,
#     owner_user_id=user, already STARTED, finished with the usual
#     result buttons) whose length is the plan part's own duration and
#     whose title/topic is the plan part's own activity;
#   * asking for a new plan while any of this is queued/running is
#     guarded by a warning + confirmation (see _confirm_replace_plan),
#     and confirming cancels the queued parts and the running part.
#
# Like routines, these jobs live directly in APScheduler and are
# re-installed from the DB on startup (reschedule_plan_parts) -- the
# UserPlanPart row (send_at / send_status) is the durable source of
# truth, so no SchedulerJob rows are involved.
# ============================================================
# A part fires within ~this many seconds of its time or not at all: a
# part whose moment has clearly passed (plan armed late, bot was down) is
# skipped rather than dumped on the person at a random later time, since
# the whole point is "exactly according to the plan's schedule".
PLAN_PART_GRACE_SECONDS = 90
# After a restart, only bother the person about parts that were missed
# recently; anything older is just skipped quietly.
PLAN_PART_MISSED_NOTIFY_HOURS = 12


def _plan_part_job_id(part_id: int) -> str:
    return f"planpart_{part_id}"


def _schedule_plan_part_job(part_id: int, run_at: datetime) -> None:
    scheduler.add_job(
        _plan_part_fire, trigger=DateTrigger(run_date=run_at),
        args=[part_id], id=_plan_part_job_id(part_id), replace_existing=True,
    )


def _remove_plan_part_job(part_id: int) -> None:
    try:
        scheduler.remove_job(_plan_part_job_id(part_id))
    except JobLookupError:
        pass


def _plan_part_title(part: "UserPlanPart") -> str:
    """Raw (unescaped) title of a plan part, exactly as the plan itself
    labels it: the activity it was scheduled for, or "پارت N" when the
    plan had no activities to distribute."""
    return part.activity_name or f"پارت {part.part_index + 1}"


async def _plan_part_display_title(session: AsyncSession, user_id: int, part: "UserPlanPart") -> Optional[str]:
    """HTML-safe activity label as shown in the plan's own schedule row
    ("<activity name> <importance label>"), or None when the plan had no
    activity for this part (its row then just reads "پارت N"). The
    importance label is looked up from the person's current activities
    (only the name is stored on the part); if the activity was
    deleted/renamed since the plan was built, the bare name is used."""
    if not part.activity_name:
        return None
    label = ""
    for act in await ActivityRepository(session).get_active_by_user(user_id):
        if act.name == part.activity_name:
            label = config.IMPORTANCE_LABELS.get(act.importance, "")
            break
    return html.escape(part.activity_name) + (f" {label}" if label else "")


def _plan_part_heading(number: int, total: Optional[int], title: Optional[str]) -> str:
    """"پارت 3 از 7 — ریاضی ⭐⭐⭐ مهم" (title omitted when the plan gave
    this part no activity, so it doesn't read "پارت 3 — پارت 3")."""
    head = f"پارت {number}" + (f" از {total}" if total else "")
    return f"{head} — {title}" if title else head


async def _plan_part_fire(part_id: int) -> None:
    """APScheduler job: the moment a plan part is due. Creates the
    person's personal Session for it and delivers the part message.

    Does nothing if the part is no longer PENDING (cancelled, replaced
    by a new plan, already handled). If the person is busy in another
    part right now (a group part they committed to, or another personal
    part still running), this one is SKIPPED and they're told why --
    two overlapping parts would make the result buttons ambiguous, the
    same reason /session refuses to start a second one.

    Every Telegram call happens AFTER the DB block, once the store lock
    is released (see the KNOWN HOTSPOT notes elsewhere)."""
    telegram_id: Optional[int] = None
    notice: Optional[str] = None
    part_msg: Optional[Tuple[str, int]] = None  # (text, session_id)
    async with get_session() as session:
        result = await session.execute(select(UserPlanPart).where(UserPlanPart.id == part_id))
        part = result.scalar_one_or_none()
        if not part or part.send_status != "PENDING":
            return
        user_result = await session.execute(select(User).where(User.id == part.user_id))
        user = user_result.scalar_one_or_none()
        if not user:
            part.send_status = "CANCELLED"
            return
        telegram_id = user.telegram_id
        siblings = await session.execute(select(UserPlanPart).where(UserPlanPart.plan_id == part.plan_id))
        total = len(siblings.scalars().all())
        number = part.part_index + 1
        heading = _plan_part_heading(
            number, total, await _plan_part_display_title(session, user.id, part)
        )

        if await _has_active_group_lock(session, user.id):
            part.send_status = "SKIPPED"
            notice = (
                f"⏳ وقت <b>{heading}</b> رسید، ولی توی یه پارت گروهی هستی که هنوز "
                f"تموم نشده؛ این پارت ارسال نشد.\n"
                f"پارت‌های بعدی برنامه سر ساعتشون می‌رسن."
            )
        elif await _has_active_personal_lock(session, user.id):
            part.send_status = "SKIPPED"
            notice = (
                f"⏳ وقت <b>{heading}</b> رسید، ولی یه پارت شخصیِ دیگه‌ت هنوز بازه؛ "
                f"این پارت ارسال نشد.\n"
                f"پارت‌های بعدی برنامه سر ساعتشون می‌رسن."
            )
        else:
            now = time_service.now()
            due = time_service.to_tehran(part.send_at) if part.send_at else now
            # The part ends when the *plan* says it does. If this fired a
            # little late (see PLAN_PART_GRACE_SECONDS) that keeps the
            # schedule intact instead of sliding everything later.
            end_at = due + timedelta(minutes=part.duration_minutes)
            if end_at <= now:
                end_at = now + timedelta(minutes=part.duration_minutes)
            manager = SessionManager(session)
            sess = await manager.create_session(
                group_id=None,
                ...
                planned_duration=part.duration_minutes,
                minimum_duration=min(part.duration_minutes, config.DEFAULT_MIN_DURATION),
                target_duration=part.duration_minutes,
                # The plan's own length is also the ceiling: credit is
                # capped by extension_duration in report_result, so a
                # 35-minute plan part can never record more than 35.
                extension_duration=part.duration_minutes,
                mode="normal",
                creation_source="plan_part",
                recommendation_confidence=1.0,
                reason_codes=["PERSONAL_PLAN"],
                topic=part.activity_name[:64] if part.activity_name else None,
            )
            # Same registration dance as _start_personal_session:
            # register_participant only accepts SCHEDULED/ATTENDANCE.
            sess.status = "ATTENDANCE"
            await session.flush()
            await manager.register_participant(sess.id, user.id)
            sess.status = "STARTED"
            sess.actual_start = now
            await session.flush()
            await SchedulerService.schedule_job(
                "session_end", sess.scheduled_end, {"session_id": sess.id}, db_session=session,
            )
            part.send_status = "SENT"
            part.session_id = sess.id
            part_msg = (
                f"📖 <b>{heading}</b>\n\n"
                f"🕐 طبق برنامه‌ی شخصیت: <b>{part.scheduled_label}</b> تا "
                f"<b>{end_at.strftime('%H:%M')}</b> · {part.duration_minutes} دقیقه\n\n"
                f"این پارت فقط برای خودته؛ توی هیچ گروهی اعلام نمی‌شه.\n"
                f"وقتی تموم شد بزن 👇",
                sess.id,
            )
    ...
    if notice:
        try:
            await bot.send_message(telegram_id, notice)
        except Exception as e:
            logger.warning(f"Could not deliver plan-part skip notice for part {part_id}: {e}")
        return
    if part_msg is None:
        return
    text, session_id = part_msg
    try:
        sent = await bot.send_message(
            telegram_id, text, reply_markup=result_keyboard(session_id, show_late_join=False)
        )
    except Exception as e:
        logger.warning(f"Could not deliver plan part {part_id} to {telegram_id}: {e}")
        return
    # Remember which message this is so a later cancel (🆕 برنامه جدید)
    # can close it out instead of leaving live result buttons behind.
    try:
        async with get_session() as session:
            row = await session.get(UserPlanPart, part_id)
            if row is not None:
                row.message_id = sent.message_id
    except Exception as e:
        logger.debug(f"Could not store message_id for plan part {part_id}: {e}")


async def reschedule_plan_parts(notify: bool = True) -> None:
    """Re-installs the APScheduler job of every PENDING plan part on
    startup (and after a DB restore), same idea as
    reschedule_all_routines. A part whose moment already passed while the
    bot was down is marked SKIPPED -- never fired late -- and, if it was
    recent, its owner gets one message per person saying so (`notify` is
    False after a restore: those are old parts from a different point in
    time, and telling people about them would just be confusing)."""
    now = time_service.now()
    cutoff = now - timedelta(seconds=PLAN_PART_GRACE_SECONDS)
    notify_cutoff = now - timedelta(hours=PLAN_PART_MISSED_NOTIFY_HOURS)
    to_schedule: List[Tuple[int, datetime]] = []
    missed_by_user: Dict[int, List[str]] = {}
    async with get_session() as session:
        result = await session.execute(select(UserPlanPart).where(UserPlanPart.send_status == "PENDING"))
        for part in result.scalars().all():
            when = time_service.to_tehran(part.send_at) if part.send_at else None
            if when is not None and when >= cutoff:
                to_schedule.append((part.id, when))
                continue
            part.send_status = "SKIPPED"
            if notify and when is not None and when >= notify_cutoff:
                user_result = await session.execute(select(User).where(User.id == part.user_id))
                user = user_result.scalar_one_or_none()
                if user:
                    missed_by_user.setdefault(user.telegram_id, []).append(
                        f"• پارت {part.part_index + 1} — {html.escape(_plan_part_title(part))} "
                        f"({part.scheduled_label})"
                    )
    for part_id, when in to_schedule:
        _schedule_plan_part_job(part_id, when)
    if to_schedule:
        logger.info(f"Rescheduled {len(to_schedule)} pending personal plan part(s)")
    if bot is not None:
        for telegram_id, lines in missed_by_user.items():
            try:
                await bot.send_message(
                    telegram_id,
                    "⚠️ ربات یه مدت خاموش بود و این پارت‌ها سر ساعتشون ارسال نشدن:\n"
                    + "\n".join(lines)
                    + "\n\nپارت‌های بعدیِ برنامه همچنان سر ساعتشون می‌رسن.",
                )
            except Exception as e:
                logger.debug(f"Could not deliver missed-plan-part notice to {telegram_id}: {e}")


async def _arm_plan_parts(telegram_user_id: int, expected_plan_id: Optional[int]) -> Dict[str, Any]:
    """'📤 ارسال پارت شخصی': queues every still-upcoming, not-yet-done
    part of today's plan to be delivered at its planned time.

    `expected_plan_id` (from the button's callback data) refuses a tap on
    an outdated plan message; None means "whatever today's plan is" (the
    main-menu button). Parts whose time has already passed are marked
    SKIPPED rather than sent late. Returns a dict with `status` in
    {"no_user", "no_plan", "stale", "ok"} plus, for "ok", what was
    queued/skipped and the fresh `parts` list (for refreshing the
    keyboard)."""
    out: Dict[str, Any] = {"status": "ok", "queued": [], "past": [], "already": 0, "parts": []}
    to_schedule: List[Tuple[int, datetime]] = []
    async with get_session() as session:
        ...
        if expected_plan_id is not None and plan.id != expected_plan_id:
            return {"status": "stale"}
        times = plan_repo.compute_part_times(plan, parts)
        now = time_service.now()
        cutoff = now - timedelta(seconds=PLAN_PART_GRACE_SECONDS)
        for part, when in zip(parts, times):
            if part.send_status == "PENDING":
                out["already"] += 1
                continue
            # SENT/SKIPPED are final for this part, and a part the person
            # already ticked ✅ needs nothing sent. Only never-queued or
            # previously-CANCELLED parts are eligible.
            if part.send_status in ("SENT", "SKIPPED") or part.is_done:
                continue
            part.send_at = when
            entry = {
                "number": part.part_index + 1,
                "label": part.scheduled_label,
                "minutes": part.duration_minutes,
                "title": _plan_part_title(part),
                "next_day": when.date() > now.date(),
            }
            if when >= cutoff:
                part.send_status = "PENDING"
                to_schedule.append((part.id, when))
                out["queued"].append(entry)
            else:
                part.send_status = "SKIPPED"
                out["past"].append(entry)
        out["parts"] = parts
        out["total"] = len(parts)
    # Registered only after the transaction above has committed.
    for part_id, when in to_schedule:
        _schedule_plan_part_job(part_id, when)
    return out


async def _disarm_plan_parts(telegram_user_id: int) -> Optional[Tuple[int, List["UserPlanPart"]]]:
    """'⏹ توقف ارسال': cancels every still-PENDING part. A part that's
    already running is left alone (the person can finish it normally).
    Returns (how many were cancelled, today's plan parts for refreshing
    the keyboard), or None if the user isn't known."""
    async with get_session() as session:
        user = await UserRepository(session).get_by_telegram_id(telegram_user_id)
        if not user:
            return None
        result = await session.execute(
            select(UserPlanPart).where(
                UserPlanPart.user_id == user.id, UserPlanPart.send_status == "PENDING"
            )
        )
        cancelled = 0
        for part in result.scalars().all():
            _remove_plan_part_job(part.id)
            part.send_status = "CANCELLED"
            cancelled += 1
        _, parts = await PlanRepository(session).get_today_plan_with_parts(user.id)
        return cancelled, parts


async def _get_personal_run_state(telegram_user_id: int) -> Dict[str, Any]:
    """Read-only look at what a new plan would clobber: parts still
    queued for delivery, and a personal part the person is in the middle
    of right now (a plan part *or* one they started with /session)."""
    state: Dict[str, Any] = {"pending": 0, "running": None}
    async with get_session() as session:
        user = await UserRepository(session).get_by_telegram_id(telegram_user_id)
        if not user:
            return state
        ...
        state["pending"] = len(pending.scalars().all())
        ...
        sessions = running.scalars().all()
        if sessions:
            sess = sessions[0]
            linked = await session.execute(select(UserPlanPart).where(UserPlanPart.session_id == sess.id))
            linked_part = linked.scalar_one_or_none()
            # Raw, optional title: a plan part's own activity, or the
            # topic a /session personal part was tagged with (None when
            # neither exists -- the warning text then just says
            # "پارت شخصی" instead of repeating a generic label).
            title = linked_part.activity_name if linked_part is not None else sess.topic
            end = time_service.to_tehran(sess.scheduled_end) if sess.scheduled_end else None
            minutes_left = max(0, round((end - time_service.now()).total_seconds() / 60)) if end else None
            state["running"] = {
                "title": title,
                "number": (linked_part.part_index + 1) if linked_part is not None else None,
                "minutes_left": minutes_left,
            }
    return state


async def _cancel_personal_run(telegram_user_id: int) -> Dict[str, Any]:
    """Cancels everything _get_personal_run_state reports: every PENDING
    plan part (job removed, row -> CANCELLED) and any running personal
    part (Session -> CANCELLED, its session_end job cancelled), then
    closes out the running part's message if one was recorded so its
    "انجام دادم" buttons stop looking live."""
    info: Dict[str, Any] = {"pending": 0, "running": 0}
    to_close: List[Tuple[int, int]] = []  # (chat_id, message_id)
    ...
    async with get_session() as session:
        ...
        for part in pending.scalars().all():
            _remove_plan_part_job(part.id)
            part.send_status = "CANCELLED"
            info["pending"] += 1
        running = await session.execute(
        ...
        manager = SessionManager(session)
        for sess in running.scalars().all():
            await _cancel_scheduled_job(_session_end, sess.id, db_session=session)
            if await manager.cancel_session(sess.id):
                info["running"] += 1
                linked = await session.execute(
                    select(UserPlanPart).where(UserPlanPart.session_id == sess.id)
                )
                linked_part = linked.scalar_one_or_none()
                if linked_part is not None and linked_part.message_id:
                    to_close.append((chat_id, linked_part.message_id))
    if bot is not None:
        for cid, mid in to_close:
            try:
                await bot.edit_message_text(
                    chat_id=cid, message_id=mid, reply_markup=None,
                    text="❌ <b>این پارت لغو شد</b> — برنامه‌ی جدید گرفتی.\n"
                         "مطالعه‌ی این پارت ثبت نشد.",
                )
            except Exception as e:
                logger.debug(f"Could not close out cancelled plan part message {mid}: {e}")
    return info
...
# ============================================================
# FATIGUE-AWARE MOTIVATION ENGINE
# ============================================================
# Design rationale (see conversation with the admin who requested this):
#
# 1. Self-Determination Theory (Ryan & Deci) treats autonomy, competence
#    and relatedness as three *distinct* psychological needs -- a person
#    who has already proven competence today (a few completed parts)
#    doesn't need more "you can do it", they need something else
#    depending on how depleted they are.
# 2. Mental-fatigue / vigilance-decrement research (e.g. Baumeister et
#    al.'s ego-depletion work; studies on brief rest breaks restoring
#    sustained attention) suggests that after repeated effort, offering
#    a genuine pause or a lower-effort, connection-oriented message
#    serves the person better than another push to keep performing.
#
# So instead of always drawing from the same "SUCCESS"/"MOTIVATION"
# pool, we classify the person into a fatigue tier -- based on how many
# parts they've *completed* today and what time it is -- and each tier
# draws from a different set of message categories.
async def count_user_parts_today(session: AsyncSession, user_id: int) -> int:
    """How many parts this user has actually studied (COMPLETED or
    PARTIAL -- i.e. real study time, not just a "MISSED" tap) since local
    midnight. Mirrors the Tehran-day-boundary conversion used in
    AnalyticsService.calculate_daily_stats: SessionResult.reported_at is
    stored as naive UTC, so local midnight has to be converted to naive
    UTC before comparing, or results near the day boundary get misfiled
    into the wrong day."""
    start_local = time_service.start_of_day()
    start_utc = start_local.astimezone(pytz.UTC).replace(tzinfo=None)
    result = await session.execute(
        select(func.count()).select_from(SessionResult).where(
            SessionResult.user_id == user_id,
            SessionResult.reported_at >= start_utc,
...
class FatigueAwareSelector:
    """Picks which pool of message categories to draw from, based on
    today's completed-part count and the current hour, then delegates the
    actual weighted pick to the existing `message_selector` so admin-added
    custom messages, usage-count weighting and the anti-repeat window all
    keep working unchanged."""

    TIER_CATEGORIES: Dict[str, List[str]] = {
        # Not tired yet -- lean into demonstrated competence and streaks.
        "fresh": ["SUCCESS", "STREAK", "COMPETENCE_BOOST"],
        # Getting tired (~4-5 parts) -- remind them the pace is their
        # choice, and that a short pause is a legitimate option.
        "threshold": ["AUTONOMY_NUDGE", "MICRO_BREAK", "MOTIVATION"],
        # Clearly fatigued (6+ parts, or very late/early hours) -- shift
        # to lower-effort, connection-oriented content instead of another
        # push to perform.
        "fatigued": ["STORY", "RECOVERY", "RELATEDNESS"],
    }

    # Admin-tunable the same way other algorithm thresholds are (see
    # Config.ALGORITHM_WEIGHTS) -- kept as plain class attributes here
    # since they're read, not scored, but overridable at runtime via
    # set_thresholds() from the admin panel if that's wired up later.
    FATIGUE_THRESHOLD_PARTS: int = 4
    FATIGUE_HIGH_PARTS: int = 6

    @classmethod
    def classify(cls, parts_today: int, hour: int) -> str:
        if parts_today >= cls.FATIGUE_HIGH_PARTS:
            return "fatigued"
        # Very late/early hours get the gentler treatment regardless of
        # part count -- reduced alertness in this window isn't only a
        # function of today's time-on-task.
        if hour >= 23 or hour < 5:
            return "fatigued"
        if parts_today >= cls.FATIGUE_THRESHOLD_PARTS:
            return "threshold"
        return "fresh"
    ...
    async def get_message(
        ...
        **variables: Any,
        ...
        tier = self.classify(parts_today, hour)
        categories = list(self.TIER_CATEGORIES[tier])
        random.shuffle(categories)
        for category in categories:
            text = await message_selector.get_message(session, category, user_id, **variables)
            if text:
                return text, tier
        return None, tier


fatigue_selector = FatigueAwareSelector()


def _utc_now() -> datetime:
    """Naive-UTC "now" derived from time_service, so it agrees with the
    naive-UTC columns (SessionResult.reported_at, UserPlanPart.done_at,
    User.created_at, ...) it gets compared against."""
    return time_service.now().astimezone(pytz.UTC).replace(tzinfo=None)
...
async def gather_inactivity_evidence(session: AsyncSession, user_row: "User") -> Dict[str, Any]:
    """Hard evidence of whether (and how) someone is actually studying,
    computed from real events instead of the profile's moving averages:

      * study events -- every COMPLETED result with minutes, plus every
        plan part ticked ✅ (a ✅ counts as showing up; its planned minutes
        only count as study time when no session backs it, so a part
        completed through its own session isn't counted twice);
      * the weekly goal -- minutes over the lookback window vs. the
        daily goal x days the person has actually been around;
      * plan adherence -- parts of *past* days' plans left unticked, and
        which subject is furthest behind;
      * presence -- days since signup and since the bot last saw them
        (someone who opens the bot daily but never studies is a different
        case from someone who vanished).

    Note what this deliberately does NOT use: profile.last_updated. It
    changes on every result, so an explicit "انجام ندادم" tap used to
    reset "days inactive" to zero and hide exactly the people this is
    meant to find."""
    now_utc = _utc_now()
    now_local = time_service.now()
    lookback = max(1, int(config.CONFRONTATION_LOOKBACK_DAYS))
    today_local = now_local.date()
    window_start = today_local - timedelta(days=lookback - 1)

    def _days_since(dt: Optional[datetime]) -> float:
        dt = _as_naive_utc(dt)
        return max(0.0, (now_utc - dt).total_seconds() / 86400) if dt else 9999.0

    signup_days = _days_since(user_row.created_at)
    seen_days = _days_since(user_row.last_seen_at)

    events: List[Tuple[datetime, int]] = []
    res = await session.execute(
        select(SessionResult.reported_at, SessionResult.actual_duration).where(
            SessionResult.user_id == user_row.id, SessionResult.completion_status == "COMPLETED",
        )
    )
    for reported_at, minutes in res.all():
        if reported_at is not None and (minutes or 0) > 0:
            events.append((_as_naive_utc(reported_at), int(minutes)))
    res = await session.execute(
        select(UserPlanPart.done_at, UserPlanPart.duration_minutes, UserPlanPart.session_id).where(
            UserPlanPart.user_id == user_row.id, UserPlanPart.is_done == True,
        )
    )
    for done_at, planned, backing_session in res.all():
        if done_at is not None:
            events.append((_as_naive_utc(done_at), 0 if backing_session else int(planned or 0)))

    def _local_date(dt: datetime) -> date:
        return time_service.to_tehran(dt).date()

    last_study_at = max((t for t, _ in events), default=None)
    study_dates = {_local_date(t) for t, _ in events}
    days_studied = len([d for d in study_dates if d >= window_start])
    minutes_window = sum(m for t, m in events if _local_date(t) >= window_start)

    goal_row = (await session.execute(select(UserGoal).where(UserGoal.user_id == user_row.id))).scalar_one_or_none()
    daily_goal = goal_row.daily_goal if goal_row else 90
    eligible_days = int(min(lookback, max(1, int(signup_days))))
    goal_total = daily_goal * eligible_days
    # Judged only once the person has been around long enough for a
    # ratio to mean something.
    goal_ratio = (minutes_window / goal_total) if (eligible_days >= 3 and goal_total > 0) else None

    today_start = time_service.start_of_day()
    earliest = today_start - timedelta(days=lookback)
    res = await session.execute(
        select(UserPlanPart.is_done, UserPlanPart.activity_name, UserPlanPart.duration_minutes, UserPlanPart.send_status)
        .join(UserPlan, UserPlanPart.plan_id == UserPlan.id)
        .where(
            UserPlanPart.user_id == user_row.id,
            UserPlan.plan_day < today_start,
            UserPlan.plan_day >= earliest,
        )
    )
    ...
    behind: Dict[str, int] = {}
    for is_done, activity_name, planned, send_status in res.all():
        # A part the *system* skipped (bot was down / person was busy in
        # another part / armed after its time) isn't the person's failure.
        if send_status == "SKIPPED" and not is_done:
            continue
        plan_total += 1
        if is_done:
            plan_done += 1
        elif activity_name:
            behind[activity_name] = behind.get(activity_name, 0) + int(planned or 0)
    worst_subject = max(behind, key=lambda k: behind[k]) if behind else None

    res = await session.execute(
        select(SessionResult.reported_at).where(
            SessionResult.user_id == user_row.id, SessionResult.completion_status == "MISSED",
        )
    )
    missed_window = sum(1 for (t,) in res.all() if t is not None and _local_date(_as_naive_utc(t)) >= window_start)

    return {
        "last_study_at": last_study_at,
        "days_since_study": _days_since(last_study_at) if last_study_at else None,
        "studied_today": today_local in study_dates,
        "days_since_signup": signup_days,
        "days_since_seen": seen_days,
        "days_studied_window": days_studied,
        ...
        "plan_done": plan_done,
        "plan_ratio": (plan_done / plan_total) if plan_total else None,
        "worst_subject": worst_subject,
        "worst_subject_minutes": behind.get(worst_subject, 0) if worst_subject else 0,
        "missed_window": missed_window,
    }


# ============================================================
# LAZINESS ASSESSMENT / "برخورد جدی" (tough-love confrontation)
# ============================================================
# Deliberately separate from FatigueAwareSelector above: fatigue is about
# *today's* effort tapering off after several genuine parts, and the
# right response there is a lighter/gentler message or a break.
# Laziness, as defined here, is a *sustained pattern* -- real silence
# over days, a weekly goal barely touched, a plan being ignored, repeated
# explicit misses -- and the right response (per what the admin asked
# for) is a direct, accountability-style message built from the person's
# OWN numbers instead. The two are intentionally never triggered by the
# same signal.
class LazinessAssessor:
    """Turns hard evidence (see gather_inactivity_evidence) + the behavior
    profile into an escalation tier: "firm" / "severe" / "checkin" / None.

    Signals are weighted and summed (Config.CONFRONTATION_MIN_SIGNALS
    gives the thresholds), and several guards keep it from ever scolding
    someone who isn't actually inactive: a signup grace period, a person
    who studied within the last ~day and a half, an active snooze, and a
    back-off once scolding has clearly stopped working."""

    # -- legacy profile-only scoring, kept for callers/tests that still
    # -- pass a profile + days_inactive. maybe_send_confrontation no longer
    # -- uses it: see evaluate().
    @staticmethod
    def signals(profile: "UserBehaviorProfile", days_inactive: Optional[float]) -> int:
        crit = config.LAZINESS_CRITERIA
        count = 0
        if profile.miss_streak >= crit["MISS_STREAK_THRESHOLD"]:
            count += 1
        if profile.completion_rate <= crit["MIN_COMPLETION_RATE"]:
            count += 1
        if profile.consistency_score <= crit["MIN_CONSISTENCY_SCORE"]:
            count += 1
        if days_inactive is not None and days_inactive >= crit["INACTIVITY_DAYS_THRESHOLD"]:
            count += 1
        return count

    @classmethod
    def assess(cls, profile: "UserBehaviorProfile", days_inactive: Optional[float] = None) -> Optional[str]:
        n = cls.signals(profile, days_inactive)
        if n >= config.CONFRONTATION_MIN_SIGNALS["severe"]:
            return "severe"
        if n >= config.CONFRONTATION_MIN_SIGNALS["firm"]:
            return "firm"
        return None

    @classmethod
    def evaluate(cls, profile: "UserBehaviorProfile", ev: Dict[str, Any],
                 calm_mode: bool = False, now: Optional[datetime] = None) -> Tuple[Optional[str], List[str]]:
        """Returns (tier, reason codes). Reason codes are for logging and
        tests: the guard that stopped it ("grace", "snoozed",
        "studied_today", "recent_study", "backoff") or the signals that
        fired ("silent", "goal", "plan", "streak", "completion",
        "consistency")."""
        now = now or _utc_now()
        crit = config.LAZINESS_CRITERIA

        # ---- guards: cases where scolding would just be wrong ----
        if ev["days_since_signup"] < config.CONFRONTATION_SIGNUP_GRACE_DAYS:
            return None, ["grace"]
        snooze = profile.confrontation_snooze_until
        if snooze is not None and _as_naive_utc(snooze) > now:
            return None, ["snoozed"]
        if ev["studied_today"]:
            return None, ["studied_today"]
        days_since_study = ev["days_since_study"]
        if days_since_study is not None and days_since_study < 1.5:
            return None, ["recent_study"]

        # ---- signals ----
        reasons: List[str] = []
        weight = 0
        # Silence: days without any study. Someone who has NEVER studied
        # is measured from signup. A long silence (2x the threshold)
        # counts double, so ~4+ dead days is enough for "firm" by itself.
        silent_days = days_since_study if days_since_study is not None else ev["days_since_signup"]
        threshold = crit["INACTIVITY_DAYS_THRESHOLD"]
        if silent_days >= threshold:
            weight += 2 if silent_days >= 2 * threshold else 1
            reasons.append("silent")
        goal_ratio = ev["goal_ratio"]
        if goal_ratio is not None and goal_ratio <= config.CONFRONTATION_GOAL_SHORTFALL_RATIO:
            weight += 1
            reasons.append("goal")
        plan_ratio = ev["plan_ratio"]
        if (plan_ratio is not None and ev["plan_total"] >= config.CONFRONTATION_PLAN_MIN_PARTS
                and plan_ratio <= config.CONFRONTATION_PLAN_IGNORED_RATIO):
            weight += 1
            reasons.append("plan")
        if profile.miss_streak >= crit["MISS_STREAK_THRESHOLD"]:
            weight += 1
        ...
        mins = config.CONFRONTATION_MIN_SIGNALS
        if weight >= mins["severe"]:
            tier: Optional[str] = "severe"
        elif weight >= mins["firm"]:
            tier = "firm"
        else:
            return None, reasons

        # Scolding that keeps getting ignored isn't working. Stop scolding
        # and only check in gently (spacing is enforced by the caller).
        streak = profile.confrontation_streak or 0
        if streak >= config.CONFRONTATION_MAX_UNANSWERED:
            return "checkin", reasons
        # ...but a *first* "firm" that got no reaction escalates.
        if streak >= 1 and tier == "firm":
            tier = "severe"
        # Calm mode: the person asked for gentleness -- never "severe".
        if calm_mode and tier == "severe":
            tier = "firm"
        return tier, reasons

    @staticmethod
    def cooldown_ok(profile: "UserBehaviorProfile", hours: Optional[float] = None) -> bool:
        if profile.last_confrontation_at is None:
            return True
        hours = config.CONFRONTATION_COOLDOWN_HOURS if hours is None else hours
        elapsed = _utc_now() - _as_naive_utc(profile.last_confrontation_at)
        return elapsed.total_seconds() >= hours * 3600


def _confrontation_facts(ev: Dict[str, Any], tier: str) -> List[str]:
    """The person's own numbers, one line each -- only the ones that are
    actually true and damning, so every line is a fact they can check.
    This is what makes the message hit: not adjectives, their data."""
    facts: List[str] = []
    dss = ev["days_since_study"]
    if dss is None:
        facts.append(f"از وقتی ثبت‌نام کردی ({int(ev['days_since_signup'])} روز پیش) حتی یه پارت هم ثبت نکردی")
    elif dss >= 1.5:
        facts.append(f"{int(dss)} روزه هیچ مطالعه‌ای ثبت نشده")
    goal_ratio = ev["goal_ratio"]
    if goal_ratio is not None and goal_ratio <= 0.5:
        facts.append(
            f"از هدفِ {ev['window_days']} روز اخیرت فقط {int(round(goal_ratio * 100))}٪ رو زدی "
            f"({ev['minutes_window']} از {ev['goal_total']} دقیقه)"
        )
    if ev["plan_ratio"] is not None and ev["plan_total"] >= 3 and ev["plan_ratio"] <= 0.5:
        facts.append(f"{ev['plan_total'] - ev['plan_done']} از {ev['plan_total']} پارتِ برنامه‌های اخیرت انجام نشده")
    if ev["worst_subject"]:
        facts.append(
            f"بیشترین عقب‌افتادگی: {html.escape(ev['worst_subject'])} "
            f"({ev['worst_subject_minutes']} دقیقه‌ی برنامه‌ریزی‌شده مونده)"
        )
    if ev["missed_window"] >= 2:
        facts.append(f"{ev['missed_window']} بار خودت «انجام ندادم» زدی")
    silent = dss is None or dss >= 1.5
    if silent and ev["days_since_seen"] <= 1.5:
        facts.append("هر روز سر می‌زنی، ولی مطالعه‌ای ثبت نمی‌کنی")
    return facts


_CONFRONTATION_DEFAULT_BODY = {
    "firm": "این الگو رو جدی بگیر: برنامه‌ای که برای خودت نوشتی دارد بی‌صاحب می‌مونه. امروز یه قدم بردار.",
    "severe": "این دیگه اتفاقی نیست، یه الگوئه -- و با نگاه کردن بهش عوض نمی‌شه. همین امروز، همین الان شروع کن.",
}


async def _build_confrontation_message(
    session: AsyncSession, user_row: "User", tier: str, ev: Dict[str, Any], miss_streak: int,
) -> Tuple[str, InlineKeyboardMarkup]:
    """Text + buttons of one confrontation. Firm/severe = a header, the
    person's own numbers, a body (an admin-editable TOUGH_LOVE template,
    with {subject}/{goal_pct}/{ignored_parts} available on top of the old
    {miss_streak}/{days_inactive}), and one concrete next action with a
    button that starts it. "checkin" = the gentle back-off message."""
    facts = _confrontation_facts(ev, tier)
    facts_block = "\n".join(f"• {f}" for f in facts)
    subject = ev["worst_subject"]
    kb = InlineKeyboardBuilder()

    if tier == "checkin":
        text = (
            "🤨 چند بار جدی باهات حرف زدم و هیچ واکنشی ندیدم، پس مسیر رو عوض می‌کنم.\n\n"
            + (f"{facts_block}\n\n" if facts_block else "")
            + "اگه یه چیزی مانعته (وقت، انگیزه، برنامه‌ی سنگین)، بگو تا سبک‌ترش کنیم — "
              "یه برنامه‌ی کوچیک که واقعاً انجام بشه، بهتر از یه برنامه‌ی بزرگ که رها می‌شه."
        )
        kb.row(InlineKeyboardButton(text="🆕 برنامه‌ی جدید", callback_data="plan:new"))
        kb.row(InlineKeyboardButton(text="⚙️ تنظیمات", callback_data="menu:settings"))
        kb.row(InlineKeyboardButton(text="⏸ چند روز نپرس", callback_data="tl:snooze"))
        return text, kb.as_markup()

    days_for_template = int(ev["days_since_study"]) if ev["days_since_study"] is not None else int(ev["days_since_signup"])
    goal_pct = int(round((ev["goal_ratio"] or 0) * 100))
    body = await message_selector.get_message(
        session, "TOUGH_LOVE", user_row.id,
        miss_streak=miss_streak,
        days_inactive=days_for_template,
        subject=html.escape(subject) if subject else "درس‌هات",
        goal_pct=goal_pct,
        ignored_parts=(ev["plan_total"] - ev["plan_done"]),
    ) or _CONFRONTATION_DEFAULT_BODY[tier]
    header = "😤 <b>گزارش وضعیت، بدون تعارف</b>" if tier == "severe" else "⚠️ <b>یه یادآوریِ جدی</b>"
    action = (
        f"👇 <b>قدم بعدی:</b> یه پارت کوتاه {'از ' + html.escape(subject) if subject else 'از برنامه‌ت'} — همین الان، نه فردا."
    )
    text = (
        f"{header}\n\n"
        + (f"📉 <b>آمارِ خودت:</b>\n{facts_block}\n\n" if facts_block else "")
    ...
    kb.row(InlineKeyboardButton(text="🚀 شروع پارت شخصی", callback_data="session:personal"))
    kb.row(InlineKeyboardButton(text="🆕 برنامه‌ی جدید", callback_data="plan:new"))
    kb.row(InlineKeyboardButton(text="⏸ چند روز نپرس", callback_data="tl:snooze"))
    return text, kb.as_markup()


async def maybe_send_confrontation(
    session: AsyncSession,
    user_row: "User",
    profile: "UserBehaviorProfile",
    days_inactive: Optional[float] = None,
) -> bool:
    """Central gate for the tough-love feature -- every trigger point
    (a MISSED tap in cb_result, or the daily inactivity sweep) goes
    through this one function, so the opt-in check, the evidence-based
    assessment, the escalation/back-off and the cooldown are enforced in
    one place. Returns True iff a DM was actually sent (and records the
    cooldown/streak immediately, even if the caller's own transaction
    doesn't commit until later).

    `days_inactive` is accepted for backward compatibility but ignored:
    it used to come from profile.last_updated, which an explicit "انجام
    ندادم" tap refreshes -- so it could never see the people who tap
    "missed" instead of studying. The real number now comes from
    gather_inactivity_evidence."""
    if bot is None:
        return False
    pref_result = await session.execute(
        select(UserPreference).where(UserPreference.user_id == user_row.id)
    )
    prefs = pref_result.scalar_one_or_none()
    if not prefs or not prefs.tough_love_enabled:
        return False
    # Cheap early-out before the evidence queries: nothing is ever sent
    # sooner than the base cooldown after the previous one.
    if not LazinessAssessor.cooldown_ok(profile):
        return False

    ev = await gather_inactivity_evidence(session, user_row)
    # They actually studied since the last confrontation -> it worked (or
    # they came back on their own): the "ignored" streak starts over.
    if (profile.last_confrontation_at is not None and ev["last_study_at"] is not None
            and ev["last_study_at"] > _as_naive_utc(profile.last_confrontation_at)):
        profile.confrontation_streak = 0

    tier, reasons = LazinessAssessor.evaluate(
        profile, ev, calm_mode=bool(prefs.calm_mode_enabled),
    )
    if tier is None:
        return False
    if tier == "checkin" and not LazinessAssessor.cooldown_ok(
        profile, hours=config.CONFRONTATION_CHECKIN_DAYS * 24
    ):
        return False

    text, keyboard = await _build_confrontation_message(session, user_row, tier, ev, profile.miss_streak)
    try:
        await bot.send_message(user_row.telegram_id, text, reply_markup=keyboard)
    except Exception as e:
        logger.debug(f"Could not deliver tough-love confrontation to {user_row.telegram_id}: {e}")
        return False
    profile.last_confrontation_at = _utc_now()
    profile.confrontation_streak = (profile.confrontation_streak or 0) + 1
    profile.last_confrontation_tier = tier
    await session.flush()
    logger.info(
        f"Confrontation sent to {user_row.telegram_id}: tier={tier} reasons={reasons} "
        f"streak={profile.confrontation_streak}"
    )
    return True


# ============================================================
# PER-ACTIVITY NEGLECT WARNINGS ("هشدار غفلت از درس/فعالیت")
# ============================================================
# Distinct from LazinessAssessor above: that one judges the *person* from
# their overall study behavior; this one judges a single *subject*
# (UserActivity) from whether the parts the daily plan actually scheduled
# for it are getting done -- weighted by that subject's own importance
# (1-5) and how many days it's been slipping. A کم‌اهمیت subject gets a
# long leash; a حیاتی/فوری one gets almost none. See
# Config.ACTIVITY_NEGLECT_RULES for the exact thresholds per level.
class ActivityNeglectAssessor:
    """Turns one activity's recent (plan_day, is_done) history into an
    escalation tier ("firm" / "severe" / None) + the stats used to build
    the warning text. Requires ACTIVITY_NEGLECT_MIN_SIGNALS["severe"]
    independent signals together for "severe", same 2-of-3 shape as
    LazinessAssessor, so a single rough week doesn't read as severe on
    its own."""

    @staticmethod
    def _rule(importance: int) -> Dict[str, float]:
        return config.ACTIVITY_NEGLECT_RULES.get(
            int(importance), config.ACTIVITY_NEGLECT_RULES[3]
        )
    ...
    def assess(
        ...
        """`history` must be newest-first (see
        PlanRepository.get_activity_part_history). Returns (tier, stats);
        tier is None if nothing was ever actually scheduled for this
        activity yet -- an activity the plan hasn't gotten to isn't
        "neglected", it just hasn't come up."""
        scheduled = len(history)
        if scheduled == 0:
            return None, {}

        rule = cls._rule(importance)
        done_count = sum(1 for _, is_done in history if is_done)
        ratio = done_count / scheduled

        miss_streak = 0
        for _, is_done in history:
            if is_done:
                break
            miss_streak += 1

        last_done_day = next((day for day, is_done in history if is_done), None)
        if last_done_day is not None:
            # Comes from UserPlan.plan_day, already Tehran-aware (see
            # TimeService.start_of_day) -- no conversion needed.
            reference_day = last_done_day
        else:
            # UserActivity.created_at is stored as naive UTC
            # (datetime.utcnow default); make it Tehran-aware before
            # comparing against `now`, same as maybe_send_confrontation
            # does for UserBehaviorProfile.last_updated.
            reference_day = time_service.to_tehran(created_at)
        days_no_progress = max(0.0, (now - reference_day).total_seconds() / 86400)

        signals = 0
        if miss_streak >= rule["max_miss_streak"]:
            signals += 1
        if days_no_progress >= rule["max_days_no_progress"]:
            signals += 1
        if scheduled >= 2 and ratio <= rule["min_completion_ratio"]:
            signals += 1

        if signals >= config.ACTIVITY_NEGLECT_MIN_SIGNALS["severe"]:
            tier = "severe"
        elif signals >= config.ACTIVITY_NEGLECT_MIN_SIGNALS["firm"]:
            tier = "firm"
        else:
            tier = None
        ...
        stats = {
            ...
            "ratio": ratio,
            "miss_streak": miss_streak,
            "days_no_progress": int(days_no_progress),
        }
        return tier, stats

    @staticmethod
    def cooldown_ok(activity: "UserActivity") -> bool:
        if activity.last_neglect_warning_at is None:
            return True
        elapsed = datetime.utcnow() - activity.last_neglect_warning_at
        return elapsed.total_seconds() >= config.ACTIVITY_NEGLECT_COOLDOWN_HOURS * 3600


def _activity_neglect_text(activity: "UserActivity", tier: str, stats: Dict[str, Any]) -> str:
    """Builds the warning text directly (not through MessageSelector's
    admin-editable templates, unlike TOUGH_LOVE) -- this has to reference
    this *specific* activity's name and numbers every time, and it has to
    stay unmistakably serious regardless of what an admin has or hasn't
    seeded, so it's hardcoded here the same way the pinned session-status
    messages are built directly in code."""
    name = html.escape(activity.name)
    importance_label = config.IMPORTANCE_LABELS.get(activity.importance, str(activity.importance))
    scheduled, done = stats["scheduled"], stats["done"]
    miss_streak, days = stats["miss_streak"], stats["days_no_progress"]

    if stats["ratio"] == 0 and done == 0:
        progress_line = f"از {scheduled} پارتی که براش برنامه‌ریزی شده، حتی یکیش رو هم انجام ندادی."
    else:
        progress_line = (
            f"فقط {done} پارت از {scheduled} پارت برنامه‌ریزی‌شده‌ش رو انجام دادی "
            f"({int(stats['ratio'] * 100)}٪)."
        )

    if tier == "severe":
        header = f"🚨 <b>هشدار جدی درباره‌ی «{name}»</b>"
        body = (
            f"این یه درس {importance_label} ـه، نه یه فعالیت اختیاری، و داره کاملاً کنار گذاشته میشه. "
            f"{progress_line} {miss_streak} پارت پشت‌سرهم رو نزدی و {days} روزه هیچ پیشرفتی توش نبوده.\n\n"
            f"این سطح از اهمیت، این سطح از بی‌توجهی رو تحمل نمی‌کنه. همین امروز یه پارت واقعی -- "
            f"هرچقدرم کوتاه -- براش بذار، وگرنه فردا از همین‌جا عقب‌تری."
        )
    else:
        header = f"⚠️ <b>غفلت از «{name}»</b>"
        body = (
            f"داره از دستت در میره. {progress_line} {days} روزه واسش وقت نذاشتی، "
            f"و سطح اهمیتش ({importance_label}) اجازه‌ی این عقب‌افتادن رو نمی‌ده.\n\n"
            f"قبل از اینکه بدتر بشه، یه پارت کوتاه براش بذار و دوباره وصلش کن به برنامه‌ت."
        )
    return f"{header}\n\n{body}"


async def maybe_send_activity_neglect_warning(
    session: AsyncSession,
    user_row: "User",
    activity: "UserActivity",
    lookback_days: Optional[int] = None,
) -> bool:
    """Central gate for this feature, mirroring maybe_send_confrontation's
    shape: pulls the activity's recent part history, runs it through
    ActivityNeglectAssessor, and -- opt-in check + per-activity cooldown
    permitting -- sends the warning DM and stamps
    last_neglect_warning_at so it can't repeat before the cooldown is up,
    even if the caller's own transaction doesn't commit until later."""
    if bot is None:
        return False
    pref_result = await session.execute(
        select(UserPreference).where(UserPreference.user_id == user_row.id)
    ...
    prefs = pref_result.scalar_one_or_none()
    if not prefs or not prefs.activity_neglect_warnings_enabled:
        return False
    if not ActivityNeglectAssessor.cooldown_ok(activity):
        return False

    history = await PlanRepository(session).get_activity_part_history(
        user_row.id, activity.name, lookback_days or config.ACTIVITY_NEGLECT_LOOKBACK_DAYS
    )
    tier, stats = ActivityNeglectAssessor.assess(
        history, activity.importance, activity.created_at, time_service.now()
    )
    if tier is None:
        return False

    text = _activity_neglect_text(activity, tier, stats)
    try:
        await bot.send_message(user_row.telegram_id, text)
    except Exception as e:
        logger.debug(
            f"Could not deliver activity-neglect warning for activity_id={activity.id} "
            f"to {user_row.telegram_id}: {e}"
        )
        return False
    activity.last_neglect_warning_at = datetime.utcnow()
    await session.flush()
    return True
...
def _chunk_message_atoms(atoms: List[Tuple[str, str]], limit: int) -> List[str]:
    """Cuts a long message into pieces of at most `limit` characters
    *without* ever splitting an atom, so HTML tags never end up open
    across two messages. `atoms` is a list of (separator_before, text).
    The separator is only used when the atom is glued onto the previous
    one inside the same chunk; the first atom of a new chunk has none.
    An atom that on its own exceeds `limit` (practically never) is first
    cut on line boundaries."""
    expanded: List[Tuple[str, str]] = []
    for sep, text in atoms:
        if len(text) <= limit:
            expanded.append((sep, text))
            continue
        pieces: List[str] = []
        buf = ""
        ...
        for k, piece in enumerate(pieces):
            expanded.append((sep if k == 0 else "\n", piece))

    chunks: List[str] = []
    current = ""
    for sep, text in expanded:
        if not current:
            current = text
        elif len(current) + len(sep) + len(text) <= limit:
            current += sep + text
        else:
...
class Advisor:
    ...
    def _activity_weight(self, activity: "UserActivity", carryover_minutes: int = 0) -> float:
        ...
        `carryover_minutes`: minutes of this activity's parts that were
        never ticked ✅ in a previous day's plan (see
        PlanRepository.get_unfinished_carryover). A part skipped
        yesterday shouldn't just quietly disappear -- it pushes this
        activity's weight up so today's plan is more likely to actually
        include it, capped so one huge backlog can't swallow the whole
        day."""
        ...
        if carryover_minutes > 0:
            weight *= 1 + min(1.5, carryover_minutes / 120)
        return weight
    ...
    async def generate_personalized_plan(self, user_id: int, session: AsyncSession,
        ...
        if goal is not None:
            ...
            # A UserGoal row is created automatically for every user at
            # signup with daily_goal defaulted to 90 -- its created_at and
            # updated_at only drift apart once the user has actually gone
            # through منو › 🎯 هدف روزانه and set a real number (see
            # set_daily_goal, which touches daily_goal and lets the
            # onupdate on updated_at fire). That's a more reliable "did the
            # user ever actually choose this" signal than just checking
            # whether the value happens to equal the default 90.
            goal_is_default = (goal.updated_at - goal.created_at) < timedelta(seconds=1)
        else:
            daily_goal = pref.daily_goal if pref else 90
            goal_is_default = True

        mode = self._detect_personal_mode(profile, calm_enabled)
        mode_minutes = self._mode_duration(mode, profile)
        # How many parts today's plan splits into is now driven by the
        # length the user actually wants a single sitting to be
        # (UserPreference.preferred_session_duration, set from ⚙️ تنظیمات)
        # rather than only the mode-based recommendation -- the mode
        # number is still used as a fallback for users who never touched
        # that setting, and durations stay clamped to
        # DEFAULT_MIN_DURATION/DEFAULT_MAX_DURATION either way so an
        # extreme custom preference can't produce a broken interval.
        if pref and pref.preferred_session_duration:
            interval_minutes = max(
                config.DEFAULT_MIN_DURATION,
                min(config.DEFAULT_MAX_DURATION, pref.preferred_session_duration),
            )
        else:
            interval_minutes = mode_minutes
        base_minutes = interval_minutes

        # No hardcoded part limit -- the count is exactly what the user's
        # own numbers say: daily_goal / interval_minutes (e.g. 600 min at
        # 30 min per part -> 20 parts, and that's fine). Only a very
        # generous safety ceiling (config.MAX_DAILY_PLAN_PARTS, default
        # 120) exists for absurd inputs. Everything downstream (durations,
        # breaks, clock times, the checkbox keyboard, message length) is
        # built to cope with a large count.
        sessions_count = max(
            1,
            min(config.MAX_DAILY_PLAN_PARTS, round(daily_goal / base_minutes) if base_minutes else 1),
        )
        ...
        plan_repo = PlanRepository(session)
        # Anything left unticked in a previous day's plan doesn't just
        # get forgotten -- it's folded back into today's weighting below
        # (and surfaced to the user in carryover_note) until it actually
        # gets checked off.
        carryover_minutes = await plan_repo.get_unfinished_carryover(user_id)
        ...
        if activities:
            weighted = [
                (a, self._activity_weight(a, carryover_minutes.get(a.name, 0)))
                for a in activities
            ]
            ...
            # Guarantee: anything carried over unticked from a previous
            # day must land at least one slot today, even if its
            # (already carryover-boosted) share still rounds down to
            # zero -- otherwise a skipped part could keep getting
            # skipped by pure rounding forever. Steals the extra slot
            # from whichever activity currently holds the most, since
            # it can afford to give one up.
            for a, _ in weighted:
                if carryover_minutes.get(a.name, 0) > 0 and counts.get(a.id, 0) == 0:
                    donor_id = max(counts, key=lambda k: counts[k])
            ...
            priority_note = (
                "<b>🧮 معیار اولویت‌بندی:</b> اهمیت (۱-۵) × فوریتِ ددلاین"
            )
            ...
            if deferred:
                ...
                priority_note += f"\n⏭ امروز جا نشدن: {deferred_names}"

            carryover_hits = [a.name for a, _ in weighted if carryover_minutes.get(a.name, 0) > 0]
            if carryover_hits:
                carryover_note = (
                    "<b>🔁 از روزهای قبل مونده:</b> تیک نخورده بودن، امروز حتماً لحاظ شدن — "
                    + "، ".join(html.escape(n) for n in carryover_hits)
                )
        else:
            activities_hint = (
        ...
        # and, separately, the parts never actually added up to the
        # user's daily_goal (they were just sessions_count copies of the
        # mode-based interval length). Both are fixed here: each part
        # still gets a relative weight from fatigue taper + activity
        # priority (so a higher-priority activity, or an earlier part,
        # still earns more minutes than the others), but those weights
        # are now used to *split daily_goal itself* between the parts,
        # so the day's total tracks what the user actually asked for.
        # ------------------------------------------------------------
        ...
        for i in range(sessions_count):
            ...
            if aid is not None and weight_by_id:
                ...
                priority_factor = 1.0
            weights.append(taper * priority_factor)

        total_weight = sum(weights) or float(sessions_count)
        raw_durations = [daily_goal * w / total_weight for w in weights]
        raw_durations = [
            max(config.DEFAULT_MIN_DURATION, min(config.DEFAULT_MAX_DURATION, r))
            for r in raw_durations
        ]
        durations: List[int] = [
            min(config.CANDIDATE_DURATIONS, key=lambda x: abs(x - r)) for r in raw_durations
        ]

        # Snapping every part to the nearest allowed candidate length
        # (CANDIDATE_DURATIONS) always drifts the day's total a bit away
        # from daily_goal. Nudge individual parts up/down to their next
        # candidate -- always picking whichever part's move closes the
        # gap the most -- until the total is as close to daily_goal as
        # the discrete candidate set allows, instead of silently letting
        # the whole plan fall short of (or run over) what the user asked
        # for.
        def _next_candidate(current: int, direction: int) -> Optional[int]:
            options = [c for c in config.CANDIDATE_DURATIONS if (c > current if direction > 0 else c < current)]
            if not options:
                return None
            return min(options) if direction > 0 else max(options)

        diff = daily_goal - sum(durations)
        guard = 0
        max_guard = max(200, sessions_count * 8)
        while diff != 0 and guard < max_guard:
            direction = 1 if diff > 0 else -1
            best_i, best_next, best_gain = None, None, 0
            for i in range(sessions_count):
                nxt = _next_candidate(durations[i], direction)
                if nxt is None or not (config.DEFAULT_MIN_DURATION <= nxt <= config.DEFAULT_MAX_DURATION):
                    continue
                gain = abs(diff) - abs(diff - (nxt - durations[i]))
                if gain > best_gain:
                    best_gain, best_i, best_next = gain, i, nxt
            if best_i is None:
                # No remaining move brings the total closer -- this is as
                # near to daily_goal as the candidate durations allow.
                break
            durations[best_i] = best_next
            diff = daily_goal - sum(durations)
            guard += 1
        ...
        # Real clock time of each part. With many parts (or a late start)
        # the day can run past 22:00 or even past midnight. The old code
        # clamped every such time to 23:45, which made several different
        # parts show the *same* time. Now the clock simply wraps past
        # midnight (label stays "HH:MM", which is what UserPlanPart stores)
        # and day_offsets[i] remembers that part i falls on the next day,
        # so the plan text can tag it with 🌙.
        DAY_MINUTES = 24 * 60
        slots: List[str] = []
        day_offsets: List[int] = []
        cursor_minutes = start_total_minutes
        last_end_abs = start_total_minutes
        for i in range(sessions_count):
            day_offsets.append(cursor_minutes // DAY_MINUTES)
            clock = cursor_minutes % DAY_MINUTES
            slots.append(f"{clock // 60:02d}:{clock % 60:02d}")
            last_end_abs = cursor_minutes + durations[i]
            cursor_minutes += durations[i]
            if i < len(breaks):
                cursor_minutes += breaks[i]

        overflow_note = ""
        if last_end_abs > day_end_minutes:
            end_clock = last_end_abs % DAY_MINUTES
            end_txt = f"{end_clock // 60:02d}:{end_clock % 60:02d}"
            overflow_note = (
                f"⚠️ با {sessions_count} پارت و استراحت‌های بینشون، این برنامه تا حدود <b>{end_txt}</b>"
                + (" (بعد از نیمه‌شب 🌙)" if last_end_abs >= DAY_MINUTES else "")
                + " طول می‌کشه و از ساعت ۲۲ رد می‌شه. اگه نمی‌خوای، از ⚙️ تنظیمات طول ترجیحی هر پارت "
                "رو بیشتر کن (پارت‌ها کمتر می‌شن) یا هدف روزانه رو کم کن."
            )
        ...
        def _schedule_row(i: int, slot: str) -> str:
            ...
            if act:
                stars = config.IMPORTANCE_LABELS.get(act.importance, '')
                label = f"{html.escape(act.name)} {stars}"
            else:
                label = f"پارت {i + 1}"
            if day_offsets[i] >= 1:
                day_tag = " 🌙" if day_offsets[i] == 1 else f" 🌙+{day_offsets[i]}"
            else:
                day_tag = ""
            # One tight line per part: number · time · duration · activity.
            # The break (if any) sits on its own short line right under it
            # so the eye can tell "study" and "rest" apart at a glance
            # instead of everything running together.
            row = f"{i + 1}. 🕐 {slot}{day_tag} · {durations[i]}د — {label}"
            if i < len(breaks):
                row += f"\n    ⏸ استراحت {breaks[i]}د"
            return row

        schedule_rows = [_schedule_row(i, slot) for i, slot in enumerate(slots)]
        ...
        if activities:
            ...
            alloc_section = f"<b>📌 تخصیص زمان</b>\n{alloc_lines}"
        ...
        if goal_is_default:
            goal_note = (
                f"ℹ️ هدف روزانه‌ت هنوز روی مقدار پیش‌فرض (<b>{daily_goal} دقیقه</b>) تنظیمه -- "
                f"از منو › 🎯 هدف روزانه می‌تونی به هر عددی که خودت می‌خوای عوضش کنی، همینجا "
                f"برنامه هم خودش رو با عدد جدید تطبیق می‌ده."
            )

        break_note = (
            f"⏸ حداقل {config.MIN_BREAK_MINUTES} دقیقه استراحتِ کامل بین پارت‌ها، و استراحت‌های بلندتر "
            f"بعد از هر ~{config.ULTRADIAN_CEILING_MINUTES} دقیقه پیوسته. (قابل تغییر از ⚙️ تنظیمات)"
        )
        if gap_compressed:
            break_note += (
                "\n⚠️ برای جا شدن قبل از ساعت ۲۲، فقط استراحت‌ها کمی فشرده‌تر شدن، نه خودِ پارت‌ها."
            )
        ...
        footer_text = (
            "این برنامه ثابت نیست؛ با /session هر وقت بخوای می‌تونی جورش کنی."
        )

        # The plan is built from "atoms" (a separator + a piece of text
        # whose HTML tags are balanced on their own): the header, one atom
        # per schedule row, then each remaining section, then the footer.
        # `message` is just all atoms joined; `message_chunks` is the same
        # content cut *between* atoms into pieces that each fit under
        # Telegram's 4096-char limit -- with 20+ parts one message isn't
        # enough (see _chunk_message_atoms / _send_plan_message).
        atoms: List[Tuple[str, str]] = [("", header_text)]
        for k, row in enumerate(schedule_rows):
            atoms.append(("\n\n" if k == 0 else "\n", row))
        for section in tail_sections:
            if section:
                atoms.append((divider, section))
        atoms.append(("\n\n", footer_text))

        message = "".join(sep + text for sep, text in atoms)
        message_chunks = _chunk_message_atoms(atoms, config.PLAN_MESSAGE_CHUNK_LIMIT)
...
class BackupService:
    ...
    def list_backups(self) -> List[Path]:
        ...
        files = [
            ...
            if p.is_file() and p.suffix == ".json" and p.name.startswith(("backup_", "merged_"))
        ]
    ...
    async def restore_from_json(self, backup_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            # The tables just got wiped and replaced wholesale, so:
            # (1) any APScheduler job still sitting in memory from *before*
            #     the restore now points at ids that mean something
            #     different (or nothing) in the restored data -- drop them;
            # (2) reconcile the freshly-restored Session/SchedulerJob rows
            #     the same way a real crash recovery would, instead of
            #     leaving stale in-flight sessions sitting there until the
            #     next full process restart (which is what used to make
            #     /session permanently report "a part is already running"
            #     until someone thought to /cancel it by hand, and could
            #     also replay long-overdue jobs live once a restart *did*
            #     happen -- re-announcing old parts in every affected group
            #     at once).
            try:
                _clear_live_session_jobs()
                await recover_from_crash()
                # user_plan_parts was replaced too: re-queue whatever the
                # restored data says is still PENDING (quietly -- these are
                # old parts, not something to message people about).
                await reschedule_plan_parts(notify=False)
            except Exception as e:
                logger.error(f"Post-restore reconciliation failed: {e}")
    ...
    def _load_backup_payload(self, path: Path) -> Tuple[Optional[dict], str]:
        try:
            with open(path, "r", encoding="utf-8") as f:
                payload = json.load(f)
        except Exception as e:
            return None, f"فایل قابل خواندن نیست: {e}"
        if not isinstance(payload.get("tables"), dict):
            return None, "ساختار فایل پشتیبان نامعتبر است."
        return payload, ""

    def merge_json_backups(self, path_a: Path, path_b: Path) -> Tuple[Optional[dict], str, Dict[str, Any]]:
        """Merges two backup JSON files into one payload, row by row per
        table, so that whichever of the two happens to be more complete
        for a given record "wins" without throwing away what the other
        one had.

        Matching: rows are matched across the two files by each table's
        real primary key (pulled from the live model definitions via
        `_pk_fields`) -- not by position -- since the two backups were
        very likely taken at different times and won't have their rows
        in the same order or even the same row counts.

        Per matched row, merge is field-by-field: an empty/None value on
        one side is filled in from the other side whenever the other
        side actually has something there. If *both* sides have a
        non-empty value for a field and they disagree, the value from
        whichever backup's `created_at` is later wins for that field
        (treated as the more current one) -- this is a heuristic, not a
        guarantee, since a "more recent" backup isn't necessarily right
        about every single field.

        A row that exists in only one of the two files is kept as-is.
        A table the currently-running schema doesn't recognize (e.g. a
        renamed/removed table from an older backup) is still carried
        through rather than dropped, de-duplicated by exact row content
        since there's no known primary key to match rows on.
        """
        payload_a, err_a = self._load_backup_payload(path_a)
        if payload_a is None:
            return None, f"فایل اول: {err_a}", {}
        payload_b, err_b = self._load_backup_payload(path_b)
        if payload_b is None:
            return None, f"فایل دوم: {err_b}", {}

        def _parse_dt(payload: dict) -> datetime:
            try:
                return datetime.fromisoformat(payload.get("created_at", ""))
            except Exception:
                return datetime.min

        # Tie-breaker for fields where both files disagree with a real
        # (non-empty) value: whichever payload is newer wins.
        a_wins_ties = _parse_dt(payload_a) >= _parse_dt(payload_b)

        pk_by_table = {t.name: (t.model._pk_fields or []) for t in Base.metadata.sorted_tables}
        known_order = [t.name for t in Base.metadata.sorted_tables]

        all_table_names = list(dict.fromkeys(
            known_order
            + list(payload_a.get("table_order", []))
            + list(payload_a["tables"].keys())
            + list(payload_b.get("table_order", []))
            + list(payload_b["tables"].keys())
        ))

        merged_tables: Dict[str, List[Dict[str, Any]]] = {}
        stats = {"only_first": 0, "only_second": 0, "combined": 0, "fields_filled": 0}

        for name in all_table_names:
            rows_a = payload_a["tables"].get(name, [])
            rows_b = payload_b["tables"].get(name, [])
            pk_fields = pk_by_table.get(name)

            if not pk_fields:
                # Unknown/keyless table -- union, de-duplicated by exact content.
                seen: Set[str] = set()
                merged_rows = []
                ...
                merged_tables[name] = merged_rows
                continue

            def _key(row: Dict[str, Any], _pk_fields=pk_fields) -> tuple:
                return tuple(row.get(pk) for pk in _pk_fields)

            keys_a = {_key(row) for row in rows_a}
            keys_b = {_key(row) for row in rows_b}
            stats["only_first"] += sum(1 for k in keys_a if k not in keys_b)
            stats["only_second"] += sum(1 for k in keys_b if k not in keys_a)

            by_key: Dict[tuple, Dict[str, Any]] = {_key(row): dict(row) for row in rows_a}
            for row in rows_b:
                k = _key(row)
                if k not in by_key:
                    by_key[k] = dict(row)
                ...
                row_changed = False
                for col, b_val in row.items():
                    a_val = existing.get(col)
                    a_empty = a_val is None or a_val == ""
                    b_empty = b_val is None or b_val == ""
                    if a_empty and not b_empty:
                        merged_row[col] = b_val
                        row_changed = True
                    elif (not a_empty) and (not b_empty) and a_val != b_val:
                        merged_row[col] = a_val if a_wins_ties else b_val
                        row_changed = True
                if row_changed:
                    stats["fields_filled"] += 1
                stats["combined"] += 1
                by_key[k] = merged_row
            merged_tables[name] = list(by_key.values())

        merged_payload = {
            "format_version": max(payload_a.get("format_version", 1), payload_b.get("format_version", 1)),
            "created_at": time_service.now().isoformat(),
            "table_order": [n for n in known_order if n in merged_tables]
                            + [n for n in merged_tables if n not in known_order],
            "tables": merged_tables,
        }
        stats["total_rows"] = sum(len(v) for v in merged_tables.values())
        return merged_payload, "", stats

    def save_merged_payload(self, payload: dict) -> Path:
        """Writes an already-merged payload (from merge_json_backups) to a
        new file in the backups dir, ready to be picked up by the normal
        restore-confirm flow just like any other backup file."""
        self.backup_dir.mkdir(parents=True, exist_ok=True)
        timestamp = time_service.now().strftime("%Y%m%d_%H%M%S")
        merged_path = self.backup_dir / f"merged_{timestamp}.json"
...
class AdminStates(StatesGroup):
    ...
    # Merge-two-backups flow (♻️ بازیابی -> 🔀 ادغام دو بک‌آپ): collects
    # the two files one after another before merge_json_backups runs --
    # see cb_admin_restore_merge / admin_merge_upload_first_received /
    # admin_merge_upload_second_received.
    MERGE_UPLOAD_FIRST = State()
    MERGE_UPLOAD_SECOND = State()
    BROADCAST_NOTIFICATION = State()
    # Bulk message import: admin sends one JSON file containing many
    # messages at once instead of using ➕ افزودن پیام one at a time -- see
    # cb_admin_msg_json / admin_msg_json_received / _parse_message_import_payload.
    MSG_JSON_UPLOAD = State()
    # Bulk message add: admin picks one category, then sends messages one
    # per Telegram message (collected in FSM state, not written to the DB
    # yet), and taps ✅ پایان to commit all of them together -- see
    # cb_admin_msg_bulkpick / admin_bulk_add_message_received / cb_admin_msg_bulkdone.
    BULK_ADD_MESSAGES = State()
...
class UserStates(StatesGroup):
    ...
    ADD_ACTIVITY_MASTERY_HOURS = State()
    EDIT_ACTIVITY_MASTERY_HOURS = State()
...
# KEYBOARDS
# ============================================================
def main_menu(is_admin: bool = False, show_personal_session: bool = False) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(
    ...
    # Private-chat only: starting a solo part used to require typing
    # /session by hand even in PV. Same _start_personal_session logic as
    # the command, just reachable with a tap now. Kept out of the group
    # menu -- a group's "شروع پارت" is the group readiness poll / /session
    # inside the group, not this personal, nobody-else-sees-it part.
    if show_personal_session:
        kb.row(InlineKeyboardButton(text="🚀 شروع پارت شخصی", callback_data="session:personal"))
        # Unlike the button above (one solo part, right now, sized by the
        # recommendation engine), this one delivers every part of *today's
        # personal plan* at the times/titles written in it -- see
        # cb_plan_send_parts. "m" = no specific plan message, use today's.
        kb.row(InlineKeyboardButton(text="📤 ارسال پارت شخصی", callback_data="planpart:send:m:0"))
    kb.row(InlineKeyboardButton(text="✨ انگیزه بده", callback_data="motivate:me"))
    if is_admin:
...
def result_keyboard(session_id: int, show_late_join: bool = True) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(
    ...
    # Explicit late-join: for someone who missed the "هستم" attendance
    # window entirely but still wants to jump into a part that's already
    # running. Tapping it (see cb_late_join / register_explicit_late_join)
    # marks the exact moment they joined, so when they later tap
    # "🟢 انجام دادم" their study time is measured from that moment, not
    # from when the whole part started. Meaningless for a personal part
    # (show_late_join=False): the owner is the only participant and is
    # already registered the instant the part starts, so the button would
    # only ever hit the "already registered" branch in cb_late_join
    # instead of doing anything useful.
    if show_late_join:
        kb.row(InlineKeyboardButton(text="⏳ با تاخیر پیوستن", callback_data=f"latejoin:{session_id}"))
    kb.row(InlineKeyboardButton(text="✨ انگیزه بده", callback_data="motivate:me"))
    return kb.as_markup()
...
def restore_menu_keyboard(backups: List[Path]) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="🔀 ادغام دو بک‌آپ", callback_data="admin:restore:merge"))
    kb.row(InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:back"))
    return kb.as_markup()
...
def bulk_add_prompt_keyboard(count: int) -> InlineKeyboardMarkup:
    """Shown while an admin is mid-way through the bulk-add-messages flow
    (AdminStates.BULK_ADD_MESSAGES): a running count on the finish button so
    they can see how many messages are queued, plus cancel. Nothing is
    written to the DB until ✅ پایان is tapped -- see cb_admin_msg_bulkdone."""
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text=f"✅ پایان ({count} پیام)", callback_data="admin:msg:bulkdone"))
    kb.row(InlineKeyboardButton(text="❌ لغو", callback_data="cancel:messages"))
    return kb.as_markup()


def yes_no_keyboard(
    confirm_cb: str, cancel_cb: str,
    confirm_text: str = "⚠️ تایید حذف", cancel_text: str = "❌ انصراف",
) -> InlineKeyboardMarkup:
    """Generic two-button confirm/cancel row, shared by every destructive
    reset/bulk-delete action added below (message bulk-delete, per-category
    reset, global message reset, activities reset, routines reset) so each
    one gets the same 'are you sure' pattern already used by
    restore_confirm_keyboard / admin:msg:del without duplicating it."""
    kb = InlineKeyboardBuilder()
    kb.row(
...
MESSAGE_CATEGORIES: List[Tuple[str, str, str]] = [
    ...
    ("STREAK", "🔥", "استریک"),
    ("MOTIVATION", "✨", "انگیزشی"),
    ("STORY", "📖", "داستان انگیزشی"),
    ("MORNING", "☀️", "صبح بخیر"),
    ("GENERAL", "💬", "عمومی"),
    # --- Fatigue-aware categories (see FatigueAwareSelector) ---
    # These four exist to give someone who has just done a few parts a
    # message of a genuinely *different kind*, not just another "keep
    # going" -- see FatigueAwareSelector.TIER_CATEGORIES.
    ("COMPETENCE_BOOST", "🏅", "تقویت حس تسلط"),
    ("AUTONOMY_NUDGE", "🧭", "یادآوری انتخاب خودت"),
    ("MICRO_BREAK", "🌿", "پیشنهاد استراحت کوتاه"),
    ("RELATEDNESS", "🤝", "حس هم‌مسیری"),
    # Opt-in only -- see UserPreference.tough_love_enabled /
    # LazinessAssessor. Deliberately firm and direct about the *behavior*
    # pattern (misses, no-shows), never about the person's worth or
    # character -- see the note on tone in SAMPLE_MESSAGES["TOUGH_LOVE"].
    ("TOUGH_LOVE", "😤", "برخورد جدی (تنبلی)"),
]
...
SAMPLE_MESSAGES: Dict[str, List[str]] = {
    ...
    "STORY": [
        (
            "📖 توماس ادیسون، قبل از اینکه لامپ رو موفق بسازه، هزاران بار تلاشش شکست خورد. "
            "وقتی ازش پرسیدن این‌همه شکست ناامیدت نکرد؟ گفت من شکست نخوردم، فقط هزاران راهی "
            "رو پیدا کردم که جواب نمی‌ده. همون سمج بودن ساده بود که آخرش نتیجه داد.\n\n"
            "درس: هر تلاش ناموفق هم داره تو رو به جواب نزدیک‌تر می‌کنه، فقط ادامه بده 💡"
        ),
        (
            "📖 جی.کی رولینگ، قبل از چاپ هری پاتر، یه مادر مجرد بیکار بود که دوازده ناشر "
            "کتابش رو رد کردن. با همه‌ی سختی‌های مالی و روحی، دست از نوشتن نکشید تا بالاخره "
            "یه ناشر کوچیک پذیرفت چاپش کنه؛ حالا یکی از پرفروش‌ترین نویسنده‌های تاریخه.\n\n"
            "درس: رد شدن یعنی هنوز به آدم درست نرسیدی، نه اینکه راه اشتباهی رفتی ✨"
        ),
        (
            "📖 آبراهام لینکلن قبل از رسیدن به ریاست‌جمهوری آمریکا، چندین بار توی کسب‌وکار "
            "شکست خورد، عزیزانش رو از دست داد و چند انتخابات رو هم باخت. با همه‌ی این‌ها هر بار "
            "دوباره بلند شد و ادامه داد تا سرانجام یکی از تاثیرگذارترین رهبران تاریخ شد.\n\n"
            "درس: مسیر موفقیت هیچ‌وقت یه خط صاف نیست، مهم اینه که بعد از هر افتادن بلند شی 🌱"
        ),
    ],
    "MORNING": [
    ...
    # Early parts of the day: reinforce mastery/self-efficacy while it's
    # cheap to notice (SDT: competence).
    "COMPETENCE_BOOST": [
        "داری نشون می‌دی که از پسش برمیای؛ همین تکرار کوچیک، تسلطته 🏅",
        "هر پارتی که تموم می‌کنی، یه مهارت واقعی داره جا میفته توی ذهنت 🧠",
        "این دقیقاً همون چیزیه که آدم‌های حرفه‌ای هم همینطوری یاد گرفتن: تکرار درست 🎯",
    ],
    # Threshold zone (~4-5 parts): remind them the choice is theirs, and
    # a short pause is a legitimate option, not slacking (SDT: autonomy).
    "AUTONOMY_NUDGE": [
        "تا اینجا خودت انتخاب کردی ادامه بدی؛ از همینجا هم خودتی که تصمیم می‌گیری چطور ادامه بدی 🧭",
        "لازم نیست دقیقاً مثل قبل پیش بری؛ می‌تونی سرعتت رو خودت تنظیم کنی 🎚",
        "هیچکس مجبورت نکرده تا اینجا بیای، خودت اومدی. همین یعنی می‌تونی مسیر بعدی رو هم خودت بچینی 🧩",
    ],
    "MICRO_BREAK": [
        "چند پارت پشت‌سرهم زدی؛ ۵ دقیقه فقط چشمات رو ببند و نفس بکش، بعد ادامه بده 🌿",
        "الان وقت خوبیه برای یه مکث کوتاه، نه توقف. یه لیوان آب بخور و برگرد 💧",
        "مغزت بعد از چندتا پارت پشت‌سرهم به یه استراحت کوتاه واقعا نیاز داره، نه فشار بیشتر 🌬",
    ],
    # High-fatigue zone: shift toward connection/belonging and lower-effort
    # content (SDT: relatedness) rather than another push to perform.
    "RELATEDNESS": [
        "خیلیا دقیقا همین لحظه‌ی خسته‌ای که تو هستی رو تجربه کردن و رد شدن؛ تنها نیستی توی این مسیر 🤝",
        "امروز به اندازه‌ی کافی زدی. همینکه بازم برگشتی و ادامه دادی خودش ارزشمنده 💙",
        "خستگی امروز نشونه‌ی ضعف نیست، نشونه‌ی اینه که واقعا کار کردی 🌙",
    ],
    # Only sent to users who explicitly opted in (tough_love_enabled) and
    # only after several independent signals of sustained laziness fire
    # together (see LazinessAssessor) -- never after one bad day.
    # Deliberately firm/blunt in tone, but always aimed at the *behavior*
    # ("این چندتا پارت رو نزدی")، never at the person's worth or character
    # -- no insults, no name-calling, no shaming. Each message ends with
    # one concrete, doable next action, not just criticism. {miss_streak}
    # and {days_inactive} are filled in from the actual profile numbers.
    "TOUGH_LOVE": [
        "بیا رک باشیم: {miss_streak} پارت رو پشت‌سرهم نزدی. این دیگه بدشانسی نیست، انتخابه. "
        "همین الان یه پارت ۱۰ دقیقه‌ای بذار، همین یکی 👊",
        "داری از خودت فرار می‌کنی، نه از درس. {days_inactive} روزه هیچی ثبت نشده. "
        "لازم نیست امروز کامل جبران کنی، فقط یه شروع کوچیک بذار و بشکنش 🔥",
        "هر بهونه‌ای که الان تو ذهنته، دفعه‌ی قبل هم همین بود. وقتشه یکی‌شون رو امروز بشکنی -- "
        "فقط یک پارت، همین حالا 💢",
        "اگه واقعا هدفی که گفتی برات مهمه، رفتار الانت باهاش جور در نمیاد. "
        "یه قدم بردار، همین الان، نه فردا 🧨",
        # Use the person's own numbers ({subject}, {goal_pct}, {days_inactive},
        # {ignored_parts}); still about behavior only, never about worth.
        "{days_inactive} روز گذشته و {subject} همون‌جا موند. این دلداری نیست، گزارشِ وضعیته: "
        "هر روزی که می‌گذره، جبرانش سنگین‌تر می‌شه. امروز شروع کن 🔥",
        "هدفت رو خودت نوشتی، ولی الان {goal_pct}٪ ش رو زدی و {ignored_parts} پارتِ برنامه انجام نشده. "
        "عدد دروغ نمی‌گه. یه پارت کوتاه از {subject}، همین حالا 💢",
    ],
}
...
def message_management() -> InlineKeyboardMarkup:
    ...
    # Two bulk-import paths so an admin never has to add messages one at a
    # time through ➕ افزودن پیام:
    #  - 📥 آپلود JSON: upload one file with many messages across any number
    #    of categories at once (see cb_admin_msg_json).
    #  - 📤 افزودن گروهی: pick one category, then send messages one per
    #    Telegram message and hit پایان to commit them all together (see
    #    cb_admin_msg_bulkadd).
    kb.row(
        InlineKeyboardButton(text="📥 آپلود JSON", callback_data="admin:msg:json"),
        InlineKeyboardButton(text="📤 افزودن گروهی", callback_data="admin:msg:bulkadd"),
    )
    # Direct shortcut to the story/motivational-text category -- skips the
    # generic category picker since this is the section admins reach for
    # most (see cb_motivate_me / STORY in MESSAGE_CATEGORIES).
    kb.row(InlineKeyboardButton(text="📖 داستان‌های انگیزشی", callback_data="admin:msg:browse:STORY"))
    kb.row(InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:back"))
    return kb.as_markup()
...
NOTIFICATION_PREF_FIELDS: List[Tuple[str, str]] = [
    ...
    ("tough_love_enabled", "😤 برخورد جدی موقع تنبلی زیاد"),
    ("activity_neglect_warnings_enabled", "⚠️ هشدار غفلت از درس/فعالیت"),
]
...
def activities_list_keyboard(activities: List["UserActivity"]) -> InlineKeyboardMarkup:
    ...
    if activities:
        kb.row(InlineKeyboardButton(text="🗑 ریست همه فعالیت‌ها", callback_data="act:resetall"))
...
def activity_detail_keyboard(activity_id: int) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="⏱ تغییر ساعت تخمینی تسلط", callback_data=f"act:remastery:{activity_id}"))
...
def _parse_mastery_hours(text: Optional[str]) -> Optional[float]:
    """Best-effort parse of the free-text "how many hours to master this"
    answer. Anything that isn't a clean positive number (an explicit skip
    word, plain text, a negative/zero/absurd value) comes back as None --
    deliberately *not* a guessed default, since the person asked the bot
    to never fill one in on their behalf."""
    raw = (text or "").strip()
    if not raw:
        return None
    # Persian digits -> ASCII, so "۴۰" parses the same as "40".
...
def _format_mastery_hours(hours: Optional[float]) -> str:
    if hours is None:
        return "نامشخص"
    text = f"{hours:g}"
    return f"{text} ساعت"
...
def routines_list_keyboard(routines: List["UserRoutine"]) -> InlineKeyboardMarkup:
    ...
    if routines:
        kb.row(InlineKeyboardButton(text="🗑 ریست همه روتین‌ها", callback_data="routine:resetall"))
    kb.row(InlineKeyboardButton(text="🔙 بازگشت", callback_data="menu:back"))
    return kb.as_markup()
...
def plan_keyboard(parts: List["UserPlanPart"], page: int = 0) -> InlineKeyboardMarkup:
    ...
    Paginated: a plan can now have 20+ parts, and one button per part
    would run into Telegram's 100-buttons-per-message limit (and be an
    endless scroll on a phone long before that). Only
    config.PLAN_KEYBOARD_PAGE_SIZE parts are shown at a time, with a
    ◀️ / ▶️ row to move between pages (handled by cb_plan_page). With
    few enough parts it's a single page and looks exactly as before --
    no nav row at all."""
    kb = InlineKeyboardBuilder()
    page_size = max(1, config.PLAN_KEYBOARD_PAGE_SIZE)
    total_pages = max(1, -(-len(parts) // page_size))
    page = max(0, min(page, total_pages - 1))
    for part in parts[page * page_size:(page + 1) * page_size]:
    ...
    if total_pages > 1:
        # plan_id in the callback lets cb_plan_page refuse a tap from
        # someone else in a group (whose plan keyboard this isn't) and
        # from an outdated plan message.
        plan_id = parts[0].plan_id
        nav: List[InlineKeyboardButton] = []
        if page > 0:
            nav.append(InlineKeyboardButton(text="◀️ قبلی", callback_data=f"planpage:{plan_id}:{page - 1}"))
        nav.append(InlineKeyboardButton(text=f"📄 {page + 1}/{total_pages}", callback_data="planpage:noop"))
        if page < total_pages - 1:
            nav.append(InlineKeyboardButton(text="بعدی ▶️", callback_data=f"planpage:{plan_id}:{page + 1}"))
        kb.row(*nav)
    if parts:
        # "📤 ارسال پارت شخصی": have the bot deliver each part of this plan
        # at its planned time. While any part is still queued the same
        # slot becomes "⏹ توقف ارسال". plan_id + page ride along in the
        # callback so a tap on an outdated plan message is refused and
        # the keyboard refresh stays on the same page.
        plan_id = parts[0].plan_id
        if any(p.send_status == "PENDING" for p in parts):
            kb.row(InlineKeyboardButton(text="⏹ توقف ارسال پارت‌های شخصی", callback_data=f"planpart:stop:{plan_id}:{page}"))
        else:
            kb.row(InlineKeyboardButton(text="📤 ارسال پارت شخصی", callback_data=f"planpart:send:{plan_id}:{page}"))
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=main_menu(is_admin, show_personal_session=message.chat.type == "private")
    )


@router.message(Command("panel"))
async def cmd_panel(message: Message):
    """Opens the admin panel straight from the ☰ hamburger command menu
    (see _register_bot_commands) instead of only being reachable through
    the "🛠 پنل مدیریت" inline button on the /start message -- that button
    scrolls out of view the moment anything else gets sent in the chat,
    which is exactly the "غیب می‌شه" (disappears) complaint. The hamburger
    entry sits next to the text box regardless of scroll position, so this
    gives admins a second, always-reachable way in. Same access check and
    same panel as the "menu:admin" callback -- this only adds an entry
    point, not a second panel."""
...
async def _start_personal_session(telegram_user_id: int, target: Message):
    """The /session command run from a private chat (and now also the
    '🚀 شروع پارت شخصی' button in the main menu -- see main_menu /
    cb_start_personal_session): a solo part that belongs to nobody's
    group (Session.group_id stays None, owner_user_id is this person).
    No attendance poll -- there's only one person to ask -- so it goes
    straight to STARTED, same shape as a group part that already
    finished its "هستید؟" phase. Completion (result:<id>:COMPLETED/
    MISSED/MANUAL) reuses the exact same cb_result / report_result /
    _finish_session_early code path as group parts: with a single READY
    participant, reporting a result always satisfies the early-finish
    check, so the part ends itself the moment this person taps a result
    button -- no separate personal "session end" flow needed. See
    _has_active_group_lock / _has_active_personal_lock for the
    mutual-exclusion rule.

    Takes the acting user's Telegram id explicitly (rather than reading
    target.from_user) because when called from the button, target is the
    bot's own menu message -- target.from_user would resolve to the bot,
    not the person who tapped it."""
    async with get_session() as session:
        repo = UserRepository(session)
        user = await repo.get_by_telegram_id(telegram_user_id)
        if not user:
            await target.answer("❌ لطفاً ابتدا دستور /start را بزنید.")
            return

        if await _has_active_group_lock(session, user.id):
            await target.answer(
                "⏳ الان توی یه پارت گروهی داوطلب شدی که هنوز تموم نشده.\n"
                "تا وقتی اون تموم بشه (یا نتیجه‌شو ثبت کنی) نمی‌تونی پارت شخصی جدید بگیری."
            )
            return
        ...
        if existing.scalars().all():
            await target.answer(
                "⏳ یه پارت شخصی دیگه همین الان برات فعاله.\n"
                "اول با «انجام دادم» یا «انجام ندادم» تمومش کن، بعد یکی جدید بگیر."
            )
            return
        ...
        sess = await manager.create_session(
            ...
            owner_user_id=user.id,
            scheduled_start=now,
            scheduled_end=now + timedelta(minutes=rec["extension"]),
            planned_duration=rec["target"],
            minimum_duration=rec["minimum"],
            target_duration=rec["target"],
            extension_duration=rec["extension"],
            mode=rec["mode"],
            creation_source="personal_request",
            recommendation_confidence=rec["confidence"],
            reason_codes=rec["reason_codes"],
        )
        # register_participant only accepts SCHEDULED/ATTENDANCE sessions
        # (see SessionManager.register_participant) -- pass through
        # ATTENDANCE momentarily so the owner gets registered exactly the
        # same way a group participant would, then jump straight to
        # STARTED since there's no one else to wait on.
        sess.status = "ATTENDANCE"
        await session.flush()
        await manager.register_participant(sess.id, user.id)
        sess.status = "STARTED"
        ...
        await SchedulerService.schedule_job(
            "session_end", sess.scheduled_end,
            {"session_id": sess.id}, db_session=session,
        )

        await target.answer(
            f"📖 <b>پارت شخصی شروع شد</b>\n"
            f"⏱ حداقل: <b>{rec['minimum']}</b> | هدف: <b>{rec['target']}</b> | "
            f"حداکثر: <b>{rec['extension']}</b> دقیقه\n\n"
            f"این پارت فقط برای خودته؛ توی هیچ گروهی اعلام نمی‌شه.\n"
            f"وقتی تموم شد بزن 👇",
            reply_markup=result_keyboard(sess.id, show_late_join=False),
        )


@router.callback_query(F.data == "session:personal")
async def cb_start_personal_session(callback: CallbackQuery):
    """'🚀 شروع پارت شخصی' from the main menu -- the button equivalent of
    typing /session in a private chat (see main_menu / _start_personal_session).
    Only meaningful in PV (a group's own '/session' already means
    something else -- a group part); guarded here in case an old menu
    message with this button is still sitting in a group."""
    if callback.message.chat.type != "private":
        await callback.answer("این دکمه فقط توی چت خصوصی با ربات کار می‌کنه.", show_alert=True)
        return
    await _start_personal_session(callback.from_user.id, callback.message)
...
async def cmd_session(message: Message):
    if message.chat.type == "private":
        await _start_personal_session(message.from_user.id, message)
        return
    async with get_session() as session:
        ...
        # Same cleanup the automatic poll does: if this group has a dead
        # expired poll sitting around (nobody tapped "هستم" last time),
        # clear it out right as this new one goes out, so there's never a
        # moment with two "هستید؟" prompts visible at once.
        await _cleanup_stale_attendance_poll(session, group_id, message.chat.id)
...
async def _send_today_status(telegram_user_id: int, target: Message):
    ...
    async with get_session() as session:
        ...
        await target.answer(
            status["message"],
            reply_markup=main_menu(show_personal_session=target.chat.type == "private"),
        )
...
def _running_part_heading(running: Dict[str, Any]) -> str:
    title = html.escape(running["title"]) if running.get("title") else None
    if running.get("number"):
        return _plan_part_heading(running["number"], None, title)
    return "پارت شخصی" + (f" — {title}" if title else "")


async def _ask_replace_plan_confirmation(telegram_user_id: int, target: Message,
                                         start_in_minutes: Optional[int]) -> bool:
    """The warning shown before a new plan is built while personal parts
    are queued ("📤 ارسال پارت شخصی") or one is running. Returns True if
    it asked (the caller must stop and wait for the tap), False if
    there's nothing at stake and the plan can just be built.

    The confirm button carries the plan mode ("new" = 🆕 برنامه جدید with
    the person's start offset, "auto" = plain /plan) so cb_plan_replace
    rebuilds exactly the plan they asked for."""
    state = await _get_personal_run_state(telegram_user_id)
    running, pending = state["running"], state["pending"]
    if not running and not pending:
        return False

    lines = ["⚠️ <b>هشدار قبل از ساخت برنامه‌ی جدید</b>", ""]
    if running:
        left = f" (حدود {running['minutes_left']} دقیقه‌ش مونده)" if running.get("minutes_left") else ""
        lines.append(f"الان وسط یه پارتی: <b>{_running_part_heading(running)}</b>{left}.")
        lines.append("")
    lines.append("اگه ادامه بدی:")
    if running:
        lines.append("• این پارت <b>لغو می‌شه</b> و مطالعه‌ای که تا الان توش داشتی <b>ثبت نمی‌شه</b>.")
    if pending:
        lines.append(f"• {pending} پارتِ ارسال‌نشده‌ی باقی‌مونده هم لغو می‌شن و دیگه سر ساعتشون نمی‌رسن.")
    lines.append("• برنامه‌ی جدید خودکار ارسال نمی‌شه؛ بعد از ساخت، دوباره «📤 ارسال پارت شخصی» رو بزن.")
    if running:
        lines.append("")
        lines.append(
            "💡 اگه می‌خوای مطالعه‌ی همین پارت ثبت بشه، اول توی پیام خودِ پارت «🟢 انجام دادم» یا "
            "«⏱ ثبت دستی» رو بزن، بعد برنامه‌ی جدید بگیر."
        )

    mode = "new" if start_in_minutes is not None else "auto"
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="✅ آره، لغو کن و برنامه‌ی جدید بساز", callback_data=f"plan:replace:{mode}"))
    kb.row(InlineKeyboardButton(text="↩️ نه، همون قبلی بمونه", callback_data="plan:replace:keep"))
...
async def _send_personalized_plan(telegram_user_id: int, target: Message,
    ...
    Building a plan replaces today's one, so if the person has personal
    parts queued for delivery ("📤 ارسال پارت شخصی") or is in the middle
    of one, nothing is built yet: a warning with a confirm/keep choice is
    sent instead and this returns False (see _ask_replace_plan_confirmation
    / cb_plan_replace, which calls back here with `confirmed=True`).
    Returns True in every other case."""
    if not confirmed and await _ask_replace_plan_confirmation(telegram_user_id, target, start_in_minutes):
        return False
    # NOTE: every DB read/write happens inside this block, but the actual
    # Telegram send (target.answer(...)) happens AFTER it, once store.lock
    # has been released. Sending a message is a slow network call; doing
    # it while still holding get_session()'s lock would make every other
    # button in the bot (e.g. the ✅/⬜ part-toggle taps below) queue up
    # and wait behind it -- the same hotspot already fixed in
    # _daily_personal_plan_broadcast, applied here too.
    no_user = False
    no_data_message = None
    plan_parts: List["UserPlanPart"] = []
    result: Dict[str, Any] = {}
    async with get_session() as session:
        repo = UserRepository(session)
        user = await repo.get_by_telegram_id(telegram_user_id)
        if not user:
            ...
            if not result.get("has_data"):
                no_data_message = result["message"]
            else:
                plan_repo = PlanRepository(session)
    ...
    if no_data_message is not None:
        ...
        return True
    await _send_plan_message(
        target.answer,
        result.get("message_chunks") or [result["message"]],
        plan_keyboard(plan_parts),
    )
    return True


async def _send_plan_message(send: Callable[..., Awaitable[Any]], chunks: List[str],
                             reply_markup: InlineKeyboardMarkup, prefix: str = "") -> None:
    """Sends a plan that may have been split into several messages (see
    _chunk_message_atoms -- a plan with many parts doesn't fit in one
    4096-char Telegram message). `send(text, reply_markup=...)` is any
    coroutine function with that shape (message.answer, or a lambda
    around bot.send_message). The done/not-done keyboard goes on the
    LAST message only, so it sits right under the end of the plan; an
    optional `prefix` (e.g. the good-morning line) is glued to the first
    message."""
    parts_out = list(chunks) or [""]
    if prefix:
        parts_out[0] = prefix + parts_out[0]
    last = len(parts_out) - 1
    for idx, text in enumerate(parts_out):
        await send(text, reply_markup=reply_markup if idx == last else None)
...
async def _send_activities_list(telegram_user_id: int, target: Message, edit: bool = False):
    ...
    if activities:
        ...
        text = (
            ...
            "اگه ددلاین هم داشته باشه، توی اولویت‌بندی روزانه در نظرش می‌گیرم. یه تخمین ساعت تا تسلط هم "
            "ازت می‌پرسم -- کاملاً اختیاریه."
        )
...
@router.message(UserStates.ADD_ACTIVITY_NAME, ~F.text.startswith("/"))
...
@router.message(UserStates.ADD_ACTIVITY_DEADLINE, ~F.text.startswith("/"))
async def add_activity_deadline(message: Message, state: FSMContext):
    ...
    await state.update_data(activity_deadline_days=days)
    await state.set_state(UserStates.ADD_ACTIVITY_MASTERY_HOURS)
    await message.answer(
        f"تقریباً چند ساعت مطالعه برای تسلط کامل روی «{name}» لازمه؟ یه عدد بفرست (مثلاً 40).\n\n"
        "اگه نمی‌دونی یا نمی‌خوای بگی، مشکلی نیست -- هر چیز دیگه‌ای بفرست یا رد کن، "
        "من خودم به‌جات عددی حدس نمی‌زنم."
    )


@router.message(UserStates.ADD_ACTIVITY_MASTERY_HOURS, ~F.text.startswith("/"))
async def add_activity_mastery_hours(message: Message, state: FSMContext):
    mastery_hours = _parse_mastery_hours(message.text)

    data = await state.get_data()
    name = data.get("activity_name")
    importance = data.get("activity_importance", 3)
    days = data.get("activity_deadline_days", 0)
    if not name:
        await message.answer("❌ چیزی گم شد، لطفاً دوباره با /activities امتحان کن.")
        await state.clear()
        return
    ...
    async with get_session() as session:
        ...
        if not user:
            ...
            return
        activity_repo = ActivityRepository(session)
        await activity_repo.create(user.id, name, importance, deadline, mastery_hours)
        await session.commit()

    await state.clear()
    deadline_note = f"، ددلاین {days} روز دیگه" if days > 0 else ""
    mastery_note = f"، تخمین تسلط {_format_mastery_hours(mastery_hours)}" if mastery_hours is not None else ""
    await message.answer(
        f"✅ «{name}» با اهمیت {config.IMPORTANCE_LABELS.get(importance, importance)}"
        f"{deadline_note}{mastery_note} اضافه شد.\n"
...
async def cb_act_view(callback: CallbackQuery):
    ...
    async with get_session() as session:
        ...
        mastery_text = f"⏱ تخمین ساعت تا تسلط: {_format_mastery_hours(activity.estimated_mastery_hours)}"
        text = (
            f"📌 {activity.name}\n\n"
...
@router.callback_query(F.data.startswith("act:remastery:"))
async def cb_act_remastery(callback: CallbackQuery, state: FSMContext):
    activity_id = int(callback.data.split(":")[-1])
    await state.update_data(edit_activity_id=activity_id)
    await state.set_state(UserStates.EDIT_ACTIVITY_MASTERY_HOURS)
    await callback.message.answer(
        "تقریباً چند ساعت مطالعه برای تسلط کامل روی این فعالیت لازمه؟ یه عدد بفرست (مثلاً 40).\n\n"
        "اگه نمی‌دونی، هر چیز دیگه‌ای بفرست -- پاکش می‌کنم و دوباره نامشخص می‌مونه.",
        reply_markup=cancel_keyboard("activities"),
    )
    await callback.answer()


@router.message(UserStates.EDIT_ACTIVITY_MASTERY_HOURS, ~F.text.startswith("/"))
async def edit_activity_mastery_hours(message: Message, state: FSMContext):
    data = await state.get_data()
    activity_id = data.get("edit_activity_id")
    if not activity_id:
        await message.answer("❌ چیزی گم شد، لطفاً دوباره امتحان کن.")
        await state.clear()
        return
    mastery_hours = _parse_mastery_hours(message.text)
    ...
    async with get_session() as session:
        ...
        if not user:
            ...
            return
        activity_repo = ActivityRepository(session)
        activity = await activity_repo.set_mastery_hours(activity_id, user.id, mastery_hours)
        if not activity:
            await message.answer("❌ این فعالیت پیدا نشد.")
            await state.clear()
            return
        await session.commit()

    await state.clear()
    await message.answer(f"✅ تخمین ساعت تسلط به‌روزرسانی شد: {_format_mastery_hours(mastery_hours)}")
    await _send_activities_list(message.from_user.id, message)
...
@router.callback_query(F.data == "act:resetall")
async def cb_act_resetall(callback: CallbackQuery):
    await callback.message.edit_text(
        "⚠️ همه‌ی فعالیت‌ها/درس‌های ثبت‌شده‌ی تو حذف می‌شن و این کار برگشت‌پذیر نیست. مطمئنی؟",
        reply_markup=yes_no_keyboard("act:resetallconfirm", "act:list"),
    )
    await callback.answer()


@router.callback_query(F.data == "act:resetallconfirm")
async def cb_act_resetallconfirm(callback: CallbackQuery):
    async with get_session() as session:
        repo = UserRepository(session)
        user = await repo.get_by_telegram_id(callback.from_user.id)
        if not user:
            ...
            return
        activity_repo = ActivityRepository(session)
        count = await activity_repo.deactivate_all_for_user(user.id)
        await session.commit()
    await callback.answer(f"🗑 {count} فعالیت پاک شد" if count else "چیزی برای پاک کردن نبود")
    await _send_activities_list(callback.from_user.id, callback.message, edit=True)
...
# Tracks which STORY-category MessageTemplate ids have already been shown
# by the "✨ انگیزه بده" button (cb_motivate_me) during the current
# rotation cycle. Cleared back to empty the moment every active story has
# been shown at least once, so the set only ever holds "seen since the
# last full cycle" -- letting us tell, on every pick, whether the story
# about to go out is the last fresh one left before repeats start.
_STORY_ROTATION_SEEN_IDS: Set[int] = set()


async def _notify_admins_story_pool_low(remaining_unseen: int, total_stories: int) -> None:
    """Pings every admin in config.ADMIN_IDS once per rotation cycle, right
    when the STORY pool (📖 داستان انگیزشی, sent by '✨ انگیزه بده') has just
    handed out its last not-yet-repeated story -- meaning the very next
    pick has to reuse one someone already saw. Only fires once per cycle
    since _STORY_ROTATION_SEEN_IDS resets right after this, not on every
    subsequent (now-necessarily-repeating) pick. Best-effort: a failed DM
    to one admin (e.g. they never started the bot in PV) must not stop the
    others from being notified, and must never block the motivate-me flow
    that triggered it."""
    if not bot or not config.ADMIN_IDS:
        return
    text = (
        "⚠️ <b>داستان‌های انگیزشی دارن تکراری می‌شن</b>\n"
        f"از {total_stories} داستان فعال، دیگه هیچ‌کدوم تازه نمونده -- از این به بعد "
        "دکمه «✨ انگیزه بده» دوباره از همون داستان‌های قبلی انتخاب می‌کنه.\n"
        "اگه دوست داری تکراری نشه، از پنل ادمین (📖 داستان‌های انگیزشی) چندتا داستان جدید اضافه کن."
    )
    for admin_id in config.ADMIN_IDS:
        try:
            await bot.send_message(admin_id, text)
        except Exception as e:
            logger.debug(f"Could not notify admin {admin_id} about low story pool: {e}")


@router.callback_query(F.data == "motivate:me")
async def cb_motivate_me(callback: CallbackQuery):
    """The '✨ انگیزه بده' button -- available from the main menu, the
    readiness poll and the part-completion keyboard (see main_menu,
    attendance_keyboard, result_keyboard). Draws from the two
    inspiration-focused categories (short MOTIVATION quotes + longer STORY
    texts, see MESSAGE_CATEGORIES/SAMPLE_MESSAGES) instead of one, so short
    taps and longer reads are both in rotation. Also widened with whichever
    fatigue-tier categories fit how many parts *this* person has already
    studied today (see FatigueAwareSelector) -- someone tapping this after
    six parts is more likely to get a MICRO_BREAK/RELATEDNESS message mixed
    in, not just another generic MOTIVATION quote. In a group: shown as an
    alert popup so it doesn't clutter the chat, unless the picked text is
    longer than Telegram's 200-char alert limit (e.g. a STORY pick), in
    which case it falls back to a normal message. In private chats the
    behavior is unchanged -- always a normal message."""
    async with get_session() as session:
        user_repo = UserRepository(session)
        db_user = await user_repo.get_by_telegram_id(callback.from_user.id)
        categories = {"MOTIVATION", "STORY"}
        if db_user:
            parts_today = await count_user_parts_today(session, db_user.id)
            tier = FatigueAwareSelector.classify(parts_today, time_service.hour())
            categories.update(FatigueAwareSelector.TIER_CATEGORIES[tier])
        result = await session.execute(
            select(MessageTemplate).where(
                MessageTemplate.category.in_(list(categories)),
                MessageTemplate.active == True,
            )
        )
        pool = result.scalars().all()
        if not pool:
            await callback.answer("فعلاً پیامی توی این بخش ثبت نشده 🙁", show_alert=True)
            return
        weights = [m.weight for m in pool]
        msg = random.choices(pool, weights=weights, k=1)[0]
        msg.usage_count += 1
        msg.last_used_at = datetime.utcnow()
        await session.flush()
        await session.commit()
        text = msg.text
        # Rotation bookkeeping is STORY-only (see _STORY_ROTATION_SEEN_IDS
        # docstring) -- short MOTIVATION quotes repeating isn't the
        # "تکرار داستان" the admin cares about, only the longer stories are.
        story_pool = [m for m in pool if m.category == "STORY"]
        should_notify = False
        if msg.category == "STORY" and len(story_pool) > 1:
            _STORY_ROTATION_SEEN_IDS.add(msg.id)
            remaining_unseen = sum(1 for m in story_pool if m.id not in _STORY_ROTATION_SEEN_IDS)
            if remaining_unseen == 0:
                should_notify = True
                # Reset now so the next full cycle starts fresh instead of
                # notifying again on every pick from here on.
                _STORY_ROTATION_SEEN_IDS.clear()
    if should_notify:
        await _notify_admins_story_pool_low(0, len(story_pool))
    # In groups: show as a popup (show_alert) so it doesn't clutter the
    # group chat -- but Telegram caps alert text at 200 chars, so anything
    # longer than that (e.g. a STORY pick) still has to go out as a normal
    # message instead. Private chats are untouched: always a normal
    # message, exactly as before.
    is_group_chat = callback.message.chat.type in ("group", "supergroup")
    if is_group_chat and len(text) <= 200:
    ...
    try:
        ...
        # Message the button was attached to may be gone/uneditable in some
        # edge cases (e.g. old group message) -- fall back to a DM.
...
@router.message(UserStates.ROUTINE_ADD_NAME, ~F.text.startswith("/"))
...
@router.message(UserStates.ROUTINE_ADD_DURATION, ~F.text.startswith("/"))
...
@router.message(UserStates.ROUTINE_ADD_ONCE_DAYS, ~F.text.startswith("/"))
...
@router.message(UserStates.ROUTINE_ADD_TIME, ~F.text.startswith("/"))
...
@router.callback_query(F.data == "routine:resetall")
async def cb_routine_resetall(callback: CallbackQuery):
    await callback.message.edit_text(
        "⚠️ همه‌ی روتین‌های تو (و استریک‌هاشون) حذف می‌شن و این کار برگشت‌پذیر نیست. مطمئنی؟",
        reply_markup=yes_no_keyboard("routine:resetallconfirm", "routine:list"),
    )
    await callback.answer()


@router.callback_query(F.data == "routine:resetallconfirm")
async def cb_routine_resetallconfirm(callback: CallbackQuery):
    async with get_session() as session:
        repo = UserRepository(session)
        user = await repo.get_by_telegram_id(callback.from_user.id)
        if not user:
            ...
            return
        routine_repo = RoutineRepository(session)
        ids = await routine_repo.deactivate_all_for_user(user.id)
        await session.commit()
    for routine_id in ids:
        unschedule_routine(routine_id)
    await callback.answer(f"🗑 {len(ids)} روتین پاک شد" if ids else "چیزی برای پاک کردن نبود")
    await _send_routines_list(callback.from_user.id, callback.message, edit=True)
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "/plan - برنامه شخصی امروز (چند پارت، چه ساعتی، بر پایه آخرین یافته‌های علمی) -- با دکمه‌ی 📤 ارسال پارت شخصی زیرش، ربات هر پارت رو دقیقاً سر ساعت برنامه برات می‌فرسته\n"
        "/activities - فعالیت‌ها/درس‌هات و میزان اهمیتشون رو مدیریت کن\n"
        "/session - ایجاد پارت جدید (داخل گروه: پارت گروهی | داخل چت خصوصی: پارت شخصی مستقل -- یا دکمه‌ی 🚀 شروع پارت شخصی توی منو)\n"
        ...
        "/cancel - (داخل گروه، فقط ادمین) لغو کامل پارت در حال اجرا/نظرسنجی جاری\n"
...
async def cb_ready(callback: CallbackQuery):
    ...
    async with get_session() as session:
        ...
        if not sess or sess.status not in ["SCHEDULED", "ATTENDANCE"]:
            ...
            return
        if await _has_active_personal_lock(session, user.id):
            await callback.answer(
                "⏳ الان داری یه پارت شخصی می‌زنی. تا وقتی تمومش نکنی "
                "(انجام دادم/ندادم رو بزنی)، نمی‌تونی توی پارت گروهی هم داوطلب بشی.",
                show_alert=True,
            )
            return
...
@router.callback_query(F.data.startswith("latejoin:"))
async def cb_late_join(callback: CallbackQuery):
    """"⏳ با تاخیر پیوستن" -- for someone who missed the "هستم" attendance
    window but wants to officially join a part that's already running.
    Records the exact moment they joined (see
    SessionManager.register_explicit_late_join); their name then shows up
    in a separate "با تاخیر پیوستگان" block under the on-time roster (see
    _format_roster), and once they tap "🟢 انجام دادم" their study time is
    measured from this moment, not from when the whole part started (see
    SessionManager.report_result)."""
    try:
        session_id = int(callback.data.split(":")[1])
    except (IndexError, ValueError):
        await callback.answer("❌ خطا")
    ...
    async with get_session() as session:
        ...
        if not sess or sess.status != "STARTED":
            await callback.answer(
                "❌ این پارت الان در وضعیتی نیست که بشه با تاخیر بهش پیوست.",
                show_alert=True,
            )
            return
        participant = await manager.register_explicit_late_join(session_id, user.id)
        if not participant:
            await callback.answer("❌ قبلاً توی این پارت ثبت شده‌ای.", show_alert=True)
            return

    # Reflect the new "با تاخیر پیوستگان" entry on the pinned roster right
    # away, same as any other roster-changing tap.
    await _refresh_roster_message(session_id)
    await callback.answer(
        "⏳ ثبت شد! از همین الان زمان مطالعه‌ات حساب می‌شه. "
        "وقتی تموم کردی، «🟢 انجام دادم» رو بزن.",
        show_alert=True,
    )
...
async def cmd_cancel(message: Message):
    """Cancels the group's currently in-flight part -- whether it's still
    in the "هستید؟" attendance phase or already started -- no matter how
    many people tapped in or already reported a result (could be 100
    people; doesn't matter, this doesn't wait for or need their
    agreement). Gated the same way /autopoll is: only that group's own
    Telegram admins/owner, or a global bot admin, can run it -- cancelling
    affects everyone in the part, not just whoever typed the command."""
    if message.chat.type not in ("group", "supergroup"):
        await message.answer(
            "این دستور فقط داخل خود گروه کار می‌کنه -- توی همون گروهی که "
            "می‌خوای پارتش رو لغو کنی بزنش."
        )
        return

    if not await _is_group_chat_admin(message.bot, message.chat.id, message.from_user.id):
        await message.answer("⛔ لغو‌کردن پارت رو فقط ادمین‌های همین گروه می‌تونن انجام بدن.")
        return
    ...
    async with get_session() as session:
        ...
        result = await session.execute(
            select(Session).where(
                Session.group_id == group.id,
                Session.status.in_(["SCHEDULED", "ATTENDANCE", "READY", "STARTED"]),
            )
        )
        active_sess = result.scalar_one_or_none()
        if not active_sess:
            await message.answer("⛔ الان پارتی در جریان نیست که لغوش کنم.")
            return
        session_id = active_sess.id
        user_repo = UserRepository(session)
        cancelled_by = await user_repo.get_by_telegram_id(message.from_user.id)

    ok = await _cancel_active_session(session_id, cancelled_by=cancelled_by)
    if ok:
        await message.answer("✅ پارت لغو شد. همه توی گروه در جریان گذاشته شدن.")
    else:
        # Lost a race -- the part ended/was cancelled between the check
        # above and here (e.g. everyone tapped "انجام دادم" a second
        # before the admin's /cancel landed).
        await message.answer("⛔ این پارت همین الان تموم/لغو شده -- دیگه چیزی برای لغو نیست.")
...
async def cb_result(callback: CallbackQuery, state: FSMContext):
    ...
    # A personal part that was cancelled (🆕 برنامه جدید -> confirm) can
    # still have a stale result-button message on screen (e.g. the cancel
    # couldn't edit it). Say so, rather than the misleading "already
    # reported" the generic failure path below would give. Group parts are
    # unaffected: their cancel already rewrites the pinned message.
    async with get_session() as guard_session:
        guard_sess = await SessionRepository(guard_session).get_by_id(session_id)
        personal_cancelled = bool(guard_sess and guard_sess.group_id is None and guard_sess.status == "CANCELLED")
    if personal_cancelled:
        await callback.answer("🚫 این پارت لغو شده و دیگه نتیجه‌ش قابل ثبت نیست.", show_alert=True)
        return
    ...
    async with get_session() as session:
        ...
        if success:
            ...
            if status == "COMPLETED":
                # Fatigue-aware pick: how many parts has this person
                # already studied today (including the one just now)? --
                # see FatigueAwareSelector.
                parts_today = await count_user_parts_today(session, user.id)
                popup_text, _tier = await fatigue_selector.get_message(
                    session, user.id, parts_today, minutes=recorded_minutes
                )
            else:
                popup_text = await message_selector.get_message(
                    session, status, user.id, minutes=recorded_minutes
                )
            await callback.answer(popup_text or "✅ ثبت شد!", show_alert=bool(popup_text))

            if status != "COMPLETED":
                # An explicit "🔴 انجام ندادم" tap just updated miss_streak/
                # completion_rate/consistency_score via report_result ->
                # BehaviorAnalyzer.update_profile_from_result above. Check
                # right now whether that crosses into real, sustained
                # laziness (not just this one miss) -- see
                # LazinessAssessor / maybe_send_confrontation.
                profile = await ProfileRepository(session).get_by_user_id(user.id)
                if profile:
                    await maybe_send_confrontation(session, user, profile)
...
@router.message(UserStates.REPORT_MANUAL_DURATION, ~F.text.startswith("/"))
    ...
    async with get_session() as session:
        ...
        if success:
            ...
            if capped_at is not None:
                ...
                # Same fatigue-aware pool as the button taps (see
                # cb_result) -- manual entry deserves the same
                # tier-appropriate message, it's just a short-lived
                # message here instead of a popup since there's no
                # callback to answer.
                parts_today = await count_user_parts_today(session, user.id)
                popup_text, _tier = await fatigue_selector.get_message(
                    session, user.id, parts_today, minutes=recorded_minutes or typed_minutes
                )
                if popup_text:
...
async def cb_toggle_plan_part(callback: CallbackQuery):
    ...
    # Stay on the page the tapped part is on, instead of jumping the
    # paginated keyboard back to page 1 after every check-off.
    tapped = next((p for p in plan_parts if p.id == part_id), None)
    page = (tapped.part_index // max(1, config.PLAN_KEYBOARD_PAGE_SIZE)) if tapped else 0
    # BUGFIX: the DB toggle above has already been committed by this point
    # -- the checkbox state IS flipped. But this used to re-raise any
    # TelegramBadRequest other than "message is not modified" (e.g. the
    # plan message being too old/deleted/inaccessible), which skipped the
    # callback.answer() call below entirely. Telegram then shows the user
    # *nothing at all*: no popup, no checkmark change, just the tap
    # spinner quietly timing out -- exactly as if the tap had done
    # nothing, even though it had actually been saved. Now any edit
    # failure is just logged, and the callback is always answered so the
    # user gets feedback (and can refresh the plan with 🆕 برنامه جدید or
    # /plan if the on-screen keyboard itself is stuck looking stale).
    try:
        await callback.message.edit_reply_markup(reply_markup=plan_keyboard(plan_parts, page=page))
    except TelegramBadRequest as e:
        if "message is not modified" not in str(e):
            logger.warning(f"cb_toggle_plan_part: could not refresh plan keyboard: {e}")
    await callback.answer("✅ به‌عنوان انجام‌شده ثبت شد!" if is_done else "⬜ به‌عنوان انجام‌نشده علامت خورد.")


@router.callback_query(F.data.startswith("planpage:"))
async def cb_plan_page(callback: CallbackQuery):
    """◀️ قبلی / بعدی ▶️ on a paginated plan keyboard (see plan_keyboard).
    Callback data is `planpage:{plan_id}:{page}`; `planpage:noop` is the
    non-clickable "📄 2/3" indicator between the arrows."""
    bits = callback.data.split(":")
    if len(bits) != 3:  # planpage:noop (or anything malformed)
        await callback.answer()
        return
    try:
        plan_id, page = int(bits[1]), int(bits[2])
    except ValueError:
        await callback.answer()
        return
    async with get_session() as session:
        ...
        if not user:
            await callback.answer("❌ لطفاً ابتدا /start را بزنید.", show_alert=True)
            return
        plan, plan_parts = await PlanRepository(session).get_today_plan_with_parts(user.id)
    # Same protection as the ✅/⬜ toggle: in a group, only the plan's own
    # owner can flip pages (someone else would otherwise swap the shared
    # message's keyboard for their own plan), and a page button on an
    # outdated plan message (after 🆕 برنامه جدید) does nothing harmful.
    if not plan or plan.id != plan_id or not plan_parts:
        await callback.answer("❌ این برنامه مال شما نیست یا قدیمی شده (شاید برنامه‌ی جدیدی ساخته شده).", show_alert=True)
        return
    try:
        await callback.message.edit_reply_markup(reply_markup=plan_keyboard(plan_parts, page=page))
    except TelegramBadRequest as e:
        if "message is not modified" not in str(e):
            logger.warning(f"cb_plan_page: could not refresh plan keyboard: {e}")
    await callback.answer()


async def _get_plan_start_offset(telegram_user_id: int) -> int:
    """Minutes from now the '🆕 برنامه جدید' plan should start (the
    person's ⚙️ setting, default 10)."""
    offset = 10
    async with get_session() as session:
        repo = UserRepository(session)
        user = await repo.get_by_telegram_id(telegram_user_id)
...
async def cb_plan_new(callback: CallbackQuery):
    ...
    If personal parts are queued ("📤 ارسال پارت شخصی") or one is running,
    _send_personalized_plan first shows a warning and waits for a
    confirmation (see cb_plan_replace) instead of building right away."""
    offset = await _get_plan_start_offset(callback.from_user.id)
    built = await _send_personalized_plan(callback.from_user.id, callback.message, start_in_minutes=offset)
    if built:
...
@router.callback_query(F.data.startswith("plan:replace:"))
async def cb_plan_replace(callback: CallbackQuery):
    """The two buttons under the warning from _ask_replace_plan_confirmation.
    `keep` leaves everything untouched. `new` / `auto` cancel the queued
    parts and any running personal part, then build the plan the person
    originally asked for (🆕 with their start offset, or the plain one)."""
    mode = callback.data.split(":")[-1]
    if mode == "keep":
        try:
            await callback.message.edit_text(
                "👍 باشه — برنامه‌ی فعلی و پارت‌هاش دست‌نخورده موندن.", reply_markup=None
            )
        except TelegramBadRequest as e:
            if "message is not modified" not in str(e):
                logger.debug(f"cb_plan_replace: could not edit keep prompt: {e}")
        await callback.answer()
        return
    if mode not in ("new", "auto"):
        await callback.answer()
        return

    info = await _cancel_personal_run(callback.from_user.id)
    done_lines = ["✅ <b>لغو شد:</b>"]
    if info["running"]:
        done_lines.append("• پارتِ در حال اجرا (مطالعه‌ش ثبت نشد)")
    if info["pending"]:
        done_lines.append(f"• {info['pending']} پارتِ ارسال‌نشده")
    if not info["running"] and not info["pending"]:
        done_lines = ["ℹ️ چیزی برای لغو نمونده بود."]
    # Rewriting the prompt also removes its buttons, so a second tap can't
    # run this twice.
    try:
        await callback.message.edit_text("\n".join(done_lines), reply_markup=None)
    except TelegramBadRequest as e:
        if "message is not modified" not in str(e):
            logger.debug(f"cb_plan_replace: could not edit confirm prompt: {e}")

    offset = await _get_plan_start_offset(callback.from_user.id) if mode == "new" else None
    await _send_personalized_plan(
        callback.from_user.id, callback.message, start_in_minutes=offset, confirmed=True
    )
    await callback.answer(
        f"🆕 برنامه جدید از {offset} دقیقه دیگه شروع می‌شه." if offset is not None else "🗓 برنامه ساخته شد."
    )


PLAN_ARM_SUMMARY_MAX_LINES = 12


def _build_arm_summary(out: Dict[str, Any]) -> str:
    """The message sent right after '📤 ارسال پارت شخصی': exactly which
    parts will arrive, at what time, under what title -- straight from
    the plan rows, so what's promised here is what actually fires."""
    queued: List[Dict[str, Any]] = out["queued"]
    past: List[Dict[str, Any]] = out["past"]
    if not queued:
        if out["already"]:
            return "✅ ارسال پارت‌های شخصی از قبل فعاله — همه‌ی پارت‌های باقی‌مونده توی صف هستن."
        lines = [
            "⛔ پارتی نمونده که ارسال بشه.",
            "پارت‌های امروز یا ساعتشون گذشته، یا قبلاً انجام/ارسال شدن.",
        ]
        if past:
            lines.append(f"(ساعتِ {len(past)} پارت گذشته بود.)")
        lines.append("")
        lines.append("اگه می‌خوای از همین الان شروع کنی، «🆕 برنامه جدید» بگیر.")
        return "\n".join(lines)

    lines = [
        "📤 <b>ارسال پارت‌های شخصی فعال شد</b>",
        "",
        "هر پارت دقیقاً سر ساعتی که توی برنامه‌ی شخصیت نوشته شده، با همون عنوان و مدت، "
        "همین‌جا برات فرستاده می‌شه:",
        "",
    ]
    for e in queued[:PLAN_ARM_SUMMARY_MAX_LINES]:
        tag = " 🌙" if e["next_day"] else ""
        lines.append(f"{e['number']}. 🕐 {e['label']}{tag} · {e['minutes']}د — {html.escape(e['title'])}")
    if len(queued) > PLAN_ARM_SUMMARY_MAX_LINES:
        rest = queued[PLAN_ARM_SUMMARY_MAX_LINES:]
        lines.append(f"… و {len(rest)} پارت دیگه (تا ساعت {rest[-1]['label']})")
    if past:
        nums = "، ".join(str(e["number"]) for e in past[:10]) + ("…" if len(past) > 10 else "")
        lines.append("")
        lines.append(f"⏭ ساعتِ پارت {nums} گذشته بود؛ اونا ارسال نمی‌شن.")
    lines.append("")
    lines.append(
        "⚠️ اگه «🆕 برنامه جدید» بگیری، پارت‌های ارسال‌نشده لغو می‌شن و اگه اون لحظه وسط یه پارت باشی، "
        "اون هم لغو می‌شه — قبلش بهت هشدار می‌دم و تأیید می‌گیرم."
    )
    lines.append("برای توقف: «⏹ توقف ارسال» زیر پیام برنامه.")
    return "\n".join(lines)


def _parse_plan_part_callback(data: str) -> Tuple[Optional[int], int]:
    """`planpart:send|stop:{plan_id|m}:{page}` -> (plan_id or None for the
    main-menu button, page)."""
    bits = data.split(":")
    plan_id: Optional[int] = None
    if len(bits) > 2 and bits[2] != "m":
        plan_id = int(bits[2])
    page = int(bits[3]) if len(bits) > 3 else 0
    return plan_id, page


@router.callback_query(F.data.startswith("planpart:send:"))
async def cb_plan_send_parts(callback: CallbackQuery):
    """'📤 ارسال پارت شخصی' (under the plan, or in the main menu): queues
    today's plan parts for delivery at their planned times -- see
    _arm_plan_parts / _plan_part_fire. PV only: the parts are delivered
    to the person's own chat, same as /session's personal parts."""
    if callback.message.chat.type != "private":
        await callback.answer(
            "پارت‌های شخصی توی چت خصوصی با ربات فرستاده می‌شن — همین دکمه رو اونجا بزن.",
            show_alert=True,
        )
        return
    try:
        plan_id, page = _parse_plan_part_callback(callback.data)
    except ValueError:
        await callback.answer()
        return

    out = await _arm_plan_parts(callback.from_user.id, plan_id)
    status = out["status"]
    if status == "no_user":
        await callback.answer("❌ لطفاً ابتدا /start را بزنید.", show_alert=True)
        return
    if status == "no_plan":
        kb = InlineKeyboardBuilder()
        kb.row(InlineKeyboardButton(text="🗓 ساخت برنامه امروز", callback_data="menu:plan"))
        await callback.message.answer(
            "🗓 هنوز برنامه‌ی امروزت ساخته نشده. اول برنامه رو بساز، بعد «📤 ارسال پارت شخصی» رو بزن — "
            "پارت‌ها از روی همون برنامه فرستاده می‌شن.",
    ...
    if status == "stale":
        await callback.answer(
            "❌ این برنامه قدیمی شده (شاید برنامه‌ی جدیدی ساخته شده). از برنامه‌ی جدید بزن.",
            show_alert=True,
        )
        return

    if plan_id is not None and out["parts"]:
        # Flip the button under the plan to "⏹ توقف ارسال" in place.
        try:
            await callback.message.edit_reply_markup(reply_markup=plan_keyboard(out["parts"], page=page))
        except TelegramBadRequest as e:
            if "message is not modified" not in str(e):
                logger.warning(f"cb_plan_send_parts: could not refresh plan keyboard: {e}")
    await callback.message.answer(_build_arm_summary(out))
    await callback.answer("📤 ارسال پارت‌ها فعال شد." if out["queued"] else None)


@router.callback_query(F.data.startswith("planpart:stop:"))
async def cb_plan_stop_parts(callback: CallbackQuery):
    """'⏹ توقف ارسال' -- cancels every part still waiting to be sent. A
    part that's already running is left alone; only 🆕 برنامه جدید
    (after its warning) cancels a running one."""
    if callback.message.chat.type != "private":
        await callback.answer("این دکمه فقط توی چت خصوصی با ربات کار می‌کنه.", show_alert=True)
        return
    try:
        _plan_id, page = _parse_plan_part_callback(callback.data)
    except ValueError:
        await callback.answer()
        return
    outcome = await _disarm_plan_parts(callback.from_user.id)
    if outcome is None:
        await callback.answer("❌ لطفاً ابتدا /start را بزنید.", show_alert=True)
        return
    cancelled, parts = outcome
    if parts:
        try:
            await callback.message.edit_reply_markup(reply_markup=plan_keyboard(parts, page=page))
        except TelegramBadRequest as e:
            if "message is not modified" not in str(e):
                logger.warning(f"cb_plan_stop_parts: could not refresh plan keyboard: {e}")
    if not cancelled:
        await callback.answer("پارتی توی صف نبود.", show_alert=True)
        return
    await callback.message.answer(
        f"⏹ ارسال پارت‌های شخصی متوقف شد؛ {cancelled} پارتِ ارسال‌نشده لغو شد.\n"
        f"اگه پارتی همین الان بازه، دست‌نخورده موند. هر وقت خواستی دوباره «📤 ارسال پارت شخصی» رو بزن."
    )
    await callback.answer("⏹ متوقف شد.")


@router.callback_query(F.data == "tl:snooze")
async def cb_confrontation_snooze(callback: CallbackQuery):
    """"⏸ چند روز نپرس" under a confrontation: the person tells the bot
    there's a real reason (sick, travelling, exams over...) instead of
    being scolded for it. Counts as a response, so it also resets the
    ignored-confrontation streak."""
    days = config.CONFRONTATION_SNOOZE_DAYS
    async with get_session() as session:
        user = await UserRepository(session).get_by_telegram_id(callback.from_user.id)
        profile = await ProfileRepository(session).get_by_user_id(user.id) if user else None
        if not profile:
            await callback.answer("❌ لطفاً ابتدا /start را بزنید.", show_alert=True)
            return
        profile.confrontation_snooze_until = _utc_now() + timedelta(days=days)
        profile.confrontation_streak = 0
    try:
        await callback.message.edit_reply_markup(reply_markup=None)
    except TelegramBadRequest as e:
        if "message is not modified" not in str(e):
            logger.debug(f"cb_confrontation_snooze: could not clear keyboard: {e}")
    await callback.answer(f"⏸ باشه، {days} روز نمی‌پرسم. بعدش دوباره سر می‌زنم.", show_alert=True)
...
async def cb_menu_reports(callback: CallbackQuery):
    async with get_session() as session:
        ...
        try:
            await callback.message.edit_text(
                msg,
                reply_markup=main_menu(show_personal_session=callback.message.chat.type == "private"),
            )
        except TelegramBadRequest as e:
...
@router.message(UserStates.SET_DAILY_GOAL, ~F.text.startswith("/"))
...
async def cb_menu_back(callback: CallbackQuery):
    ...
    await callback.message.edit_text("📋 منوی اصلی:", reply_markup=main_menu(is_admin, show_personal_session=callback.message.chat.type == "private"))
    await callback.answer()
...
@router.message(UserStates.SET_PREFERRED_DURATION, ~F.text.startswith("/"))
...
@router.message(UserStates.SET_PLAN_GAP, ~F.text.startswith("/"))
...
@router.message(UserStates.SET_PLAN_START_OFFSET, ~F.text.startswith("/"))
...
async def cb_settings_delivery(callback: CallbackQuery):
    async with get_session() as session:
        ...
        await callback.message.edit_text(
            ...
            "⚠️ اگه «گروه» یا «هر دو» رو انتخاب کنی، همین برنامه با اسم/تگ خودت توی همه‌ی "
            "گروه‌های مطالعه‌ای که عضوشونی هم فرستاده می‌شه و بقیه‌ی اعضا هم می‌بیننش.",
...
def _parse_message_import_payload(payload: Any) -> Tuple[List[Dict[str, Any]], List[str]]:
    """Turn an uploaded JSON payload into a flat list of message entries
    ready for MessageRepository.create_message, plus a list of
    human-readable problems found along the way (bad items are skipped,
    not fatal -- the rest of the file still gets imported).

    Two accepted shapes:
      1) {"CATEGORY_CODE": ["text 1", "text 2", ...], ...} -- same shape as
         SAMPLE_MESSAGES, so it's already a format the admin has seen
         (via 🧪 افزودن نمونه‌های آماده). List items can also be objects
         ({"text": ..., "weight": ...}) for a per-message weight.
      2) [{"category": "CODE", "text": "...", "weight": 1.0,
           "subcategory": "..."}, ...] -- one object per message, for when
         messages need mixed categories/weights in one flat list.
    """
    entries: List[Dict[str, Any]] = []
    errors: List[str] = []

    def _add(category_raw: Any, item: Any, fallback_category: Optional[str] = None):
        category = str(category_raw if category_raw not in (None, "") else fallback_category or "").strip().upper()
        if not category:
            errors.append("آیتمی بدون category نادیده گرفته شد.")
            return
        if isinstance(item, str):
            text, weight, subcategory = item.strip(), 1.0, None
            ...
            errors.append(f"آیتم نامعتبر در دسته «{category}» (باید متن یا شیء باشه).")
            return
        if not text:
            errors.append(f"متن خالی در دسته «{category}» نادیده گرفته شد.")
            return
        entries.append({"category": category, "text": text, "weight": weight, "subcategory": subcategory})

    if isinstance(payload, dict):
        for category, items in payload.items():
            if not isinstance(items, list):
                errors.append(f"مقدار «{category}» باید یک لیست از پیام‌ها باشه.")
                continue
            for item in items:
                _add(item.get("category") if isinstance(item, dict) else None, item, fallback_category=category)
    elif isinstance(payload, list):
        for item in payload:
            if not isinstance(item, dict):
                errors.append("هر آیتم توی لیست باید یک شیء (object) با category و text باشه.")
                continue
            _add(item.get("category"), item)
    else:
        errors.append("ساختار فایل باید یک شیء JSON (دسته‌بندی → لیست پیام) یا یک لیست از پیام‌ها باشه.")

    return entries, errors


@router.callback_query(F.data == "admin:msg:json")
async def cb_admin_msg_json(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    await state.set_state(AdminStates.MSG_JSON_UPLOAD)
    category_codes = ", ".join(code for code, _, _ in MESSAGE_CATEGORIES)
    await callback.message.edit_text(
        "📥 آپلود گروهی پیام از فایل JSON\n\n"
        "فایل رو با یکی از این دو ساختار بفرست:\n\n"
        "۱) بر اساس دسته‌بندی (مثل نمونه‌های آماده):\n"
        "<code>{\"MOTIVATION\": [\"متن ۱\", \"متن ۲\"], \"STORY\": [\"متن ۳\"]}</code>\n\n"
        "۲) لیست تخت، وقتی هر پیام وزن/دسته‌بندی جدا می‌خواد:\n"
        "<code>[{\"category\": \"MOTIVATION\", \"text\": \"متن ۱\", \"weight\": 1.0}]</code>\n\n"
        f"کدهای دسته‌بندی: {category_codes}\n\n"
        "حالا فایل JSON رو همینجا ارسال کن:",
        reply_markup=cancel_keyboard("messages"),
    )
    await callback.answer()


@router.message(AdminStates.MSG_JSON_UPLOAD, F.document)
async def admin_msg_json_received(message: Message, state: FSMContext):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    doc = message.document
    if not doc.file_name or not doc.file_name.lower().endswith(".json"):
        await message.answer("❌ فقط فایل JSON پذیرفته می‌شود.")
        return
    tmp_dir = Path("./tmp_uploads")
    tmp_dir.mkdir(parents=True, exist_ok=True)
    dest = tmp_dir / f"msgimport_{int(time.time())}.json"
    file = await bot.get_file(doc.file_id)
    await bot.download_file(file.file_path, destination=str(dest))
    try:
        payload = json.loads(dest.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as e:
        await message.answer(f"❌ فایل JSON قابل خواندن نبود: {e}")
        dest.unlink(missing_ok=True)
        return
    dest.unlink(missing_ok=True)

    entries, errors = _parse_message_import_payload(payload)
    if not entries:
        text = "❌ هیچ پیام معتبری توی فایل پیدا نشد."
        if errors:
            text += "\n\n" + "\n".join(f"• {e}" for e in errors[:10])
        await message.answer(text, reply_markup=message_management())
        await state.clear()
        return

    unknown_categories = sorted({e["category"] for e in entries if e["category"] not in MESSAGE_CATEGORY_LABELS})
    async with get_session() as session:
        repo = MessageRepository(session)
        for e in entries:
            await repo.create_message(
                e["category"], e["text"], created_by=message.from_user.id,
                weight=e.get("weight", 1.0), subcategory=e.get("subcategory"),
            )
        await session.commit()
    await state.clear()

    summary = f"✅ {len(entries)} پیام از فایل JSON اضافه شد!"
    if errors:
        summary += f"\n⚠️ {len(errors)} مورد نامعتبر رد شد (بقیه اضافه شدن)."
    if unknown_categories:
        summary += f"\n⚠️ دسته‌بندی ناشناخته ولی ذخیره‌شده: {', '.join(unknown_categories)}"
    await message.answer(summary, reply_markup=message_management())


@router.message(AdminStates.MSG_JSON_UPLOAD)
async def admin_msg_json_invalid(message: Message):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    await message.answer("❌ لطفاً یک فایل JSON بفرست (نه متن).")


@router.callback_query(F.data == "admin:msg:bulkadd")
async def cb_admin_msg_bulkadd(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    await callback.message.edit_text(
        "📤 اول دسته‌بندی مقصد رو انتخاب کن:",
        reply_markup=category_picker_keyboard(prefix="admin:msg:bulkpick", back_callback="admin:messages"),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("admin:msg:bulkpick:"))
async def cb_admin_msg_bulkpick(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    category = callback.data.split(":")[-1]
    await state.update_data(bulk_category=category, bulk_texts=[])
    await state.set_state(AdminStates.BULK_ADD_MESSAGES)
    label = MESSAGE_CATEGORY_LABELS.get(category, category)
    await callback.message.edit_text(
        f"📤 افزودن گروهی به «{label}»\n\n"
        "پیام‌ها رو یکی‌یکی بفرست -- هر پیامی که بفرستی توی صف اضافه‌شدن قرار می‌گیره "
        "(هنوز ذخیره نمی‌شه). وقتی همه‌شون رو فرستادی، «✅ پایان» رو بزن تا همه با هم "
        "به این دسته اضافه شن.",
        reply_markup=bulk_add_prompt_keyboard(0),
    )
    await callback.answer()


@router.message(AdminStates.BULK_ADD_MESSAGES)
async def admin_bulk_add_message_received(message: Message, state: FSMContext):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    text = (message.text or "").strip()
    if not text:
        await message.answer("❌ متن پیام نمی‌تونه خالی باشه. یه متن بفرست یا «پایان» رو بزن.")
        return
    data = await state.get_data()
    texts: List[str] = data.get("bulk_texts", [])
    texts.append(text)
    await state.update_data(bulk_texts=texts)
    label = MESSAGE_CATEGORY_LABELS.get(data.get("bulk_category"), data.get("bulk_category"))
    await message.answer(
        f"✅ به صف اضافه شد ({len(texts)} پیام آماده‌ی «{label}»).\nپیام بعدی رو بفرست یا «پایان» رو بزن.",
        reply_markup=bulk_add_prompt_keyboard(len(texts)),
    )


@router.callback_query(F.data == "admin:msg:bulkdone")
async def cb_admin_msg_bulkdone(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    data = await state.get_data()
    ...
    if not category or not texts:
        await callback.message.edit_text(
            "چیزی توی صف نبود، پیامی اضافه نشد.", reply_markup=message_management()
        )
        await callback.answer()
        return
    async with get_session() as session:
        ...
        await session.commit()
    label = MESSAGE_CATEGORY_LABELS.get(category, category)
    await callback.message.edit_text(f"✅ {len(texts)} پیام به «{label}» اضافه شد!", reply_markup=message_management())
    await callback.answer()
...
async def cb_admin_msg_categories(callback: CallbackQuery):
    ...
    async with get_session() as session:
        ...
        kb_markup = category_picker_keyboard(
            prefix="admin:msg:browse", back_callback="admin:messages", counts=counts
        )
        # Wipes messages in EVERY category, not just one -- only makes sense
        # here on the overview screen, so it's appended straight onto this
        # call site's keyboard rather than built into the shared
        # category_picker_keyboard (also used for the add/bulk-add pickers,
        # where a reset button would be out of place).
        kb_markup.inline_keyboard.append(
            [InlineKeyboardButton(text="🗑 ریست همه پیام‌ها (همه دسته‌ها)", callback_data="admin:msg:resetall")]
        )
        await callback.message.edit_text(
...
@router.callback_query(F.data == "admin:msg:resetall")
async def cb_admin_msg_resetall(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    async with get_session() as session:
        total = await MessageRepository(session).count_all()
    if not total:
        await callback.answer("هیچ پیامی برای پاک کردن نیست.", show_alert=True)
        return
    await callback.message.edit_text(
        f"⚠️ همه‌ی {total} پیامِ همه‌ی دسته‌بندی‌ها حذف می‌شن و این کار برگشت‌پذیر نیست. مطمئنی؟",
        reply_markup=yes_no_keyboard("admin:msg:resetallconfirm", "admin:msg:categories"),
    )
    await callback.answer()


@router.callback_query(F.data == "admin:msg:resetallconfirm")
async def cb_admin_msg_resetallconfirm(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    async with get_session() as session:
        deleted = await MessageRepository(session).delete_all()
        await session.commit()
    await callback.message.edit_text(f"🗑 {deleted} پیام حذف شد.", reply_markup=message_management())
    await callback.answer()
...
async def _render_msg_category_browse(callback: CallbackQuery, category: str):
    ...
    async with get_session() as session:
        ...
        kb.row(
            InlineKeyboardButton(text="➕ افزودن به این دسته", callback_data=f"admin:msg:pick:{category}"),
            InlineKeyboardButton(text="📤 افزودن گروهی", callback_data=f"admin:msg:bulkpick:{category}"),
        )
        if messages:
            kb.row(
                InlineKeyboardButton(text="🗑 حذف گروهی", callback_data=f"admin:msg:bulkdel:{category}"),
                InlineKeyboardButton(text="♻️ ریست این دسته", callback_data=f"admin:msg:catreset:{category}"),
            )
        kb.row(InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:msg:categories"))
        await callback.message.edit_text(text, reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("admin:msg:catreset:"))
async def cb_admin_msg_catreset(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    category = callback.data.split(":")[-1]
    ...
    async with get_session() as session:
        result = await session.execute(
            select(func.count(MessageTemplate.id)).where(MessageTemplate.category == category)
        )
        total = result.scalar() or 0
    if not total:
        await callback.answer("این دسته پیامی نداره.", show_alert=True)
        return
    await callback.message.edit_text(
        f"⚠️ همه‌ی {total} پیام دسته‌ی «{label}» حذف می‌شن و این کار برگشت‌پذیر نیست. مطمئنی؟",
        reply_markup=yes_no_keyboard(f"admin:msg:catresetconfirm:{category}", f"admin:msg:browse:{category}"),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("admin:msg:catresetconfirm:"))
async def cb_admin_msg_catresetconfirm(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    category = callback.data.split(":")[-1]
    async with get_session() as session:
        deleted = await MessageRepository(session).delete_by_category(category)
        await session.commit()
    await callback.answer(f"🗑 {deleted} پیام حذف شد")
    await _render_msg_category_browse(callback, category)


# ------------------------------------------------------------
# Bulk-delete (select several messages, then delete them together).
# Selection lives in FSMContext.update_data (no dedicated AdminStates
# entry needed -- nothing here reads text input, it's callback-only), keyed
# under bulkdel_category / bulkdel_ids so re-rendering after every ☑️ tap
# just reads it back. Cleared explicitly on both cancel and confirm so a
# stale selection never leaks into the next bulk-delete session.
# ------------------------------------------------------------
async def _render_msg_bulk_delete_select(callback: CallbackQuery, state: FSMContext, category: str):
    data = await state.get_data()
    selected: Set[int] = set(data.get("bulkdel_ids", [])) if data.get("bulkdel_category") == category else set()
    await state.update_data(bulkdel_category=category, bulkdel_ids=list(selected))
    label = MESSAGE_CATEGORY_LABELS.get(category, category)
    async with get_session() as session:
        result = await session.execute(
            select(MessageTemplate).where(MessageTemplate.category == category).order_by(MessageTemplate.id)
    ...
    kb = InlineKeyboardBuilder()
    for msg in messages[:30]:
        mark = "☑️" if msg.id in selected else "⬜️"
        preview = msg.text if len(msg.text) <= 28 else msg.text[:28] + "…"
        kb.row(InlineKeyboardButton(text=f"{mark} {preview}", callback_data=f"admin:msg:bdtoggle:{msg.id}"))
    kb.row(InlineKeyboardButton(text=f"🗑 حذف انتخاب‌شده‌ها ({len(selected)})", callback_data="admin:msg:bdconfirm"))
    kb.row(InlineKeyboardButton(text="🔙 انصراف", callback_data="admin:msg:bdcancel"))
    text = (
        f"🗑 حذف گروهی از «{label}»\n\nروی پیام‌هایی که می‌خوای حذف کنی بزن تا ☑️ بشن، "
        "هر تعدادی که می‌خوای، بعد «حذف انتخاب‌شده‌ها» رو بزن."
    )
    await callback.message.edit_text(text, reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("admin:msg:bulkdel:"))
async def cb_admin_msg_bulkdel(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    category = callback.data.split(":")[-1]
    await state.update_data(bulkdel_category=category, bulkdel_ids=[])
    await _render_msg_bulk_delete_select(callback, state, category)
    await callback.answer()


@router.callback_query(F.data.startswith("admin:msg:bdtoggle:"))
async def cb_admin_msg_bdtoggle(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    msg_id = int(callback.data.split(":")[-1])
    data = await state.get_data()
    category = data.get("bulkdel_category")
    if not category:
        await callback.answer("❌ حالت انتخاب منقضی شده، دوباره از «حذف گروهی» شروع کن.", show_alert=True)
        return
    selected: Set[int] = set(data.get("bulkdel_ids", []))
    if msg_id in selected:
        selected.discard(msg_id)
    else:
        selected.add(msg_id)
    await state.update_data(bulkdel_ids=list(selected))
    await _render_msg_bulk_delete_select(callback, state, category)
    await callback.answer()


@router.callback_query(F.data == "admin:msg:bdcancel")
async def cb_admin_msg_bdcancel(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    data = await state.get_data()
    category = data.get("bulkdel_category")
    await state.update_data(bulkdel_category=None, bulkdel_ids=[])
    await callback.answer()
    if category:
        await _render_msg_category_browse(callback, category)
    else:
...
@router.callback_query(F.data == "admin:msg:bdconfirm")
async def cb_admin_msg_bdconfirm(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    data = await state.get_data()
    category = data.get("bulkdel_category")
    selected: List[int] = data.get("bulkdel_ids", [])
    if not category or not selected:
        await callback.answer("چیزی انتخاب نشده.", show_alert=True)
        return
    await callback.message.edit_text(
        f"⚠️ {len(selected)} پیام انتخاب‌شده حذف بشن؟ این کار برگشت‌پذیر نیست.",
        reply_markup=yes_no_keyboard("admin:msg:bddelete", "admin:msg:bdcancel"),
    )
    await callback.answer()


@router.callback_query(F.data == "admin:msg:bddelete")
async def cb_admin_msg_bddelete(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    data = await state.get_data()
    ...
    if not category or not selected:
        ...
        return
    async with get_session() as session:
        deleted = await MessageRepository(session).delete_many(selected)
        await session.commit()
    await callback.answer(f"🗑 {deleted} پیام حذف شد")
    await _render_msg_category_browse(callback, category)
...
async def seed_sample_messages(session, created_by: Optional[int] = None) -> int:
    """Idempotently backfills any category in SAMPLE_MESSAGES that has zero
    rows (active or not) with its ready-made samples. Shared by the manual
    '🧪 افزودن نمونه‌های آماده' admin action and by on_startup, so the STORY
    (and any other) samples are always present without an admin having to
    remember to tap seed -- a category that already has messages (including
    ones the admin edited/deleted down to fewer than the originals) is left
    alone, so this never duplicates or resurrects deleted content."""
    repo = MessageRepository(session)
    added = 0
    for category, samples in SAMPLE_MESSAGES.items():
        ...
        if existing:
            continue
        for text in samples:
            await repo.create_message(category, text, created_by=created_by)
            added += 1
    return added
...
async def cb_admin_msg_seed(callback: CallbackQuery):
    if callback.from_user.id not in config.ADMIN_IDS:
        ...
        return
    async with get_session() as session:
        added = await seed_sample_messages(session, created_by=callback.from_user.id)
        await session.commit()
        if added:
            await callback.message.edit_text(
...
@router.callback_query(F.data == "admin:restore:merge")
async def cb_admin_restore_merge(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in config.ADMIN_IDS:
        await callback.answer("⛔ دسترسی غیرمجاز!")
        return
    await state.clear()
    await callback.message.edit_text(
        "🔀 ادغام دو بک‌آپ:\n\n"
        "وقتی دو بک‌آپ مختلف دارید که هرکدوم ممکنه بخشی از اطلاعات رو داشته "
        "باشن و بخشی رو نه، این گزینه هر دو رو با هم ادغام می‌کنه: برای هر "
        "رکورد، فیلدهای خالی از یک فایل با مقدار موجود در فایل دیگه پر "
        "می‌شن. نتیجه یه فایل جدید روی سرور می‌شه که می‌تونید قبل از "
        "بازیابی، خلاصه‌ش رو ببینید.\n\n"
        "فایل پشتیبان اول (JSON) را ارسال کنید:",
        reply_markup=cancel_keyboard("restore"),
    )
    await state.set_state(AdminStates.MERGE_UPLOAD_FIRST)
    await callback.answer()


@router.message(AdminStates.MERGE_UPLOAD_FIRST, F.document)
async def admin_merge_upload_first_received(message: Message, state: FSMContext):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    doc = message.document
    if not doc.file_name.lower().endswith(".json"):
        ...
        return
    backup_service.backup_dir.mkdir(parents=True, exist_ok=True)
    dest = backup_service.backup_dir / f"mergesrc_{int(time.time())}_a.json"
    file = await bot.get_file(doc.file_id)
    await bot.download_file(file.file_path, destination=str(dest))
    await state.update_data(merge_file_a=str(dest))
    await state.set_state(AdminStates.MERGE_UPLOAD_SECOND)
    await message.answer(
        "✅ فایل اول دریافت شد.\n\nحالا فایل پشتیبان دوم (JSON) را ارسال کنید:",
        reply_markup=cancel_keyboard("restore"),
    )


@router.message(AdminStates.MERGE_UPLOAD_FIRST)
async def admin_merge_upload_first_invalid(message: Message):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    await message.answer("❌ لطفاً یک فایل JSON پشتیبان ارسال کنید.")


@router.message(AdminStates.MERGE_UPLOAD_SECOND, F.document)
async def admin_merge_upload_second_received(message: Message, state: FSMContext):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    doc = message.document
    if not doc.file_name.lower().endswith(".json"):
    ...
    data = await state.get_data()
    path_a = Path(data.get("merge_file_a", ""))
    if not path_a.exists():
        await state.clear()
        await message.answer("❌ فایل اول دیگر در دسترس نیست، از ابتدا شروع کنید.", reply_markup=admin_panel())
        return

    backup_service.backup_dir.mkdir(parents=True, exist_ok=True)
    path_b = backup_service.backup_dir / f"mergesrc_{int(time.time())}_b.json"
    file = await bot.get_file(doc.file_id)
    await bot.download_file(file.file_path, destination=str(path_b))

    await message.answer("⏳ در حال ادغام دو فایل...")
    merged_payload, err, stats = backup_service.merge_json_backups(path_a, path_b)

    # The two uploaded source files were only ever needed to compute the
    # merge; the merged result below is what actually matters going
    # forward, so don't leave copies of the raw sources cluttering the
    # backups dir (and don't let them get mistaken for real backups).
    ...
    if merged_payload is None:
        await message.answer(f"❌ ادغام ناموفق بود: {err}", reply_markup=admin_panel())
        return

    merged_path = backup_service.save_merged_payload(merged_payload)
    summary = (
        "✅ ادغام انجام شد.\n\n"
        f"📄 فقط در فایل اول: {stats['only_first']} رکورد\n"
        f"📄 فقط در فایل دوم: {stats['only_second']} رکورد\n"
        f"🔗 رکوردهای مشترک (ادغام‌شده): {stats['combined']} رکورد "
        f"({stats['fields_filled']} مورد با فیلد پرشده از فایل دیگر)\n"
        f"📦 مجموع رکوردهای فایل نهایی: {stats['total_rows']}\n\n"
        f"فایل ادغام‌شده روی سرور ذخیره شد: {merged_path.name}\n\n"
        "⚠️ توجه: با تایید بازیابی، تمام داده‌های فعلی دیتابیس با محتوای "
        "این فایل ادغام‌شده جایگزین می‌شود و غیرقابل بازگشت است."
    )
    try:
        await message.answer_document(
            document=FSInputFile(str(merged_path), filename=merged_path.name),
            caption="📦 فایل ادغام‌شده (برای بررسی/نگهداری)",
        )
    except Exception as e:
        logger.error(f"Failed to send merged backup file: {e}")
    await message.answer(summary, reply_markup=restore_confirm_keyboard(merged_path.name))


@router.message(AdminStates.MERGE_UPLOAD_SECOND)
async def admin_merge_upload_second_invalid(message: Message):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    await message.answer("❌ لطفاً یک فایل JSON پشتیبان ارسال کنید.")
...
async def recover_from_crash():
    ...
    stale_cutoff = timedelta(minutes=config.RECOVERY_STALE_THRESHOLD_MINUTES)

    def _is_stale(reference_dt: Optional[datetime]) -> bool:
        """True when `reference_dt` is far enough in the past that replaying
        the normal flow would mean resurrecting a part from a different
        point in time (e.g. right after restoring an old JSON backup)
        rather than genuinely catching up after a brief restart."""
        ref_naive = _naive(reference_dt)
        if ref_naive is None:
            return False
        return (now_naive - ref_naive) > stale_cutoff

    async def _quietly_close_stale_session(session_id: int, fallback_status: str):
        """Closes out a stale session without touching the group -- no
        re-announcement, no report button, nothing for members to tap."""
        async with get_session() as db_session:
            s = await SessionManager(db_session).get_session(session_id)
            if s and s.status in ("ATTENDANCE", "READY", "STARTED"):
                s.status = fallback_status
    ...
    overdue_job_ids, restored, stale_closed = [], 0, 0
    for job in pending_jobs:
        ...
        if run_at_naive is not None and run_at_naive <= now_naive:
            if job.job_type in ("attendance_timeout", "session_start", "session_end") and _is_stale(job.run_at):
                # Belongs to a different point in time (typically a
                # just-restored backup) -- close it out quietly instead of
                # running the normal flow, which would re-announce a
                # long-dead part in the group.
                fallback = "MISSED" if job.job_type == "attendance_timeout" else "CANCELLED"
                try:
                    await _quietly_close_stale_session(key_id, fallback)
                    stale_closed += 1
                except Exception as e:
                    logger.error(f"Crash recovery: failed to quietly close stale job {job.job_id}: {e}")
            else:
                try:
                    await func(key_id)
                except Exception as e:
            ...
            restored += 1
            # The 5-second countdown ticks aren't persisted (see
            # _start_countdown_job) -- a still-pending attendance_timeout
            # or session_end job being restored means the matching poll/
            # roster message is presumably still live in the group, so
            # re-arm its countdown too. Each tick re-checks the session's
            # actual status/message id and self-stops if either is gone,
            # so this is safe even if the message was deleted while the
            # bot was down.
            if job.job_type == "attendance_timeout":
                _start_countdown_job("attend", key_id)
            elif job.job_type == "session_end":
                _start_countdown_job("start", key_id)

    for sess in stuck_sessions:
        try:
            if _is_stale(sess.scheduled_end) or _is_stale(sess.created_at):
                fallback = "MISSED" if sess.status == "ATTENDANCE" else "CANCELLED"
                await _quietly_close_stale_session(sess.id, fallback)
                stale_closed += 1
            elif sess.status == "ATTENDANCE":
    ...
    logger.info(
        ...
        f"{stale_closed} stale sessions quietly closed"
    )
...
async def _daily_personal_plan_broadcast():
    ...
    When it goes to a group, the message is tagged with the person's own
    mention so nobody has to guess whose plan it is, and carries the same
    done/not-done keyboard as the DM version -- safe to expose there
    because PlanRepository.toggle_part already scopes every toggle to
    (part_id, user_id), so anyone else in the group tapping it just gets
    "این پارت پیدا نشد", never someone else's checklist."""
    async with get_session() as session:
        result = await session.execute(select(User))
        all_users = result.scalars().all()

    for user_row in all_users:
        internal_id, telegram_id = user_row.id, user_row.telegram_id
        try:
            async with get_session() as session:
                pref_result = await session.execute(
                ...
                # Someone who already queued their plan's parts for
                # delivery ("📤 ارسال پارت شخصی") must not have that plan
                # silently replaced -- and its parts cancelled -- by the
                # morning push. Rebuilding it is their call (🆕 برنامه جدید
                # asks for confirmation first).
                queued = await session.execute(
                    select(UserPlanPart).where(
                        UserPlanPart.user_id == internal_id, UserPlanPart.send_status == "PENDING"
                    )
                )
                if queued.scalars().all():
                    logger.info(f"Daily plan skipped for user {telegram_id}: personal plan parts are queued")
                    continue
            ...
            plan_chunks = plan.get("message_chunks") or [plan["message"]]
            dm_prefix = "🌅 صبح بخیر! برنامه شخصی امروزت آماده‌ست:\n\n"
            ...
            if delivery in ("private", "both"):
                try:
                    ...
                    await _send_plan_message(
                        lambda t, **kw: bot.send_message(telegram_id, t, **kw),
                        plan_chunks, plan_keyboard(plan_parts), prefix=dm_prefix,
                    )
                except Exception as e:
                    logger.debug(f"Daily plan DM failed for user {telegram_id}: {e}")

            if delivery in ("group", "both"):
                # A plain, unlabeled personal plan dropped into a shared
                # group is ambiguous by itself (whose is this?) and, if
                # several members opted into group delivery, several of
                # these can land back-to-back with nothing to tell them
                # apart. A real mention up top fixes both: it says whose
                # plan it is, and -- since it's a real tg://user mention
                # -- it also notifies that person specifically instead of
                # relying on them noticing a generic message.
                group_prefix = f"🌅 برنامه شخصی امروز {_mention_html(user_row)}:\n\n"
                for group in groups:
                    try:
                        await _send_plan_message(
                            lambda t, _cid=group.telegram_chat_id, **kw: bot.send_message(_cid, t, **kw),
                            plan_chunks, plan_keyboard(plan_parts), prefix=group_prefix,
                        )
                    except Exception as e:
                        logger.debug(
...
async def _daily_laziness_confrontation_check() -> None:
    """Catches the case cb_result's MISSED-tap check can't: someone who
    never taps *anything* at all for days (no COMPLETED, no MISSED --
    just silence). Runs once a day (config.CONFRONTATION_CHECK_HOUR/MINUTE,
    default 21:00) over every user with a behavior profile, and routes
    each one through the exact same maybe_send_confrontation gate the
    real-time path uses -- same opt-in check, same signal threshold, same
    cooldown -- so this can never double up with a confrontation the
    MISSED-tap path already sent."""
    async with get_session() as session:
    ...
    for profile in profiles:
        try:
            async with get_session() as session:
                ...
                if not user_row:
                    continue
                # Re-fetch the profile inside this session so any change
                # (e.g. last_confrontation_at from a concurrent MISSED tap)
                # is seen fresh, not the possibly-stale copy from the
                # listing query above.
                fresh_profile = await ProfileRepository(session).get_by_user_id(profile.user_id)
                if not fresh_profile:
                    continue
                # No days_inactive computed here anymore: it used to come
                # from fresh_profile.last_updated, which any result --
                # including an explicit "انجام ندادم" -- refreshes. The gate
                # now measures real study evidence itself.
                await maybe_send_confrontation(session, user_row, fresh_profile)
        except Exception as e:
            logger.error(f"Daily laziness confrontation check failed for profile user_id={profile.user_id}: {e}")


async def reschedule_confrontation_job() -> None:
    """(Re)installs the daily inactivity-sweep cron job at
    config.CONFRONTATION_CHECK_HOUR/MINUTE. Same pattern as
    reschedule_daily_plan_job."""
    scheduler.add_job(
        _daily_laziness_confrontation_check,
        trigger=CronTrigger(
            hour=config.CONFRONTATION_CHECK_HOUR, minute=config.CONFRONTATION_CHECK_MINUTE, timezone="Asia/Tehran"
        ),
        id="daily_laziness_confrontation_check",
        replace_existing=True,
    )
    logger.info(
        f"Laziness confrontation check scheduled at "
        f"{config.CONFRONTATION_CHECK_HOUR:02d}:{config.CONFRONTATION_CHECK_MINUTE:02d}"
    )


async def _daily_activity_neglect_check() -> None:
    """Once-a-day sweep (config.ACTIVITY_NEGLECT_CHECK_HOUR/MINUTE,
    default 21:30) over every active UserActivity across every user,
    routing each one through maybe_send_activity_neglect_warning -- same
    shape as _daily_laziness_confrontation_check, just scoped to one
    subject/task at a time instead of the person's whole profile. Each
    activity gets its own DB session so one bad activity/user can't take
    the rest of the sweep down with it."""
    async with get_session() as session:
        activities = await ActivityRepository(session).get_all_active()

    for activity in activities:
        try:
            async with get_session() as session:
                user_result = await session.execute(select(User).where(User.id == activity.user_id))
                user_row = user_result.scalar_one_or_none()
                if not user_row:
                    continue
                # Re-fetch inside this session so a concurrent change
                # (e.g. last_neglect_warning_at from earlier in this same
                # sweep, or the user deactivating/editing the activity
                # mid-run) is seen fresh rather than the listing snapshot.
                fresh_activity = await ActivityRepository(session).get_by_id(activity.id, activity.user_id)
                if not fresh_activity or not fresh_activity.active:
                    continue
                await maybe_send_activity_neglect_warning(session, user_row, fresh_activity)
        except Exception as e:
            logger.error(f"Daily activity-neglect check failed for activity_id={activity.id}: {e}")


async def reschedule_activity_neglect_job() -> None:
    """(Re)installs the daily per-activity neglect sweep cron job at
    config.ACTIVITY_NEGLECT_CHECK_HOUR/MINUTE. Same pattern as
    reschedule_confrontation_job."""
    scheduler.add_job(
        _daily_activity_neglect_check,
        trigger=CronTrigger(
            hour=config.ACTIVITY_NEGLECT_CHECK_HOUR,
            minute=config.ACTIVITY_NEGLECT_CHECK_MINUTE,
            timezone="Asia/Tehran",
        ),
        id="daily_activity_neglect_check",
        replace_existing=True,
    )
    logger.info(
        f"Activity-neglect check scheduled at "
        f"{config.ACTIVITY_NEGLECT_CHECK_HOUR:02d}:{config.ACTIVITY_NEGLECT_CHECK_MINUTE:02d}"
    )


async def _register_bot_commands() -> None:
    """Populates Telegram's own ☰ command menu (next to the text box) --
    previously never set, so it was empty for everyone. Unlike the "🛠 پنل
    مدیریت" inline button on /start, this menu is a fixed part of the chat
    UI: it doesn't scroll away or get buried once other messages come in,
    which is exactly what was reported ("غیب می‌شه" -- the admin button
    disappearing after using other features).

    Every user gets /start in that menu. Each id in config.ADMIN_IDS gets
    an extra per-chat command set (via BotCommandScopeChat) that also
    includes /panel -- Telegram scopes commands per chat_id, not per
    "role", so this has to be set once for every known admin individually
    rather than as a single global rule. A failure for one admin_id (e.g.
    they've never opened a DM with the bot, so Telegram has no chat to
    scope commands to yet) must not stop the default menu or the other
    admins' menus from being set."""
    if not bot:
        return
    try:
        await bot.set_my_commands(
            [BotCommand(command="start", description="🚀 استارت")],
    ...
    admin_commands = [
        BotCommand(command="start", description="🚀 استارت"),
        BotCommand(command="panel", description="🛠 پنل ادمین"),
    ]
    for admin_id in config.ADMIN_IDS:
        try:
            await bot.set_my_commands(admin_commands, scope=BotCommandScopeChat(chat_id=admin_id))
        except Exception as e:
            logger.debug(f"Could not set admin bot commands for {admin_id}: {e}")


async def on_startup():
    logger.info("Starting Study Coach...")
    await init_db()
    # Backfill any empty message category (STORY, MOTIVATION, ...) with its
    # ready-made samples -- see seed_sample_messages. Runs every startup but
    # is a no-op once a category already has content, so admin-added/edited
    # messages are never touched or duplicated.
    async with get_session() as _seed_session:
        await seed_sample_messages(_seed_session)
    ...
    await recover_from_crash()
    # ☰ hamburger-menu commands: /start for everyone, /panel added for
    # every configured admin -- see _register_bot_commands.
    ...
    # Evening sweep for the opt-in "برخورد جدی" tough-love feature --
    # catches genuinely inactive users the MISSED-tap check in cb_result
    # never sees (see _daily_laziness_confrontation_check /
    # UserPreference.tough_love_enabled).
    await reschedule_confrontation_job()
    # Evening sweep for per-activity "هشدار غفلت از درس/فعالیت" warnings --
    # unlike the tough-love sweep above, this looks at each subject's own
    # scheduled-parts history rather than the person's overall profile
    # (see _daily_activity_neglect_check / UserPreference.
    # activity_neglect_warnings_enabled).
    await reschedule_activity_neglect_job()
    ...
    # Plan parts queued with "📤 ارسال پارت شخصی" -- like routines, their
    # APScheduler jobs are lost on restart and re-installed from the DB
    # (anything that came due while the bot was down is skipped and its
    # owner told; see reschedule_plan_parts).
    await reschedule_plan_parts()
...
def run_webhook():
    ...
    async def webhook_handler(request: web.Request) -> web.Response:
        ...
        try:
            ...
            # exc_info=True so the log actually shows *where* (file/line)
            # the exception came from -- the bare f"...{e}" used to give
            # only the exception's message with no traceback, which made
            # tracking an error like this back to a specific line
            # basically guesswork.
