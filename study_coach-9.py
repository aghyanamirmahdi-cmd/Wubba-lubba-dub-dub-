...
def _format_roster(users: List["User"], statuses: Optional[Dict[int, str]] = None) -> str:
    ...
    the same reminder effect as being @-tagged. `statuses`, if given, maps
    user.id -> completion_status ("COMPLETED"/"PARTIAL"/"MISSED"); anyone
    missing from it is shown as still-pending (⚪). Returns a friendly
    placeholder line if empty, instead of an empty/blank section."""
    if not users:
        return "— هنوز کسی اعلام آمادگی نکرده —"
    lines = []
    for i, u in enumerate(users, start=1):
        status = (statuses or {}).get(u.id, "PENDING")
        dot = _ROSTER_STATUS_EMOJI.get(status, _ROSTER_STATUS_EMOJI["PENDING"])
        lines.append(f"{i}. {dot} {_mention_html(u)}")
    return "\n".join(lines)


async def _get_session_result_statuses(db_session: "AsyncSession", session_id: int) -> Dict[int, str]:
    """user_id -> completion_status for every result already reported on
    this session. Used to keep the roster's colored dots (see
    _format_roster / _ROSTER_STATUS_EMOJI) in sync with who has actually
    tapped 🟢/🟡/🔴 so far."""
    result = await db_session.execute(
        select(SessionResult.user_id, SessionResult.completion_status).where(
            SessionResult.session_id == session_id
        )
    )
...
async def _refresh_roster_message(session_id: int) -> None:
    ...
    try:
        async with get_session() as db_session:
            ...
            statuses = await _get_session_result_statuses(db_session, session_id)
            ...
            roster = _format_roster(ready_users, statuses)
            text = (
