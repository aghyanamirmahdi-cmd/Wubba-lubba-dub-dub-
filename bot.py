...
class ActivityRepository:
    ...
    async def get_active_by_user(self, user_id: int) -> List["UserActivity"]:
        result = await self.session.execute(
            ...
            .order_by(UserActivity.importance.desc(), UserActivity.created_at.asc())
        )
        return result.scalars().all()
...
class SessionManager:
    ...
    async def report_result(self, session_id: int, user_id: int,
        ...
        # If the user taps "🟢 کامل" or "🟡 نصفه" directly, no duration is
        # ever typed -- only the separate "⏱ ثبت مدت واقعی" button asks for
        # one. Without this, actual_duration either stayed None (PARTIAL)
        # or was blindly set to the full planned_duration (COMPLETED),
        # which meant an accidental tap 1-2 minutes after the part started
        # got credited as a complete full-length part instead of the
        # minute or two that actually passed. Using the real elapsed time
        # (capped at what was planned) fixes both: PARTIAL now gets an
        # honest duration, and an early/accidental COMPLETED tap only ever
        # records what actually elapsed.
        if actual_duration is None and completion_status in ("COMPLETED", "PARTIAL"):
            if elapsed_minutes is not None:
                actual_duration = min(elapsed_minutes, sess.planned_duration)
                ...
                # the old behavior rather than recording nothing.
...
def _format_roster(users: List["User"]) -> str:
    """Renders a numbered roster of names for a "کیا هستن؟" style
    announcement. Returns a friendly placeholder line if empty, instead
    of an empty/blank section."""
    if not users:
        return "— هنوز کسی اعلام آمادگی نکرده —"
    return "\n".join(f"{i}. {_display_name(u)}" for i, u in enumerate(users, start=1))
...
async def _finish_session_early(session_id: int):
    ...
    if group and bot:
        ...
        text = _build_session_end_message(part_number, actual_minutes, next_run_at, header=header)
        try:
...
async def _session_end(session_id: int):
    async with get_session() as db_session:
        ...
        part_number = (
            await manager.get_group_part_number(sess.group_id, session_id) if sess else None
        )
        await manager.end_session(session_id)
        actual_minutes = None
    ...
    if group_id:
        ...
        # all).
        if group and bot:
            text = _build_session_end_message(part_number, actual_minutes, next_run_at)
            try:
                await bot.send_message(group.telegram_chat_id, text)
            except Exception as e:
...
class Advisor:
    async def analyze_user(self, user_id: int, session: AsyncSession) -> Dict[str, Any]:
        ...
        if profile.completion_rate < 0.4:
            insights.append("📉 این اواخر پارت‌ها رو زیاد تموم نمی‌کنی. شاید بد نباشه یه مدت پارت‌های کوتاه‌تر (۲۰-۳۰ دقیقه) رو امتحان کنی.")
        elif profile.completion_rate > 0.8:
            insights.append("🌟 تقریباً هر پارتی که شروع می‌کنی رو هم تموم می‌کنی -- همینطوری ادامه بده!")

        if profile.starting_friction_score > 0.6:
            insights.append("🔄 این روزها انگار شروع‌کردن برات سخت‌تر شده. با پارت‌های ۱۵ دقیقه‌ای شروع کن و کم‌کم بیشترش کن.")
        elif profile.starting_friction_score < 0.3:
            insights.append("💪 شروع‌کردن برات کار سختی نیست -- جا داری پارت‌های بلندتر رو هم امتحان کنی.")

        if profile.consistency_score < 0.4:
            insights.append("📊 مطالعه‌ات این مدت یکم نامنظم بوده. سعی کن هر روز حتی یه پارت کوچیک هم داشته باشی، فقط برای اینکه ریتمت نیفته.")

        if profile.success_streak >= 3:
            insights.append(f"🔥 {profile.success_streak} پارت پشت سر هم موفق بودی! همینطوری ادامه بده.")

        if profile.miss_streak >= 2:
            insights.append(f"🌱 {profile.miss_streak} پارت رو پشت سر هم از دست دادی. اتفاقیه که می‌افته -- یه پارت کوتاه بردار و دوباره شروع کن.")

        if profile.preferred_hour:
            insights.append(f"⏰ به نظر می‌رسه حدود ساعت {profile.preferred_hour:02d}:00 بهترین حالتو داری. اگه می‌شه، پارت‌های مهم‌تر رو همون موقع بذار.")

        # --- Attendance: distinct from completion_rate -- this is about
        # whether you show up to the poll at all, not whether a part you
        # started gets finished. Low attendance with an otherwise fine
        # completion rate points at a *starting* problem, not a stamina one.
        if profile.attendance_rate < 0.4:
            insights.append(
                "📵 این اواخر تو بیشتر جلسه‌ها نبودی. اگه قضیه سر ساعتشه، از تنظیمات می‌تونی زمان مناسب‌تر خودتو انتخاب کنی."
            )
        elif profile.attendance_rate > 0.85:
            insights.append("✅ همیشه سر جلسه‌هایی -- همین ثبات، بزرگ‌ترین نقطه‌قوتته.")

        # --- Momentum vs. recovery: these two tell slightly different
        # stories. Momentum is "are you currently on a roll"; recovery is
        # "how well do you bounce back after a miss". A person can be low
        # on momentum right now but still have a strong recovery score
        # (bounces back fast), which deserves a different message than
        # someone who's low on both.
        if profile.momentum_score < 0.3 and profile.recovery_score < 0.3:
            insights.append(
                "🌧️ این روزها هم شتابت افتاده هم دیرتر از قبل بعد از وقفه‌ها برمی‌گردی. "
                "یه پارت خیلی کوچیک (۱۰-۱۵ دقیقه) همین امروز می‌تونه این چرخه رو بشکنه."
            )
        elif profile.momentum_score < 0.3 <= profile.recovery_score:
            insights.append("🔁 شتابت الان پایینه، ولی سابقه نشون می‌ده خوب برمی‌گردی. نگران روند نباش.")
        elif profile.momentum_score >= 0.7:
            insights.append("⚡ شتاب خوبی داری؛ پارت‌ها پشت سر هم دارن جلو می‌رن.")
        ...
        if profile.long_session_tolerance > 0.7 and profile.short_session_response < 0.4:
            insights.append("⏳ توی پارت‌های بلندتر بهتر کار می‌کنی؛ پارت‌های خیلی کوتاه انگار ریتمتو نمی‌گیرن.")
        elif profile.short_session_response > 0.7 and profile.long_session_tolerance < 0.4:
            insights.append("⏱ پارت‌های کوتاه‌تر برات بهتر جواب می‌دن؛ لازم نیست خودتو مجبور به پارت‌های طولانی کنی.")

        # --- Untapped potential: the gap between what the person has
        # actually managed once (maximum_observed_capacity) and where their
        # rolling current_capacity sits now. A wide gap after a good streak
        # is worth surfacing -- it's evidence, not just encouragement.
        if (profile.maximum_observed_capacity - profile.current_capacity >= 15
                and profile.success_streak >= 2):
            insights.append(
                f"🏅 یه بار تا {int(profile.maximum_observed_capacity)} دقیقه هم پیش رفتی. "
                f"با این روندی که داری، رسیدن دوباره به همون سطح خیلی دور نیست."
            )

        # --- Short vs. long-term direction: recent_average vs
        # average_session catches a shift the coarse `trend` field can miss
        # (e.g. trend stuck on STABLE while the last few parts are quietly
        # drifting up or down).
        if profile.average_session > 0:
            delta_ratio = (profile.recent_average - profile.average_session) / profile.average_session
            if delta_ratio >= 0.2:
                insights.append("📈 پارت‌های اخیرت از میانگین کلی‌ت بلندتر شدن -- ظرفیتت داره واقعاً بالا می‌ره.")
            elif delta_ratio <= -0.2:
                insights.append("📉 پارت‌های اخیرت از میانگین همیشگیت کوتاه‌تر شدن. اتفاق عجیبی نیست، فقط بدون که شاید بد نباشه پارت بعدی رو یکم کوتاه‌تر بگیری.")

        if profile.current_load > 90:
            insights.append("⚖️ امروز زیاد مطالعه کردی. بهتره یه استراحت کوتاه بکنی و با یه پارت آروم ادامه بدی.")
        elif profile.current_load < 20 and profile.miss_streak == 0:
            insights.append("🌱 امروز کمتر از معمول مطالعه کردی. یه پارت کوچیک می‌تونه ریتمتو برگردونه.")

        # Generate a friendly message
        msg = "📊 یه نگاه به روند مطالعه‌ات:\n\n"
        if insights:
            for i, insight in enumerate(insights[:7], 1):
                msg += f"{insight}\n\n"
        else:
            msg += "📊 فعلاً همه‌چی روی روال عادیه. همینطوری ادامه بده! 🌱\n\n"

        trend = profile.trend
        if trend == "UPWARD":
            msg += "📈 در کل روندت رو به رشده -- عالی پیش می‌ری."
        elif trend == "DOWNWARD":
            msg += "📉 در کل یه کم افت داشتی. نگران نباش، با قدم‌های کوچیک برمی‌گردی سر جات."
        elif trend == "STABLE":
            msg += "📊 در کل خیلی باثباتی، یعنی داری استمرار رو حفظ می‌کنی!"
        else:
            msg += "📊 روندت هنوز شکل مشخصی نگرفته. فعلاً روی همین استمرار تمرکز کن."
    ...
    async def get_recovery_suggestion(self, user_id: int, session: AsyncSession) -> str:
        ...
        profile = await profile_repo.get_by_user_id(user_id)
        if not profile:
            return "🌱 بیا با یه پارت ۱۵ دقیقه‌ای شروع کنیم و کم‌کم بیشترش کنیم."
        current = profile.current_capacity
        comfort = profile.comfort_capacity
        ...
        return (
            ...
            "چند پارت اخیر برات سخت‌تر از معمول بوده -- کاملاً طبیعیه.\n"
            f"بیا این‌بار با یه پارت {int(suggested)} دقیقه‌ای شروع کنیم و کم‌کم دوباره بالا بریم.\n\n"
            "الان هدف اینه که برگردی به ریتمت، نه اینکه یه‌جا جبران همه‌چیزو بکنی. "
            "کوچیک شروع‌کردن بعد از یه وقفه، به جای جبران افراطی، شانس واقعی برگشتن به عادتو خیلی بیشتر می‌کنه.\n"
            "آماده‌ای؟"
        )
    ...
    async def get_growth_suggestion(self, user_id: int, session: AsyncSession) -> str:
        ...
        profile = await profile_repo.get_by_user_id(user_id)
        if not profile:
            return "🌱 چند پارت موفق پشت سر هم داشتی -- می‌تونی پارت بعدی رو یه‌کم بیشتر کنی."
        current = profile.current_capacity
        challenge = profile.challenge_capacity
        ...
        return (
            ...
            f"{profile.success_streak} پارت موفق پشت سر هم داشتی! 👏\n"
            f"ظرفیتت داره بالا می‌ره -- بیا پارت بعدی رو به {int(suggested)} دقیقه برسونیم.\n\n"
            "هدف پیشرفت تدریجی و پایداره، نه یه جهش ناگهانی. "
            "همین افزایش پله‌ای و کوچیکه که باعث می‌شه به‌اندازه‌ای سخت باشه که پیشرفت بدی، "
            "نه اونقدر که انگیزه‌ات بریزه.\n"
            "آماده‌ای؟"
        )

    async def get_calm_suggestion(self, user_id: int, session: AsyncSession) -> str:
        profile_repo = ProfileRepository(session)
        profile = await profile_repo.get_by_user_id(user_id)
        if not profile:
            return "🌙 بیا امشب رو با یه پارت کوتاه و آروم جمع کنیم."
        comfort = profile.comfort_capacity
        suggested = min(comfort * 0.7, 25)
        suggested = max(10, suggested)
        suggested = min(config.CANDIDATE_DURATIONS, key=lambda x: abs(x - suggested))
        is_night = time_service.is_late_night()
        rationale = (
            "بیشتر تثبیت حافظه توی خواب اتفاق می‌افته، نه با فشار آوردن نزدیک زمان خواب -- "
            "یه مرور سبک الان بهتر از یه پارت سنگین دیروقته."
            if is_night else
            "وقتی بار امروزت زیاد بوده، یه پارت سبک‌تر همون نتیجه‌ی حفظ ریتمو می‌ده، "
            "بدون اینکه خستگی کیفیت کارتو پایین بیاره."
        )
        return (
            "🌙 حالت آرامش (Calm Mode)\n\n"
            "امروز بار زیادی رو دوشته یا وقتشه یه پارت سبک‌تر بگیری.\n"
            f"یه پارت آروم {int(suggested)} دقیقه‌ای بردار -- فقط تمومش کن، لازم نیست فشار بیاری.\n\n"
            f"{rationale}\n"
            "آماده‌ای؟"
        )
    ...
    # mode (recovery/growth/calm/normal) someone is currently in, so
    # every /advice reply also carries one concrete, evidence-based
    # *method* to apply during the part itself, not just a duration.
    _METHOD_TIPS: List[str] = [
        "🧠 روش یادآوری فعال (Active Recall): به‌جای دوباره‌خوانی، سعی کن بعد از هر پارت، "
        "بدون نگاه‌کردن به جزوه/کتاب، نکات مهم رو با صدای بلند یا روی کاغذ توضیح بدی. "
        "در پژوهش‌های حافظه، این کار نسبت به صرفاً خوندن دوباره، یادسپاری بلندمدت بهتری ایجاد می‌کنه.",
        "📅 اثر فاصله‌گذاری (Spacing Effect): مرور یک مطلب در چند روز پخش‌شده، اثرش رو حافظه "
        "به‌مراتب بیشتر از یک‌جا و فشرده خوندنشه -- حتی اگه مجموع زمان یکی باشه.",
        "🔀 آموزش به‌هم‌آمیخته (Interleaving): اگه چند موضوع/درس داری، جابه‌جا کردن بینشون "
        "توی یک پارت (به‌جای غرق شدن ساعت‌ها توی یک موضوع) معمولاً یادگیری عمیق‌تری می‌سازه.",
        "😴 خواب و تثبیت حافظه: مغز بخش مهمی از تثبیت اطلاعات رو حین خواب انجام می‌ده -- "
        "یه پارت مرور کوتاه چند ساعت قبل خواب، مؤثرتر از شب‌بیداری و فشردهٔ آخر شبه.",
    ]
    ...
    async def get_personalized_recommendation(self, user_id: int, session: AsyncSession) -> Dict[str, Any]:
        ...
        if mode == "recovery":
            suggestion = await self.get_recovery_suggestion(user_id, session)
        elif mode == "growth":
            suggestion = await self.get_growth_suggestion(user_id, session)
        elif mode == "calm":
            suggestion = await self.get_calm_suggestion(user_id, session)
        else:
            assistant = Assistant()
            suggestion = await assistant.get_next_suggestion(user_id, session)
        ...
        method_tip = self._METHOD_TIPS[tip_index]
    ...
    # ------------------------------------------------------------------
    def _suggested_duration_for_mode(self, profile: UserBehaviorProfile, mode: str) -> int:
        """Same arithmetic as get_recovery/growth/calm_suggestion, but
        returning just the number -- used when building a full day's
        schedule instead of a single conversational suggestion."""
        if mode == "recovery":
            suggested = min(15, profile.current_capacity * 0.5, profile.comfort_capacity * 0.6)
            suggested = max(10, suggested)
        elif mode == "growth":
            suggested = min(profile.current_capacity + 5, profile.challenge_capacity, 60)
            suggested = max(15, suggested)
        elif mode == "calm":
            suggested = min(profile.comfort_capacity * 0.7, 25)
            suggested = max(10, suggested)
        else:
            suggested = profile.current_capacity or profile.comfort_capacity or 30
        return min(config.CANDIDATE_DURATIONS, key=lambda x: abs(x - suggested))
    ...
    async def generate_personalized_plan(self, user_id: int, session: AsyncSession) -> Dict[str, Any]:
        ...
        mode = self._detect_personal_mode(profile, calm_enabled)
        session_minutes = self._suggested_duration_for_mode(profile, mode)

        sessions_count = max(1, min(4, round(daily_goal / session_minutes)))

        # Spread sessions across the day (spacing effect) rather than
        # back-to-back -- see RESEARCH_LIBRARY "consistency_low"/"general".
        #
        # Previously this always jumped the clock by a flat +3h per slot,
        # no matter how long a part actually was or what the user wanted --
        # e.g. two 15-minute "recovery mode" parts still ended up 3 real
        # hours apart, and it silently ignored pref.plan_gap_minutes. Once
        # the clock hit 22:00 it also stopped advancing, so a 4th slot
        # would land on the exact same time as the 3rd instead of erroring
        # or compressing -- a duplicate, not a schedule.
        #
        # Now: gap = the part's own length + the user's actual break
        # preference, and if that plan genuinely can't fit before 22:00
        # (e.g. many long parts), the gap is compressed down to a floor
        # instead of stacking sessions on top of each other.
        start_hour = min(max(profile.preferred_hour or 18, 8), 20)
        day_end_minutes = 22 * 60
        preferred_gap = pref.plan_gap_minutes if pref else 60
        window_minutes = max(0, day_end_minutes - start_hour * 60)
        gap_compressed = False
        if sessions_count > 1:
            needed_minutes = sessions_count * session_minutes + (sessions_count - 1) * preferred_gap
            if needed_minutes > window_minutes:
                gap_minutes = max(20, (window_minutes - sessions_count * session_minutes) // (sessions_count - 1))
                gap_compressed = gap_minutes < preferred_gap
            else:
                gap_minutes = preferred_gap
        else:
            gap_minutes = preferred_gap
        ...
        for _ in range(sessions_count):
            # Even after compressing the gap to its floor, a very packed
            ...
            cursor_minutes += session_minutes + gap_minutes
            ...
            while sum(remaining.values()) > 0:
                for a, _ in weighted:
                    if remaining[a.id] > 0:
                        order.append(a.id)
                        remaining[a.id] -= 1
            order = order[:sessions_count]

            schedule_lines = "\n".join(
                f"🕐 {slot} — پارت {i+1}: {session_minutes} دقیقه — "
                f"{act_by_id[order[i]].name} ({config.IMPORTANCE_LABELS.get(act_by_id[order[i]].importance, '')})"
                for i, slot in enumerate(slots)
            )

            minute_alloc: Dict[int, int] = {}
            for aid in order:
                minute_alloc[aid] = minute_alloc.get(aid, 0) + session_minutes
            total_minutes = session_minutes * sessions_count
            alloc_lines = "\n".join(
                f"• {act_by_id[aid].name}: {mins} دقیقه ({int(round(100 * mins / total_minutes))}٪)"
                for aid, mins in sorted(minute_alloc.items(), key=lambda kv: -kv[1])
            )
            alloc_section = f"📌 تخصیص زمان بر اساس اولویت:\n{alloc_lines}\n\n"

            priority_note = (
                "🧮 معیار اولویت‌بندی من: اهمیتی که خودت دادی (۱ تا ۵) × ضریب فوریت بر اساس فاصله‌ی "
                "ددلاین (هرچی ددلاین نزدیک‌تر، ضریب بیشتر) -- دقیقاً مثل اولویت‌بندی آیزنهاور، نه حدس."
            )
            deferred = [a for a, _ in weighted if counts.get(a.id, 0) == 0]
            if deferred:
                deferred_names = "، ".join(a.name for a in deferred)
        ...
        message = (
            f"🗓 برنامه شخصی امروز\n\n"
            f"🎯 هدف کلی: {daily_goal} دقیقه در {sessions_count} پارت\n\n"
            f"{schedule_lines}\n\n"
            f"{alloc_section}"
            f"⏸ بین پارت‌ها {gap_minutes} دقیقه فاصله در نظر گرفتم (حداقل {config.MIN_BREAK_MINUTES} دقیقه‌ش رو "
            f"حتماً استراحت کامل باش -- بدون گوشی/شبکه اجتماعی، تا مغز واقعاً استراحت کنه). "
            f"می‌تونی این فاصله رو از ⚙️ تنظیمات › فاصله بین پارت‌ها عوض کنی.\n\n"
            + (
                "⚠️ با تعداد و طول پارت‌های امروز، فاصله‌ی دلخواهت قبل از ساعت ۲۲ جا نمی‌شد، "
                "برای همینه که کمی فشرده‌ترش کردم.\n\n"
                if gap_compressed else ""
            )
            + f"{mode_labels.get(mode, mode_labels['normal'])}\n\n"
            f"{'─' * 20}\n\n"
            f"{research_tip}"
            f"{activities_hint}\n\n"
...
async def _send_personalized_plan(telegram_user_id: int, target: Message):
    """Shared by /plan, the '🗓 برنامه امروز' button, and the daily
    proactive push -- builds a full-day plan (how many parts, roughly
    when, how long each) instead of just the single next-part suggestion
    /advice gives, and folds in one research-backed tip from
    ...
    async with get_session() as session:
        ...
        if not user:
            await target.answer("❌ لطفاً ابتدا /start را بزنید.")
            return
        result = await advisor.generate_personalized_plan(user.id, session)
        await target.answer(result["message"], reply_markup=main_menu())
...
async def cb_result(callback: CallbackQuery, state: FSMContext):
    ...
    if status == "ACTUAL":
        ...
        await callback.message.answer(
            "⏱ لطفاً مدت واقعی مطالعه را به دقیقه وارد کنید:",
...
async def cb_cancel_input(callback: CallbackQuery, state: FSMContext):
    ...
    if target == "admin" and is_admin:
        ...
        current_hours = await get_backup_interval_hours()
        await callback.message.edit_text(
            f"💾 پشتیبان‌گیری\n⏱ فاصله فعلی: هر {current_hours} ساعت.",
            reply_markup=backup_menu_keyboard(),
        )
