...
# Laziness, as defined here, is a *sustained pattern* -- repeated misses,
# a persistently low completion rate, or real inactivity over days -- and
# the right response (per what the admin asked for) is a direct,
# accountability-style message instead. The two are intentionally never
# triggered by the same signal.
class LazinessAssessor:
    """Turns a UserBehaviorProfile + days-since-any-activity into an
    escalation tier ("firm" / "severe" / None), by counting how many
    independent laziness signals fire at once. Requiring 2+ signals
    together (see Config.CONFRONTATION_MIN_SIGNALS) is what keeps this
    from firing off a single unlucky day -- e.g. completion_rate alone
    dipping under the threshold isn't enough by itself."""
    ...
    @staticmethod
    def cooldown_ok(profile: "UserBehaviorProfile") -> bool:
        if profile.last_confrontation_at is None:
            return True
        elapsed = datetime.utcnow() - profile.last_confrontation_at
        return elapsed.total_seconds() >= config.CONFRONTATION_COOLDOWN_HOURS * 3600
...
async def maybe_send_confrontation(
    ...
    through this one function, so the opt-in check, the signal
    threshold, and the cooldown are only ever enforced in one place.
    Returns True iff a DM was actually sent (and records last_confrontation_at
    so the cooldown takes effect immediately, even if the caller's own
    transaction doesn't commit until later)."""
    if bot is None:
        return False
    pref_result = await session.execute(
    ...
    if not LazinessAssessor.cooldown_ok(profile):
        return False
    tier = LazinessAssessor.assess(profile, days_inactive)
    if tier is None:
        return False
    text = await message_selector.get_message(
        session,
        "TOUGH_LOVE",
        user_row.id,
        miss_streak=profile.miss_streak,
    ...
    try:
        ...
        return False
    profile.last_confrontation_at = datetime.utcnow()
    await session.flush()
    return True
    ...
    async def generate_personalized_plan(self, user_id: int, session: AsyncSession,
        ...
        def _schedule_row(i: int, slot: str) -> str:
            ...
            if day_offsets[i] >= 1:
                ...
                day_tag = ""
            row = f"🕐 <b>{slot}</b>{day_tag} — پارت {i + 1}: {durations[i]} دقیقه{label}"
            if i < len(breaks):
        ...
        header_text = (
            ...
            f"(تعداد پارت‌ها بر اساس طول دلخواهت برای هر پارت -- حدود {base_minutes} دقیقه -- "
            f"و مجموع دقیقه‌ها بر اساس هدف روزانه‌ت تعیین شده؛ طول هر پارت با اولویت فعالیت و "
            f"ریتم روزت هم تنظیم می‌شه، نه یه عدد ثابت)"
...
def result_keyboard(session_id: int) -> InlineKeyboardMarkup:
    ...
    # from when the whole part started.
...
async def _start_personal_session(telegram_user_id: int, target: Message):
    ...
    async with get_session() as session:
        ...
        await target.answer(
            ...
            f"وقتی تموم شد بزن 👇",
            reply_markup=result_keyboard(sess.id),
        )
        ...
        try:
            async with get_session() as session:
                ...
                if not fresh_profile:
                    continue
                days_inactive = (
                    time_service.now() - time_service.to_tehran(fresh_profile.last_updated)
                ).total_seconds() / 86400
                await maybe_send_confrontation(session, user_row, fresh_profile, days_inactive=days_inactive)
        except Exception as e:
