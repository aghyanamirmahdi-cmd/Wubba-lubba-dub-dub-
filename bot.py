...
class Config:
    ...
    # into. sessions_count (see Advisor.generate_personalized_plan) is
    # now driven by daily_goal / interval_minutes -- i.e. a real,
    # user-facing number instead of a silent hardcoded "4" -- so this
    # only kicks in for genuinely extreme inputs (e.g. a very large
    # daily_goal with a very short preferred session length) instead of
    # routinely truncating ordinary goals the way the old fixed cap did.
    MAX_DAILY_PLAN_PARTS: int = int(os.getenv("MAX_DAILY_PLAN_PARTS", "8"))
...
def _format_roster(
    ...
    instead of an empty/blank section. `late_ids`, if given, is the set of
    user ids who never tapped "هستم" and only showed up by reporting a
    result directly -- they get a 🆕 "تازه‌پیوست" tag so the roster is
    honest about who was actually in the original headcount vs who
    joined via a late report."""
    if not users:
    ...
    for i, u in enumerate(users, start=1):
        ...
        late_note = " 🆕 <i>(تازه‌پیوست)</i>" if late_ids and u.id in late_ids else ""
        lines.append(f"{i}. {dot} {_mention_html(u)}{minutes_note}{late_note}")
    return "\n".join(lines)
...
async def _refresh_roster_message(session_id: int) -> None:
    ...
    try:
        async with get_session() as db_session:
            ...
            late_note = f" + 🆕 {len(late_ids)} تازه‌پیوست" if late_ids else ""
...
async def _finalize_roster_message(
    ...
    closed-out one from `_build_completed_roster_text`, and drops the
    "پارت تموم شد" / result buttons since tapping them after the part has
    already ended does nothing. Same message_id `_refresh_roster_message`
    was patching throughout the part -- so the one card people had pinned
    the whole time visibly changes state instead of quietly going stale.
    Best-effort and silent on failure, same reasoning as
...
class Advisor:
    ...
    async def generate_personalized_plan(self, user_id: int, session: AsyncSession,
        ...
        # No more hardcoded "at most 4 parts" -- the count now actually
        # reflects daily_goal / interval_minutes, with only a generous
        # safety ceiling (config.MAX_DAILY_PLAN_PARTS) for extreme inputs.
        sessions_count = max(
            1,
...
async def _start_personal_session(message: Message):
    """The /session command run from a private chat: a solo part that
    belongs to nobody's group (Session.group_id stays None,
    owner_user_id is this person). No attendance poll -- there's only
    one person to ask -- so it goes straight to STARTED, same shape as
    a group part that already finished its "هستید؟" phase. Completion
    (result:<id>:COMPLETED/MISSED/MANUAL) reuses the exact same
    cb_result / report_result / _finish_session_early code path as
    group parts: with a single READY participant, reporting a result
    always satisfies the early-finish check, so the part ends itself
    the moment this person taps a result button -- no separate personal
    "session end" flow needed. See _has_active_group_lock /
    _has_active_personal_lock for the mutual-exclusion rule."""
    async with get_session() as session:
        repo = UserRepository(session)
...
async def cmd_session(message: Message):
    if message.chat.type == "private":
        await _start_personal_session(message)
        return
    async with get_session() as session:
...
async def _send_personalized_plan(telegram_user_id: int, target: Message,
    ...
    if no_data_message is not None:
        await target.answer(no_data_message, reply_markup=main_menu())
        return
...
async def cb_motivate_me(callback: CallbackQuery):
    ...
    in, not just another generic MOTIVATION quote. Sent as a fresh message
    rather than an alert popup because story texts can exceed Telegram's
    200-char alert limit, and it works the same in a group or in PV."""
    async with get_session() as session:
        user_repo = UserRepository(session)
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "/session - ایجاد پارت جدید (داخل گروه: پارت گروهی | داخل چت خصوصی: پارت شخصی مستقل)\n"
    ...
    for user_row in all_users:
        ...
        try:
            ...
            if delivery in ("group", "both"):
                ...
                group_text = f"🌅 برنامه شخصی امروز {_mention_html(user_row)}:\n\n" + plan["message"]
                for group in groups:
                    try:
                        await bot.send_message(
                            group.telegram_chat_id, group_text, reply_markup=plan_keyboard(plan_parts)
                        )
                    except Exception as e:
                        logger.debug(
