...
async def _schedule_next_poll(session_id: int):
    ...
    async with get_session() as session:
        ...
        if run_at < now:
            ...
            # extra buffer on top, offer the next poll right away.
            run_at = now
...
async def _finish_session_early(session_id: int):
    ...
    if group and bot:
        try:
            await bot.send_message(
                group.telegram_chat_id,
                "🎉 همه اعلام اتمام کردن! پارت بعدی به‌زودی برنامه‌ریزی می‌شه.",
            )
        except Exception as e:
...
async def cb_result(callback: CallbackQuery, state: FSMContext):
    ...
    async with get_session() as session:
        ...
        if success:
            ...
            await callback.message.answer(f"{emoji} گزارش ثبت شد! ممنون از همراهی‌ات 🌱")
