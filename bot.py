...
async def init_db():
    ...
    if async_url.startswith("sqlite"):
        # SQLite allows only one writer at a time. This app opens nested
        # sessions on separate connections (e.g. SchedulerService.schedule_job
        # writing to scheduler_jobs while a caller's outer session is still
        # open/uncommitted), which under the default rollback-journal mode
        # causes "database is locked" errors almost immediately. WAL mode
        # lets readers proceed without blocking on a writer, and the
        # busy_timeout above makes a second writer wait for the first to
        # commit instead of failing right away.
        def _set_sqlite_pragma(dbapi_connection, connection_record):
            ...
            cursor.close()
        listen(_engine.sync_engine, "connect", _set_sqlite_pragma)
